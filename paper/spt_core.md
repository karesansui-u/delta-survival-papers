...
4. **Formal verification**: 428 Lean 4 modules, zero sorry/admit/axiom, with conditional bridges to 60+ fields, a theorem map separating formal claims from non-claims, and a bundled `MLDomain` layer for persistence-facing projections of several major theories (§6).
...
- Of the ~428 modules, approximately 20 are **Tier A** (core-routed, with non-trivial conclusions). Approximately 160 are **Tier C** (vocabulary mappings, not theorems). The mathematical weight is concentrated in Tier A and the core.
...
The ~428 modules include cross-domain bridges at three tiers:
...
428 modules. 3,592 build jobs. sorry = 0. admit = 0. axiom = 0.
