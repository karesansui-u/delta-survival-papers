...
4. **Formal verification**: 433 Lean 4 modules, zero sorry/admit/axiom, with conditional bridges to 60+ fields, a theorem map separating formal claims from non-claims, and a bundled `MLDomain` layer for persistence-facing projections of several major theories (§6).
...
- Of the ~433 modules, approximately 20 are **Tier A** (core-routed, with non-trivial conclusions). Approximately 160 are **Tier C** (vocabulary mappings, not theorems). The mathematical weight is concentrated in Tier A and the core.
...
The ~433 modules include cross-domain bridges at three tiers:
...
A newer repair-affordability spine makes the interface operational rather than merely semantic. Kalman target covariance recovery is tied to a closed-form covariance trajectory, an observation lower bound, and an observation budget. BSC target reliability is tied to a BSC-native retransmission profile `1 - errorRate^(r+1)`, bounded monotone reliability certificates, redundancy cost, and repair budget. The finite-block layer is two-sided but certificate-mediated: achievability certificates can imply affordable target recovery, while finite obstruction certificates can imply target-recovery infeasibility; optional rate/capacity metadata may be carried, but it does not generate the certificate. The rank-probability substrate now includes generic finite-uniform event ratios, a bare IID sequence PMF scaffold with length-two compatibility, recursive IID event-product skeletons, one-step uniform-PMF event masses in both `ENNReal` and real readout forms, rectangular finite-uniform and sequential PMF product-event masses including fixed two-state prefix escape, supplied constant conditional-mass dependent-event rules, supplied-state binary prefix escape readouts, proof-indexed and totalized on-event evolving successor-state two-step rules, canonical empty-prefix state and one-step mass certificates, deterministic event-to-rank-growth / prefix-state / explicit state-transition lemmas, supplied-step product certificates, and coherent supplied prefix-growth process certificates. It now proves a finite arbitrary-length IID binary full-column-rank event mass theorem: the event `finrank(span cols) = m` has real mass `fullRankEscapeProduct r m`, and the complementary rank-failure event has `eventProb = 1 - fullRankEscapeProduct r m`; row slack `m + slack <= r` yields the `hRank`-shaped upper bound `eventProb(rank failure) <= 2^{-slack}`. That generated rank witness is consumed by a finite-block M/L target-recovery theorem while the cover witness `hcover` and erasure-tail witness `hTail` remain supplied. This still does not derive row slack from rate/capacity, does not plug the resulting witness into a capacity-achieving construction, and does not prove Shannon's asymptotic coding theorem or a strong converse. The causal layer packages adjustment-formula certificates as producer data for causal-effect and information-gain readouts, without deriving do-calculus or identifiability criteria. These are target-relative recovery statements: the domain supplies a native trajectory, certificate, or witness; the M/L ledger decides whether the target-restoring action is affordable.
...
- **Information-theoretic witnesses/certificates, not Shannon coding theorem**: The BSC and finite-block layers connect retransmission profiles and finite-block achievability/obstruction witnesses and certificates to M/L affordability. Binary span-escape lemmas now reach one-step finite-uniform PMF event-mass bridges, sequential and recursive IID product skeletons, supplied-state prefix escape readouts, deterministic event-to-rank-growth / prefix-state / explicit state-transition lemmas, and a finite arbitrary-length IID binary full-column-rank event mass theorem. In particular, `finrank(span cols) = m` has real mass `fullRankEscapeProduct r m`, the complementary rank-failure event has `eventProb = 1 - fullRankEscapeProduct r m`, row slack `m + slack <= r` yields the `hRank`-shaped upper-bound form, and the generated rank witness is consumed by a finite-block M/L target-recovery theorem while `hcover` and `hTail` remain supplied. They do not derive row slack from rate/capacity, and they do not formalize an asymptotic Shannon coding theorem, capacity-achieving construction, or strong converse.
...
433 modules. 3,597 build jobs. sorry = 0. admit = 0. axiom = 0.
