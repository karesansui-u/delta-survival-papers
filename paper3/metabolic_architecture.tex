...
Most metabolism experiments use open-source models (8B--27B parameters), but a completed frontier full-pipeline replication on Sonnet~4.6 ($n = 3$, 30 turns) yields T30 fact recall means of 95.6\% $\pm$ 3.8 (\texttt{ON}), 97.8\% $\pm$ 3.8 (\texttt{NC}), and 46.7\% $\pm$ 29.1 (\texttt{OFF}), with \texttt{ON} $>$ \texttt{OFF} in all three trials.
...
We now also have a completed frontier full-pipeline replication on Sonnet~4.6 (Table~\ref{tab:frontier_pipeline}), but it remains only one frontier model family and 30 turns. Early Gemini runs exposed setup-path and pipeline-format failures that were not model failures per se (confirmed by direct recall control, Section~\ref{sec:frontier}), so broader frontier validation remains model-specific and incomplete.
...
First, after completing a 30-turn $n = 3$ Sonnet~4.6 full-pipeline replication, the next step is longer-horizon validation and then replication across other frontier models (GPT-4o, Gemini, future Claude variants). The Exp35-R replication established contradiction resistance patterns; the new controlled replication shows that full metabolism is feasible on at least one frontier model, but not yet general.
...
Frontier model replication ($n = 1$ per condition) suggests that $\delta_c$ is model-specific, producing three distinct response patterns: lightweight models collapse ($\delta > \delta_c$), frontier models resist ($\delta < \delta_c$), and GPT-4o exhibits non-retention where safety policies prevent information accumulation entirely. A controlled frontier full-pipeline replication on Sonnet~4.6 ($n = 3$, 30 turns) further shows \texttt{ON} $\approx$ \texttt{NC} and \texttt{ON} $\gg$ \texttt{OFF} on fact recall (mean T30: 95.6\%, 97.8\%, 46.7\%), indicating that the metabolic effect may extend beyond local open-source models.
...
These results suggest that metabolism is a promising approach to context rot for open-source models that lack intrinsic contradiction resistance and, under controlled replication, at least one frontier model. At the same time, the evidence remains early, and larger model sets, longer frontier runs, and cross-language validation are still required.
