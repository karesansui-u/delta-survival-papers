...
\title{\textbf{Cognitive Sleep for LLMs:\\How Contradiction Metabolism Prevents Context Rot}}
...
We present evidence that the primary driver is \emph{contradiction accumulation} rather than context length:
even Google's 1M-token context window suffers a 47.8 percentage point accuracy drop when contradictions are present, while removing contradictions restores performance regardless of length.
We propose a metabolic architecture inspired by human sleep, in which an external layer processes knowledge during idle time---detecting, resolving, and preserving contradictions as temporal pairs.
Across 8 open-source models (8B--27B parameters) and 11 paired comparisons over 180-turn dialogues, metabolism-enabled systems significantly outperform controls (sign test $p = 0.0107$).
In a controlled three-condition experiment with gemma3:27b ($n = 3$ per condition), metabolism achieves 73.3\% accuracy versus 21.1\% without metabolism (Kruskal-Wallis $p = 0.027$; all three groups fully rank-separated).
An unexpected observation reveals that metabolized systems exceed even the contradiction-free baseline (73.3\% vs.\ 56.7\%), suggesting that contradiction pairs may act as knowledge anchors that prevent fact retrieval degradation over time.
\end{abstract}
...
We propose \emph{cognitive sleep} for LLMs: an external metabolism layer that processes accumulated knowledge during idle time, detecting contradictions, resolving conflicts, and maintaining knowledge integrity.
...
\item \textbf{Metabolic architecture (strong):} We propose and validate an external metabolism architecture that prevents contradiction-induced collapse. Across 8 models, metabolism significantly improves performance ($p = 0.0107$), with the strongest effect yielding $+$52.2pp improvement ($p < 0.001$, $d = 8.80$).
...
That work (currently under review) proved the problem exists; this paper presents an engineering solution to keep $\delta$ near zero through active metabolism.
...
The value $k = 3.0$ was selected empirically to produce meaningful S-value differentiation across the observed range of $\delta$ (typically 0--0.3); higher values cause the rollback threshold to trigger too frequently, while lower values reduce sensitivity to contradiction accumulation.
...
Three resolution strategies emerged from implementation:
...
ON vs.\ OFF & 0.05 (MW) & 0.014 (paired $t$) & $r = 1.00$; full rank separation \\
ON vs.\ NC & 0.05 (MW) & 0.013 (paired $t$) & $r = 1.00$; full rank separation \\
NC vs.\ OFF & 0.05 (MW) & 0.029 (paired $t$) & $r = 1.00$; full rank separation \\
\bottomrule
...
\caption{Three-condition comparison ($n = 3$). Error bars show $\pm 1$ SD. Metabolism ON significantly exceeds both OFF and the contradiction-free baseline (NC). Brackets indicate statistically significant pairwise differences.}
\label{fig:threecond}
\end{figure}
...
\subsection{The Sonnet Exception}

Sonnet~4.6 is the only model tested that shows zero degradation under contradiction (Table~\ref{tab:exp35}).
This demonstrates that contradiction resistance is achievable within a model's weights, at least for recognition tasks.
However, Sonnet's resistance is passive---it tolerates contradictions without managing them.
For the 5 of 6 models that lack this resistance, external metabolism provides a model-agnostic solution.
Furthermore, even for contradiction-resistant models, the knowledge anchoring benefit (preventing fact retrieval degradation) remains relevant, as Sonnet does not address the gradual burial of old facts under new conversation.
...
All metabolism experiments use local open-source models (8B--27B parameters).
No frontier model (GPT-4o, Claude) has been tested with the full metabolism pipeline.
The Exp35 results (Table~\ref{tab:exp35}) demonstrate that the contradiction problem is universal across model sizes, but the solution's effectiveness on frontier models remains an open question.
...
Measuring vector search hit rates across conditions would provide stronger evidence.

\subsection{Future Work}

Several directions follow naturally from these results.
First, frontier model validation: testing the full metabolism pipeline with GPT-4o and Claude would establish whether the approach scales beyond local models.
Second, cross-model $n = 3$ replication with different architectures (e.g., Mistral, LLaMA) would strengthen generality.
Third, direct measurement of the knowledge anchoring effect via retrieval probability analysis would elevate the exploratory observation to a confirmed finding.
Fourth, cross-language benchmarks in English and Chinese would address the language limitation.
Finally, the metabolism architecture suggests a lightweight middleware variant---scanning and resolving contradictions in API requests before they reach the LLM---that could provide benefits without the full four-layer memory system.
...
Context rot in LLMs is driven primarily by contradiction accumulation rather than context length.
This pattern is consistent across vendors: even Google's 1M-token context window drops 47.8 percentage points under contradiction, while removing contradictions restores performance regardless of length.

An external metabolism architecture---processing contradictions during idle time, analogous to memory consolidation during sleep---prevents this collapse with high statistical significance.
Across 8 models and 11 paired comparisons, metabolism-enabled systems consistently outperform controls ($p = 0.0107$).
In a controlled three-condition experiment ($n = 3$ per condition), metabolism achieves 73.3\% accuracy where unmetabolized systems collapse to 21.1\% (Kruskal-Wallis $p = 0.027$; complete rank separation between all groups).

An unexpected exploratory observation suggests that metabolism provides benefits beyond contradiction resolution.
Metabolized systems exceeded even the contradiction-free baseline in all three trials ($+$16.7pp), potentially because contradiction pairs serve as retrieval anchors that maintain fact accessibility.
If confirmed by direct measurement, this dual benefit---preventing both cognitive collapse and knowledge forgetting---would establish metabolism as a fundamental component of long-running LLM systems.
