...
We show that the primary driver is \emph{contradiction accumulation}, not context length.

We teach an LLM a set of facts, then conduct a 180-turn conversation with it. Afterward, we ask it to recall those facts.
With ordinary conversation alone, accuracy drops to 57\%---the model forgets nearly half.
When contradictory information is mixed in, accuracy collapses to 21\%.
With an external metabolism layer that detects and resolves contradictions during idle time (inspired by human sleep), accuracy reaches 73\%---exceeding even the contradiction-free baseline ($n = 3$ per condition, $p = 0.027$, $d = 8.80$).

This pattern is consistent across models: in 8 models and 11 paired comparisons, metabolism consistently outperforms controls ($p = 0.0107$).
Even with Google's 1M-token context window, contradictions cause a 47.8pp accuracy drop, while removing contradictions restores performance regardless of length.

Replication with frontier models reveals that the collapse threshold is model-specific: lightweight models (GPT-4o-mini, Gemini Flash) collapse catastrophically, while frontier models (Sonnet~4.6, Gemini~3.1) show conditional resistance.
However, a factor analysis uncovers a \emph{capability--vulnerability paradox}: high-capability models comply with coercive fact overrides that low-capability models reject, because RLHF helpfulness training makes them interpret contradictions as legitimate user corrections.
A lightweight middleware (\texttt{delta-prune}) that resolves contradictions before they enter the model's context eliminates this compliance in preliminary testing.
\end{abstract}
