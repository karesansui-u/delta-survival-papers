...
Most metabolism experiments use open-source models (8B--27B parameters), but a preliminary frontier full-pipeline pilot on Sonnet~4.6 ($n = 1$, 30 turns) yields \texttt{ON / NC / OFF} fact recall of 93.3\% / 93.3\% / 26.7\%.
...
\paragraph{Frontier full-pipeline pilot.}
We then ran a preliminary full-pipeline pilot on Sonnet~4.6 using the actual DeltaZero metabolism pipeline under three conditions: \texttt{ON} (contradictions plus metabolism), \texttt{OFF} (contradictions without metabolism), and \texttt{NC} (no contradictions, metabolism enabled). The run used the production-relevant \texttt{standard setup} path, a coercive F3 contradiction protocol, 30 turns, and benchmarks at T15 and T30. Table~\ref{tab:frontier_pipeline} reports the results.

\begin{table}[htbp]
\centering
\caption{Preliminary frontier full-pipeline pilot on Sonnet~4.6 (30 turns, standard setup, F3 contradiction protocol, $n = 1$). Percentages are benchmark accuracy on 15 questions per category.}
...
The pattern is qualitatively clean: \texttt{ON} $\approx$ \texttt{NC} and \texttt{ON} $\gg$ \texttt{OFF}. At T30, fact recall is 93.3\% for both \texttt{ON} and \texttt{NC}, versus 26.7\% for \texttt{OFF}. This is still only a pilot ($n = 1$), so it does not support a statistical claim. However, it is the first evidence in this project that the full metabolism pipeline can transfer to at least one frontier model without degrading into the setup-path failures observed in early Gemini runs.
...
First, the initial Sonnet~4.6 full-pipeline pilot should be extended to multi-trial and longer-horizon validation, and then replicated across other frontier models (GPT-4o, Gemini, future Claude variants). The Exp35-R replication established contradiction resistance patterns; the new pilot shows that full metabolism is feasible on at least one frontier model, but not yet general.
...
Frontier model replication ($n = 1$ per condition) suggests that $\delta_c$ is model-specific, producing three distinct response patterns: lightweight models collapse ($\delta > \delta_c$), frontier models resist ($\delta < \delta_c$), and GPT-4o exhibits non-retention where safety policies prevent information accumulation entirely. A preliminary frontier full-pipeline pilot on Sonnet~4.6 further shows \texttt{ON} $\approx$ \texttt{NC} and \texttt{ON} $\gg$ \texttt{OFF} on fact recall (93.3\%, 93.3\%, 26.7\% at T30), indicating that the metabolic effect may extend beyond local open-source models.
...
These results suggest that metabolism is a promising approach to context rot for open-source models that lack intrinsic contradiction resistance and, in preliminary pilot form, at least one frontier model. At the same time, the evidence remains early, and larger samples, longer frontier runs, and cross-language validation are still required.
