...
Replication with frontier models (GPT-4o, Gemini~3.1, Sonnet~4.6) reveals that the collapse threshold $\delta_c$ is model-specific, producing three distinct patterns: catastrophic collapse in lightweight models, conditional resistance in frontier models, and safety-policy-driven non-retention in GPT-4o.
A factor analysis of contradiction type and force level uncovers a \emph{capability--vulnerability paradox}: high-capability models comply with coercive fact overrides that low-capability models reject, because the same RLHF helpfulness training that makes them useful makes them susceptible to contradictions framed as user corrections.
A lightweight middleware (\texttt{delta-prune}) that resolves contradictions before they enter the model's context eliminates this compliance in preliminary testing ($n = 1$).
\end{abstract}
