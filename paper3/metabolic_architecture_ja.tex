...
\title{\textbf{LLMに認知的睡眠を：\\矛盾代謝によるContext Rotの防止}}
...
%==============================================================================
\begin{abstract}
大規模言語モデル（LLM）は長時間の対話において性能が劣化する——この現象は「context rot」として知られる。
従来の支配的な仮説は、コンテキスト長の制約がその原因であるというものだった。
本研究は、その主要因がコンテキスト長ではなく\emph{矛盾の蓄積}であることを示す。
Googleの100万トークンコンテキストウィンドウでさえ、矛盾が存在する場合に47.8パーセントポイントの精度低下を示し、一方で矛盾を除去すればコンテキスト長に関わらず性能は維持される。
我々は、人間の睡眠に着想を得た代謝アーキテクチャを提案する。外部レイヤーがアイドル時間中に知識を処理し、矛盾を検出・解消し、時間的ペアとして保存する。
8つのオープンソースモデル（8B〜27Bパラメータ）と11組のペア比較（180ターン対話）において、代謝有効システムは有意にコントロールを上回った（符号検定 $p = 0.0107$）。
gemma3:27bを用いた3条件統制実験（各条件$n = 3$）では、代謝ONが73.3\%の精度を達成し、代謝OFFの21.1\%を大幅に上回った（Kruskal-Wallis $p = 0.027$、全3群が完全にランク分離）。
予想外の観察として、代謝システムは矛盾のない基準条件（56.7\%）さえも上回り、矛盾ペアが事実検索の劣化を防ぐ知識アンカーとして機能する可能性を示唆する。
フロンティアモデル（GPT-4o、Gemini~3.1、Sonnet~4.6）での追試により、崩壊閾値$\delta_c$がモデル固有であり、軽量モデルの壊滅的崩壊、フロンティアモデルの条件付き耐性、GPT-4oの安全ポリシーによる非保持の三類型を生むことが判明した。矛盾タイプと強制力の因子分析は\emph{能力-脆弱性パラドックス}を明らかにする：高能力モデルは低能力モデルが拒否する強制的事実上書きに従う。RLHFの「helpfulness」訓練が有用性と脆弱性を同時に生む。矛盾をモデルのコンテキストに入る前に解消する軽量ミドルウェア（\texttt{delta-prune}）は、予備テスト（$n = 1$）でこのcomplianceを排除した。
\end{abstract}
...
我々はLLMのための\emph{認知的睡眠}を提案する：アイドル時間中に蓄積された知識を処理し、矛盾を検出し、対立を解消し、知識の整合性を維持する外部代謝レイヤーである。
...
\begin{enumerate}
\item \textbf{問題の再定義（強い）：} context rotの主要因がコンテキスト長ではなく矛盾蓄積であることを、ベンダー横断的に示す。100万トークンのコンテキストウィンドウは矛盾に対して保護を提供しない。
\item \textbf{代謝アーキテクチャ（強い）：} 矛盾誘起崩壊を防止する外部代謝アーキテクチャを提案・検証する。8モデルにおいて代謝は有意に性能を改善し（$p = 0.0107$）、最大$+$52.2ppの改善を示す。
...
さらにその研究は、非独立な制約を弱依存性サンドイッチ（$e^{-\delta(1+\rho)} \leq P \leq e^{-\delta(1-\rho)}$）で扱う枠組みに拡張しており、制約相関$\rho = 0$で$e^{-\delta}$に厳密帰着し、それ以外では保証付き保守的上界を提供する（Lean~4で19モジュール、286命題、$\texttt{sorry} = 0$）。
...
矛盾検出カテゴリは構造的パラドックスにより除外：代謝が矛盾を成功裏に解消すると、モデルは正しく「矛盾は存在しない」と回答する——これをベンチマークは失敗と採点する。詳細は付録~\ref{app:rejudge}を参照。
...
Exp35結果（表~\ref{tab:exp35}）はモデルあたり単一実行（$n = 1$）であり、統計的に決定的ではなく例示的と解釈すべき。フロンティアモデル追試（\S\ref{sec:frontier}）も$n = 1$である。

\paragraph{モデルスコープ}
代謝実験はローカルオープンソースモデル（8B〜27B）を使用。フロンティアモデル（GPT-4o、Gemini~3.1、Sonnet~4.6）は矛盾耐性を検証したが（\S\ref{sec:frontier}）、完全な代謝パイプラインでは未検証。DeltaZeroのExtractorはAPIモデルの応答形式に最適化されておらず、事実想起の劣化はモデル固有ではなくパイプライン固有である（直接想起制御により確認）。
...
フロンティアAPIモデルでの完全代謝パイプライン検証、クロスモデル$n = 3$反復、知識アンカリング効果の直接測定、英語・中国語でのクロス言語ベンチマーク、および$\delta_c$のモデルサイズ・RLHF強度との関係特性化が今後の方向性である。
...
LLMのcontext rotは、コンテキスト長ではなく、主に矛盾蓄積によって駆動される。
Googleの100万トークンコンテキストウィンドウでさえ矛盾下で47.8パーセントポイント低下する一方、矛盾除去により長さに関わらず性能が回復する。

アイドル時間中に矛盾を処理する外部代謝アーキテクチャ——睡眠中の記憶統合に類似——は、高い統計的有意性でこの崩壊を防止する。
8モデル・11ペア比較において、代謝有効システムは一貫してコントロールを上回った（$p = 0.0107$）。
統制3条件実験（各条件$n = 3$）では、代謝が73.3\%の精度を達成し、非代謝システムは21.1\%に崩壊した（Kruskal-Wallis $p = 0.027$、全群完全ランク分離）。

フロンティアモデル追試により、$\delta_c$がモデル固有であり、3つの異なる応答パターンを生むことが明らかになった。軽量モデルは崩壊し（$\delta > \delta_c$）、フロンティアモデルは耐性を示し（$\delta < \delta_c$）、GPT-4oは安全ポリシーが情報蓄積自体を防止する非保持を示す。
知識アンカリング効果——矛盾ペアが検索精度を改善する現象——はgemma3:27b（代謝あり$+$16.7pp、$n = 3$）で観察され、Gemini~3.1（代謝なし$+$2.2pp、$n = 1$）で方向的に一致する結果が得られており、さらなる調査が必要である。

これらの結果は、固有の矛盾耐性を持たない大多数のモデルに対するcontext rotのモデル非依存解決策としての代謝を確立するとともに、そのアプローチが適用される境界条件を明示する。
