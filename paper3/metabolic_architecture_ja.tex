...
本研究は、主要因がコンテキスト長ではなく\emph{矛盾の蓄積}であることを示す。

LLMに事実を教え、180ターンの対話を行い、教えた事実を再度質問する。
通常の会話だけで正答率は57\%に低下する——半分近くを忘れる。
矛盾する情報を混入させると、正答率は21\%に崩壊する。
人間の睡眠に着想を得た外部代謝レイヤー（アイドル時間中に矛盾を検出・解消）を適用すると、正答率は73\%に達し、矛盾のない基準条件すら上回る（各条件$n = 3$、$p = 0.027$、$d = 8.80$）。

この傾向はモデル間で一貫する：8モデル・11組のペア比較で代謝は一貫してコントロールを上回った（$p = 0.0107$）。
Googleの100万トークンコンテキストウィンドウでも矛盾は47.8ppの精度低下を引き起こし、矛盾除去で性能が回復する。

フロンティアモデルでの追試により、崩壊閾値がモデル固有であることが判明：軽量モデル（GPT-4o-mini、Gemini Flash）は壊滅的に崩壊し、フロンティアモデル（Sonnet~4.6、Gemini~3.1）は条件付き耐性を示す。
しかし因子分析は\emph{能力-脆弱性パラドックス}を明らかにする：高能力モデルは、低能力モデルが拒否する強制的事実上書きに従う。RLHFの「helpfulness」訓練が矛盾をユーザーの正当な訂正として解釈させるためである。
矛盾をモデルのコンテキストに入る前に解消する軽量ミドルウェア（\texttt{delta-prune}）は、予備テストでこのcomplianceを排除した。
\end{abstract}
