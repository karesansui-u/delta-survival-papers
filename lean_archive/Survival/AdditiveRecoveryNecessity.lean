...
/-!
# Additive Recovery Necessity Theorem

Proves that the additive net consumption model b_t = d_t - r_t
is a **necessary consequence** of the log-ratio loss form.
...
This is not a modeling choice — it is forced by the logarithm's
additive property, which itself is forced by the representation theorem.

## Significance

"Why is net consumption additive?" → "Because the representation
theorem forces log-ratio loss, and log converts multiplication
to addition."
-/

namespace Survival.AdditiveRecoveryNecessity
...
/-- **Additive recovery is forced by log composition.**

The net action (loss - gain) equals the log-ratio of endpoint
masses. This is forced by the logarithm's additive property:
log(a/b) + log(b/c) = log(a/c).

No alternative composition rule is consistent with the
representation theorem's log-ratio form. -/
...
end Survival.AdditiveRecoveryNecessity
