...
| Anchor | Maintained quantity | \(d_t\) | \(r_t\) | What \(b_t>0\) means | What \(b_t<0\) means | Transform / reversal used? | Claim strength |
|---|---|---|---|---|---|---|---|
...
| Repair-maintenance balance | accumulated damage \(D_t\) or remaining margin \(M_t\) | damage amount \(d_t\) | repair / maintenance amount \(r_t\) | net damage accumulation, threshold approach | repair-dominant recovery, larger remaining margin | equivalent switch between damage and margin views, no sign reversal | G4 v2 algebraic anchor |
...
|---|---|
| Route A loss-only anchors | \(b_t<0\) is usually not exercised, because \(r_t=0\) in the primary reading |
| Observational anchors | \(d_t\) and \(r_t\) are often proxies rather than directly measured flows |
