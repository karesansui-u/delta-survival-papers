...
defect-controlled compatibility, Bernoulli / Foster-Lyapunov /
Repair-Maintenance \(\Sigma\) Lean wrappers, the Phase-7 v0 cross-class
registry, the Phase-7 v1 unifying-schema extraction, and the Phase-7 v2
registered-interface closure.
...
- The admissible-maps / hierarchical-invariants supplement now makes the
  ...
  `analysis/phase6_foster_lyapunov_template.md`, with a thin Lean wrapper in
  `Survival.FosterLyapunovTemplate`; together they map the Bernoulli-CSP
  \(\Sigma\) template to Foster-Lyapunov / queueing anchors and now include
  resource-bounded stopped-collapse / hitting-time high-probability wrappers
  plus conditional coarse transfer, without claiming recurrence, ergodicity,
  pathwise monotonicity, or an unconditional Lyapunov second law. Phase 6.2 is
  now staged in `analysis/phase6_repair_maintenance_template.md`, with a thin
  Lean wrapper in `Survival.RepairMaintenanceTemplate`; it maps explicit damage
  / repair finite-prefix algebra, \(\Sigma\) repair-cost grammar,
  resource-bounded certificates, and conditional coarse transfer into the same
  limited-class template without claiming stochastic reliability, optimal
  maintenance policy, or an unconditional repair law. Phase 7 v0 now has a
  narrow cross-class registry in `analysis/phase7_cross_class_unification_v0.md`
  and `Survival.CrossClassUnificationV0`: Bernoulli-CSP, Foster-Lyapunov /
  queueing, and Repair-Maintenance all register the shared profile of
  \(\Sigma\) grammar, expectation-level tendency, finite-horizon
  high-probability certificate route, and conditional coarse transfer. This is
  a registry and common-profile closure, not a generic universal theorem. Phase
  7 v1 now extracts the first schema in
  `analysis/phase7_unifying_schema_v1.md` and
  `Survival.CrossClassUnificationV1`: an ordered Sigma carrier, a
  nonnegative tendency driver, a finite-horizon certificate route, and an
  admissible-transfer guard. The v1 schema deliberately does not make
  Bernoulli-style pathwise nondecrease or subadditivity the generic requirement.
  Phase 7 v2 now turns this into an explicit interface in
  `analysis/phase7_unifying_schema_v2.md` and
  `Survival.CrossClassUnificationV2`, with registered instances for all three
  current limited classes.
...
| Formal tendency / rival-framework stress test | M1 completed; admissible-map supplement added; Phase 2 wrappers added; saturation-defect, exact compatibility, defect-controlled compatibility, Bernoulli \(\Sigma\) wrappers, Bernoulli admissible-map v0 wrapper, Phase-5 ladder note added, Phase-6.1 Foster-Lyapunov / queueing template plus v1 high-probability Lean wrappers staged, Phase-6.2 Repair-Maintenance v0 wrappers staged, Phase-7 v0 cross-class registry added, Phase-7 v1 unifying schema extracted, and Phase-7 v2 registered-interface closure added; G6-c iteration 1 closed; falsification stress-test layer expanded | Expectation-level target theorem 4 formally accessible via existing theorems; reader-facing wrappers now exist in `Survival.SecondLawTotalProduction`, `Survival.AdmissibleMapInvariants`, `Survival.SaturationDefect`, `Survival.AdmissibleMapCompatibility`, `Survival.DefectControlledAdmissibleMap`, `Survival.BernoulliTypicalSigma`, `Survival.BernoulliAdmissibleMapV0`, `Survival.FosterLyapunovTemplate`, `Survival.RepairMaintenanceTemplate`, `Survival.CrossClassUnificationV0`, `Survival.CrossClassUnificationV1`, and `Survival.CrossClassUnificationV2`. The defect-controlled wrapper closes the readout-level cancellation of contracted-intermediate defects in signed action. The Bernoulli \(\Sigma\) wrapper gives the one-sided iid bad-event \(\Sigma\) observable, adjacent-step nondecrease, expected monotonicity, finite-path interior Chernoff lower-tail certificate, the corresponding good-event lower-bound and fixed-time typical-growth certificates, endpoint-defect coarse-transfer certificates, and collapse/stopped-collapse aliases without claiming an unconditional second law. `Survival.BernoulliAdmissibleMapV0` packages endpoint identity, endpoint defect budget, and coarse monotonicity as sufficient conditions for those coarse certificates; it is not a full admissible-map characterization. `analysis/phase5_admissible_map_ladder.md` records the ladder from exact readout through v0 sufficient readout to set-level and necessary/sufficient open targets, so Phase 5 can be used as a Phase-6 template without overclaiming full closure. `analysis/phase6_foster_lyapunov_template.md` and `Survival.FosterLyapunovTemplate` stage Foster-Lyapunov / queueing as the second limited class template by mapping existing `LyapunovBalanceEmbedding`, `QueueStability`, `ResourceBoundedConditionalAzuma`, `ResourceBoundedStochasticCollapse`, `SecondLawTotalProduction`, and `CoarseTypicalNondecrease` anchors into the same \(\Sigma\) / drift / concentration / coarse-transfer grammar. The v1 layer adds resource-bounded stopped-collapse / hitting-time high-probability wrappers and coarse high-probability transfer under explicit stochastic compatibility, while still not claiming positive recurrence, geometric ergodicity, Bernoulli-style pathwise nondecrease, or an unconditional Lyapunov second law. `analysis/phase6_repair_maintenance_template.md` and `Survival.RepairMaintenanceTemplate` stage Repair-Maintenance as the third limited class template by exposing damage / repair net-consumption algebra, margin / threshold crossing, \(\Sigma\) / repair-cost grammar, resource-bounded certificates, and conditional coarse transfer, while still not claiming stochastic reliability, optimal maintenance policy, Bernoulli-style pathwise nondecrease, or an unconditional repair law. `analysis/phase7_cross_class_unification_v0.md` and `Survival.CrossClassUnificationV0` register the shared three-class Phase-7 v0 profile. `analysis/phase7_unifying_schema_v1.md` and `Survival.CrossClassUnificationV1` extract the first generic schema: ordered \(\Sigma\) carrier, nonnegative tendency driver, finite-horizon certificate route, and admissible-transfer guard. `analysis/phase7_unifying_schema_v2.md` and `Survival.CrossClassUnificationV2` turn that schema into an explicit interface and register Bernoulli-CSP, Foster-Lyapunov / queueing, and Repair-Maintenance as instances. The v1/v2 schema keeps pathwise nondecrease and subadditivity out of the generic requirement. `analysis/second_law_level_roadmap.md` now separates the route into admissible maps, \(\Sigma\) / total production, typical nondecrease, limited-class universality, and eventual cross-class unification. `analysis/falsification_and_rival_frameworks.md` records the main ways the program could still be weakened, especially rival-framework subsumption by LDP / free-energy / contraction-style frameworks. Concrete defenses now include `analysis/ldp_rate_function_comparison.md`, `analysis/cross_domain_sign_convention_table.md`, and `analysis/scope_silence_catalog.md` | Next decisive choice: Phase 7 v3 semantic evidence upgrade or Phase 5 open admissible-map ladder |
