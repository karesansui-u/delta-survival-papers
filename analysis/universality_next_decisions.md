...
| G4 / non-CSP | G4 v1 closed; G4 v2 direction selected | Queueing / Foster-Lyapunov is the primary G4 v1 anchor; serial reliability and constant-fraction decay are loss-only controls. G4 v2 is scoped toward repair / maintenance reliability-fatigue balance, where \(g_t\) is explicit in a non-CSP open-system model | Optional Lean skeleton `RepairMaintenanceBalance.lean` or reader-facing supplement after theorem map stabilizes |
