...
| G4 / non-CSP | G4 v1 closed; G4 v2 minimal skeleton implemented | Queueing / Foster-Lyapunov is the primary G4 v1 anchor; serial reliability and constant-fraction decay are loss-only controls. G4 v2 is scoped toward repair / maintenance reliability-fatigue balance, where \(g_t\) is explicit in a non-CSP open-system model. `RepairMaintenanceBalance.lean` now formalizes the finite-prefix algebraic skeleton | Optional reader-facing supplement after theorem map stabilizes |
