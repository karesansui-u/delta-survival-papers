...
| External reception | Open | OSF is available; Mixed-CSP and Exp43c now both have local fresh reruns plus published-remote outside-workspace reruns, but no true outside-group rerun yet. Mixed-CSP also now has a sender-side runbook at `analysis/route_a_mixed_csp/mixed_csp_true_outside_send_runbook.md` for exact-hash capture and outbound packaging. | Independent review / replication |
