...
q-coloring primary validation, and the first two returned Mixed-CSP true
outside-group reruns.
...
Independent replication is now underway with two clean returned Mixed-CSP
outside-group reruns; further heterogeneous-domain extensions remain open.
```
...
| External reception | Underway | OSF is available; Mixed-CSP and Exp43c now both have local fresh reruns plus published-remote outside-workspace reruns. Mixed-CSP now has two returned true outside-group rerun successes (`analysis/route_a_mixed_csp/mixed_csp_true_outside_rerun_01_katsumasa1234.md`, `analysis/route_a_mixed_csp/mixed_csp_true_outside_rerun_02_SCRAPRO.md`) and an interim report (`analysis/route_a_mixed_csp/mixed_csp_true_outside_interim_report.md`) with `2/3` completed, `2/2` clean success, `1` pending. Mixed-CSP also has a sender-side runbook at `analysis/route_a_mixed_csp/mixed_csp_true_outside_send_runbook.md`. Exp43c now has a locked sender-side zip packet and return template via `analysis/exp43_qcoloring/handoff_exports/LOCKED_BUNDLE_NOTE.md`, `analysis/exp43_qcoloring/exp43c_true_outside_send_runbook.md`, and `analysis/exp43_qcoloring/exp43c_g7_replication_report_template.md`. | Remaining Mixed-CSP outside return, Exp43c outside-group return, independent review |
...
17. Exp43c now also has sender-side true outside-group materials at
...
18. Treat q-coloring and Cardinality-SAT as optional Route A width extensions,
   not as required gates.
19. For G4 v2, use `analysis/g4_v2_exploratory_dataset_scan.md` as the immediate
...
20. For repair-flow escalation, separate the next move into two layers:
