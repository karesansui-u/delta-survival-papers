...
12. Treat q-coloring and Cardinality-SAT as optional Route A width extensions,
   not as required gates.
13. For G4 v2, use `analysis/g4_v2_exploratory_dataset_scan.md` as the immediate
...
14. For the next cross-domain non-CSP empirical move, treat
    ...
    the first exact-subset choice note, with
    `analysis/g4_cmapss_fd001_archive_feasibility_note.md` now recording one
    exact public archive acquisition,
    `analysis/g4_cmapss_fd001_loss_only_preregistration_draft.md` as the
    first freeze-eligible draft path, and
    `analysis/g4_cmapss_fd001_loss_only/freeze_manifest_draft.md` as the
    current hardening surface, plus
    `analysis/g4_cmapss_fd001_loss_only/train_smoke_note.md` as the completed
    no-peek parser / train-fit check; keep all six explicitly below repair-flow
    evidence.
