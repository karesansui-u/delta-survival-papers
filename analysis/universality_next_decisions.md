...
q-coloring primary validation, and the first returned Mixed-CSP true
outside-group rerun.
...
Independent replication is now underway with one clean returned Mixed-CSP
outside-group rerun; further heterogeneous-domain extensions remain open.
```
...
| External reception | Underway | OSF is available; Mixed-CSP and Exp43c now both have local fresh reruns plus published-remote outside-workspace reruns. Mixed-CSP now has one returned true outside-group rerun success (`analysis/route_a_mixed_csp/mixed_csp_true_outside_rerun_01_katsumasa1234.md`) and an interim report (`analysis/route_a_mixed_csp/mixed_csp_true_outside_interim_report.md`) with `1/3` completed, `1/1` clean success, `2` pending. Mixed-CSP also has a sender-side runbook at `analysis/route_a_mixed_csp/mixed_csp_true_outside_send_runbook.md`. Exp43c now has a sender-side zip packet and return template via `analysis/exp43_qcoloring/exp43c_true_outside_send_runbook.md` and `analysis/exp43_qcoloring/exp43c_g7_replication_report_template.md`. | Remaining Mixed-CSP outside returns, Exp43c outside-group return, independent review |
