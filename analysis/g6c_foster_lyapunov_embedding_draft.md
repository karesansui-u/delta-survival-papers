...
本 draft の目的は、構造持続の収支原理が既存理論と単に「似ている」のではなく、少なくとも一つの古典的 drift calculus を構造持続の収支原理の変数へ形式的に埋め込めることを示すことである。
...
| G6-c formal embedding | 既存の drift / balance 条件が構造持続の収支原理の変数へ埋め込める | 本 draft の中心 |
...
\begin{quote}
Foster-Lyapunov drift 条件は、構造負荷 \(Z_t=W(X_t)\) を用いることで、構造持続の収支原理の expectation-level tendency の特例として埋め込める。
\end{quote}
...
本 draft の最小 formal embedding は完了している。

完了:

1. prose draft として G6-c の scope, embedding, non-claim を固定した。
...
4. reader-facing 補論として `v2/補論_構造持続の収支原理とFoster-Lyapunovドリフトの形式的埋め込み.md` を作成した。

したがって、G6-c iteration 1 は minimal algebraic embedding として閉じた状態にある。

本 iteration で扱わず、次の G6-c iteration に回す項目:

1. Foster-Lyapunov drift 条件から positive recurrence を導く theorem の Lean 化。
2. \(R_n \le R_0 e^{-cn}\) 型の geometric ergodicity / exponential decay bound。
3. continuous-time generator / stochastic thermodynamics 方向への拡張。
4. non-Markov Lyapunov functional への拡張。
