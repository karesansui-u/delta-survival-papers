# M Flow Network Testbed Freeze Manifest Draft v1
...
- add the calibration-selected candidate region from calibration review v1;
- keep the final primary setting unfrozen until an explicit freeze decision;
- record that held-out-allocation and full-grid calibration scores do not pick
  the same single top candidate.
...
- `scripts/report_flow_degeneracy.py`;
...
- `dry_runs/v0_smoke/degeneracy_group_summary.csv`;
- `dry_runs/v0_smoke/degeneracy_summary.json`;
- `dry_runs/calibration_sweep_v0/sweep_summary.csv`;
- `dry_runs/calibration_sweep_v0/sweep_diagnostics.json`;
- `dry_runs/calibration_review_v1/sweep_summary.csv`;
- `dry_runs/calibration_review_v1/sweep_diagnostics.json`;
- `dry_runs/calibration_review_v1/full_grid_smoke/sweep_summary.csv`;
- `dry_runs/calibration_review_v1/full_grid_smoke/sweep_diagnostics.json`.
...
## 5. Calibration-Selected Candidate Region

Calibration review v1 narrows the default calibration sweep to the neighborhood
around \(Q=4\). The review does not freeze a single primary setting. It defines
the candidate region from which a later freeze decision must choose.
...
- held-out-allocation review top candidate:
  \(Q=4\), damage intensity \(0.28\), horizon \(T=9\);
- full-grid smoke top candidate:
  \(Q=4\), damage intensity \(0.34\), horizon \(T=8\);
- the top candidates differ, so the manifest must not freeze a single setting
  solely from the calibration-score winner.

Calibration-selected candidate region:

- required flow: \(Q=4\);
- damage intensity: \(0.28 \le d \le 0.34\), implemented as the grid
  \(d \in \{0.28,0.30,0.32,0.34\}\) unless the final freeze explicitly records
  a different grid refinement inside the same interval;
- horizon: \(T \in \{8,9,10\}\) for the first primary candidate region;
...
1. Choose the final primary setting from the candidate region above.
2. Do not choose the setting by primary outcome, M-profile evaluator
