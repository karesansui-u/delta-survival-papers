#!/usr/bin/env python3
"""Optional PySAT solver wrapper for Exp44 Cardinality-SAT."""
...
try:  # pragma: no cover
    from .cnf_encoder import decode_model_to_assignment, encode_cardinality_sat, verify_assignment
    from .generator import CardinalityConstraint, CardinalitySATInstance
...
def solve_formula(
    *,
    ...
    formula = encode_cardinality_sat(n=n, constraints=constraints_tuple)
    ...
    if sat:
        ...
        assignment = decode_model_to_assignment(model, n=n)
        verified = assignment is not None and verify_assignment(
            n=n,
        ...
        if not verified:
            ...
            error = "SAT model failed independent cardinality verification"

    return SolveResult(
        sat_feasible=sat if status in {"SAT", "UNSAT"} else None,
        timeout=False,
        runtime_sec=runtime,
...
def solve_instance(
    instance: CardinalitySATInstance,
    *,
    timeout_sec: float = 120.0,
    backend: str = "minisat22",
) -> SolveResult:
