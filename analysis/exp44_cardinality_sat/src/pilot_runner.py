#!/usr/bin/env python3
"""Append-safe pilot runner for Exp44 Cardinality-SAT."""
...
def iter_mixture_rho_pairs(config: dict[str, Any]) -> Iterable[tuple[str, float]]:
    if "per_mixture_config" in config:
        for mixture_id, mix_config in sorted(config["per_mixture_config"].items()):
            for rho_fm in mix_config["rho_fm_values"]:
                yield str(mixture_id), float(rho_fm)
        return

    for mixture_id in config["mixture_ids"]:
        for rho_fm in config["rho_fm_values"]:
            yield str(mixture_id), float(rho_fm)


def iter_plan(config: dict[str, Any]) -> Iterable[tuple[str, int, float, int]]:
    for mixture_id, rho_fm in iter_mixture_rho_pairs(config):
        for n in config["n_values"]:
            for instance_idx in range(config["instances_per_cell"]):
                yield mixture_id, int(n), float(rho_fm), int(instance_idx)
...
def print_plan(config: dict[str, Any], output: Path) -> None:
    ...
    for mixture_id, n, rho_fm, _idx in rows:
        key = (mixture_id, n, rho_fm)
        cells[key] = cells.get(key, 0) + 1
    print("=" * 72)
    print("EXP44 CARDINALITY-SAT PILOT")
    print("=" * 72)
    ...
    for key in sorted(cells)[:12]:
        print(f"  mixture={key[0]} n={key[1]} rho_fm={key[2]:.2f} count={cells[key]}")
    if len(cells) > 12:
...
def dry_run(config: dict[str, Any], output: Path) -> None:
    ...
    print("\nExample instances:")
    for idx, (mixture_id, n, rho_fm, instance_idx) in enumerate(iter_plan(config)):
        if idx >= 5:
            break
        ...
        print(
            f"  {rec['instance_id']}: m={rec['m_semantic']} "
            f"counts=({rec['m_AL1_4']},{rec['m_EX2_4']},{rec['m_EX1_4']}) "
...
def run(config: dict[str, Any], output: Path, *, execute: bool, no_resume: bool) -> None:
    ...
    for mixture_id, n, rho_fm, instance_idx in iter_plan(config):
        instance = generate_instance(
            phase=config["phase"],
            mixture_id=mixture_id,
            n=n,
        ...
        print(
            ...
            f"t={result.runtime_sec:.4f}s m={instance.m_semantic} cnf={instance.cnf_clause_count}"
        )
