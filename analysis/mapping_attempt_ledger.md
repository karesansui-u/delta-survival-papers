...
| M-flow network final candidate v0 | Controlled synthetic flow-network M-side testbed | `frozen_test` | Primary package frozen but not executed: `Q=4`, `damage_intensity=0.34`, `T=8`, seed block `2000..2029`, full graph/damage/allocation grid, held-out graph `random_geometric`, held-out damage `repairable_wear`, held-out allocation `(3,4,3)` | No outcome yet | M-side intervention-ranking can be tested in a controlled functional structure before returning to noisy real domains | Execute one-time guarded primary or keep as frozen package; no support claim before outcome |
