...
2. zip の場合は同梱の `requirements.txt`、git の場合は
   `analysis/route_a_mixed_csp/requirements_mixed_csp.txt` から依存関係を入れる
...
1. Python 実行環境がある
2. zip 同梱の `requirements.txt`、または
   `analysis/route_a_mixed_csp/requirements_mixed_csp.txt` から依存関係を入れられる
...
詰まった場合は、変更せず止めて、その時点のエラーメッセージを返してもらえ
れば十分です。
...
2. zip の場合は同梱の `requirements.txt`、git の場合は
   `analysis/route_a_mixed_csp/requirements_mixed_csp.txt` に基づいて依存関係を導入
...
3. zip の場合は `pip install -r requirements.txt`
   git の場合は
