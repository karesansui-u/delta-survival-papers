...
| Epistemic-control protocol chain | `EpistemicControlBridge*`, `EpistemicBenchmarkProtocol.lean`, `EpistemicBenchmarkResultCertificate.lean`, `SoftwareEvidenceNetActionBridge.lean`, `analysis/epistemic_control_frozen_toy_v0/run_eval.py`, `v3/05_evidence/llm_epistemic_control_frozen_toy_v0/llm_epistemic_control_frozen_toy_v0_result_001.json`, `v3/05_evidence/llm_epistemic_control_real_eval_candidate_mapping.md`, `v3/05_evidence/llm_epistemic_premise_update_v0/`, `analysis/epistemic_control_premise_update_v0/run_eval.py` | Assumption-to-guarantee chain plus deterministic toy result artifact, real-eval planning map, first frozen premise-update task surface, and premise-update scorer / schema | If explicit finite witnesses are supplied, the protocol and result-certificate layers invoke the no-worse-net-action / coherent-mass comparison theorem; existing logs can guide future frozen protocols; the premise-update task surface is frozen before outcome-bearing execution and the scorer can emit a certificate-shaped result from external outputs | LLM semantics, real model performance, benchmark validity, memory safety, workflow correctness, result support before execution, or retrospective support from old logs |
