...
1 battery M-profile no-support, and completed three-run true outside-group
rerun sets for both Mixed-CSP and Exp43c q-coloring.
...
| DeltaLint software operational benchmark | `analysis/deltalint_software_operational_benchmark_note.md` + `/Users/sunagawa/Project/delta-lint-bench` | Initial internal L-side / Route C software operational support | Defines software structure as a distributed contract set. The 2026-04-29 Product-arm calibration records five frozen OSS items: Formbricks +0, Flask +1, httpx +2, Chi +4, Traefik +2; four of five items show positive Product-additive gain, with nine credited Product-additive `valid_structural` roots under the same-scope/additive rule | Not direct software-collapse evidence, not M-supplement validation, not maintainer acceptance, and not outside replication; static/frontend baselines remain partial for some items |
...
| M-flow network testbed | `analysis/m_flow_network_testbed/` | Controlled mechanistic M-side frozen package; primary not executed | Freezes a synthetic flow-network intervention-ranking test where buffer / recovery / reconfiguration allocation must beat total-resource and policy-prior baselines on held-out graph, damage, and allocation axes. The final candidate is `Q=4`, `damage_intensity=0.34`, `T=8`, with three required sensitivity settings and a guarded `final_candidate_v0` primary package | Not real-domain support, not executed primary evidence, and not a universal M-law. It is a controlled mechanistic validation candidate intended to test whether M-component allocation has incremental ranking value beyond total resource |
...
(Oxford Part 1), while the M-flow network testbed has a guarded controlled
mechanistic primary package but no primary outcome yet. Repair-flow empirical
