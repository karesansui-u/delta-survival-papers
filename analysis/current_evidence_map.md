...
| G7 independent replication | Open, but Mixed-CSP package now has Level 1 audit replay, Level 2 local fresh rerun, and an external handoff note; Exp43c now has a completed local fresh rerun | Public replication package starts with Mixed-CSP via `analysis/g7_mixed_csp_replication_package_plan.md`; `analysis/route_a_mixed_csp/mixed_csp_audit_replay_note.md` records Level 1 artifact replay, `analysis/route_a_mixed_csp/mixed_csp_level2_rerun_note.md` records the separate-output fresh rerun, and `analysis/route_a_mixed_csp/mixed_csp_external_rerun_package.md` records the external handoff boundary. Exp43c now has a matching local rerun at `analysis/exp43_qcoloring/exp43c_level2_rerun_note.md`. The next clean move is an outside rerun before threshold-local or observational branches |
