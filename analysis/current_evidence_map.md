...
completed three-run Mixed-CSP true outside-group rerun set, plus the first
returned Exp43c true outside-group rerun success.
...
| Structural Persistence Balance | `v2/3_構造持続の収支原理.md` | Central theory layer | \(b_t=d_t-r_t\), \(B_n=\sum b_t\), collapse / maintenance / recovery regimes | Universal-law declaration |
...
| Exp43c q-coloring | Primary validated; first true outside-group rerun success | `fm_plus_n` log loss 0.440189 < best primary raw baseline 2.804019; H1 direction passed for q=3/4/5; the first returned outside-group rerun by `philia_channel` completed `4000` rows with `0` checked core mismatches, `TIMEOUT = 0`, `MALFORMED = 0`, and the same qualitative support decision | SAT-looking syntax is not the only Route A surface; first-moment coordinate transfers across q; the frozen q-coloring package is now confirmed executable outside the project environment in one returned run | Not a q-coloring threshold theorem; one Exp43c outside return is not a completed multi-run requested set unless more returns are requested and received |
