...
Status: after q-coloring (Exp43c package) primary validation, G4 v2 repair-maintenance
...
1 battery optional M-component no-support, the controlled M-flow network primary
no-support result, and completed three-run true outside-group rerun sets for
both Mixed-CSP and q-coloring (Exp43c package).
...
| M operationalization | `v2/補論_構造持続における資源項Mの操作的定式化.md` | Operational mapping layer | How to read scalar \(M\) as effective maintenance surplus and, optionally, decompose it for diagnostics | Universal resource metric or core component law |
| Observation/estimation indicator ecosystem protocol | `analysis/proxy_ecosystem_protocol.md` | Future empirical-governance design | How candidate indicators are discovered, frozen, upgraded, superseded, and recorded under public or controlled-access validation | Not evidence; does not upgrade existing mappings |
...
| Epistemic-control protocol chain | `EpistemicControlBridge*`, `EpistemicBenchmarkProtocol.lean`, `EpistemicBenchmarkResultCertificate.lean`, `SoftwareEvidenceNetActionBridge.lean`, `analysis/epistemic_control_frozen_toy_v0/run_eval.py`, `v3/05_evidence/llm_epistemic_control_frozen_toy_v0/llm_epistemic_control_frozen_toy_v0_result_001.json`, `v3/05_evidence/llm_epistemic_control_real_eval_candidate_mapping.md`, `v3/05_evidence/llm_epistemic_premise_update_v0/`, `analysis/epistemic_control_premise_update_v0/run_eval.py` | Assumption-to-guarantee chain plus deterministic toy result artifact, real-eval planning map, first frozen premise-update task surface, output-collection protocol, run manifest, and premise-update scorer / schema | If explicit finite witnesses are supplied, the protocol and result-certificate layers invoke the no-worse-net-action / coherent-mass comparison theorem; existing logs can guide future frozen protocols; the premise-update task surface, output-collection protocol, and result_001 run condition are frozen before outcome-bearing execution and the scorer can emit a certificate-shaped result from external outputs | LLM semantics, real model performance, benchmark validity, memory safety, workflow correctness, result support before execution, or retrospective support from old logs |
...
| q-coloring (Exp43c package) | Primary validated; three true outside-group rerun successes | `fm_plus_n` log loss 0.440189 < best primary raw baseline 2.804019; H1 direction passed for q=3/4/5; three returned outside-group reruns by `philia_channel`, `katsumasa1234`, and `SCRAPRO` each completed `4000` rows with `0` checked core mismatches, `TIMEOUT = 0`, `MALFORMED = 0`, and the same qualitative support decision; final package report at `analysis/exp43_qcoloring/exp43c_true_outside_final_report.md` | SAT-looking syntax is not the only Route A surface; first-moment coordinate transfers across q; the frozen q-coloring package is now confirmed executable outside the project environment in three returned runs | Not a q-coloring threshold theorem; three Exp43c outside returns strengthen package-level G7 but do not close non-CSP or observational replication |
...
| Battery optional M-component branch | `analysis/g4_battery_m_profile/` | Public battery primary no-support outcome + weak-axis audit | Oxford Path Dependent Part 1 progressed through exact identity, no-metric structure counts, MATLAB training conversion (`168` records), converted-smoke, header-only schema draft, `transition_aggregate_v1` training-feature smoke (`149` transition rows), and then a one-time held-out primary (`74` held-out rows). The frozen M/SP primary did not improve over `B3`: `primary RMSE = 0.23508673118782375` versus `B3 RMSE = 0.2296038662551124`; `H1 = false`, `H2 = false`, `H3 = true`, `primary_support = false` | No-support for this frozen Oxford battery M/SP mapping; additionally warns that the added M/SP axis was too close to ordinary battery state indicators. Not causal intervention-ranking evidence, not repair-flow evidence, not universal-law support, and not a same-archive rescue target |
| M-flow network testbed | `analysis/m_flow_network_testbed/` | Controlled mechanistic M-side primary no-support outcome | The guarded `final_candidate_v0` primary was executed once. In the primary setting, optional M-component regret improved over total-resource regret (`0.2204` vs `0.2372`) but did not beat the policy-prior baseline (`0.1506`). Sensitivity settings showed the same qualitative pattern. Frozen-rule result: no optional M-component diagnostic support, with a secondary signal relative to total resource only | Not real-domain support and not a universal M-law. The result shows that simple M allocation carries some information beyond scalar budget in this testbed, but not enough to beat a strong policy-prior baseline |
...
optional M-component no-support result (Oxford Part 1), and one controlled-mechanistic
...
disappearing into scattered notes. The observation/estimation indicator ecosystem protocol records the
future-facing rule that early indicators may be superseded by stronger
