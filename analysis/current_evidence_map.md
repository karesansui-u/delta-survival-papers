...
| G4 non-CSP | G4 v1/v2 minimal skeletons closed; repair-flow public dataset search paused; Backblaze loss-only primary completed with no support under frozen log-loss rule | Next clean move: do not rescue Backblaze post-hoc; use `analysis/backblaze_loss_only_v2_exploration_note.md` as the design boundary and `analysis/backblaze_loss_only_v2_archive_ranking_note.md` as the metadata-only candidate ranking if opening a new calibration / survival preregistration on a fresh archive |
