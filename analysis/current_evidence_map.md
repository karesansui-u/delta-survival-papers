...
| G4 non-CSP | G4 v1/v2 minimal skeletons closed; repair-flow public dataset search paused; Backblaze loss-only branch now contains one closed no-support attempt and one same-domain support pass; C-MAPSS FD001 exact archive feasibility, a first loss-only prereg draft, and a freeze-manifest draft are now recorded | Keep both Backblaze outcomes visible without rescue language. The main open gap remains repair-flow / cross-domain non-CSP support. The current public-work path is now split explicitly into `analysis/g4_cmapss_loss_only_feasibility_note.md` for the candidate logic, `analysis/g4_cmapss_fd001_archive_note.md` for the subset choice, `analysis/g4_cmapss_fd001_archive_feasibility_note.md` for one exact public archive acquisition, `analysis/g4_cmapss_fd001_loss_only_preregistration_draft.md` for the first freeze-eligible draft path, `analysis/g4_cmapss_fd001_loss_only/freeze_manifest_draft.md` for the freeze surface, and `analysis/g4_v2_repair_flow_candidate_criteria.md` for future repair-flow gating |
