...
| G4 non-CSP | G4 v1/v2 minimal skeletons closed; repair-flow public dataset search paused; Backblaze loss-only primary completed with no support under frozen log-loss rule; Backblaze v2 is now frozen and awaiting one Q3 test-block run | Next clean move: do not rescue Backblaze post-hoc; use `analysis/backblaze_loss_only_v2_exploration_note.md` as the design boundary, `analysis/backblaze_loss_only_v2_archive_ranking_note.md` as the metadata-only ranking, `analysis/g4_backblaze_q3_2025_archive_feasibility_note.md` as the fresh-archive feasibility check, `analysis/backblaze_loss_only_v2_preregistration_draft.md` as the current Q3 2025 calibration-aware prereg draft, `analysis/backblaze_loss_only_v2/freeze_manifest_draft.md` as the frozen package, `analysis/backblaze_loss_only_v2/scripts/evaluate_backblaze_loss_only_v2.py` as the frozen evaluation script, and `analysis/backblaze_loss_only_v2/validation_smoke_note.md` as the completed pre-freeze integration check; then run the Q3 primary validation once |
