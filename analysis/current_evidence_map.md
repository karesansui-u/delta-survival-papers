...
returned Mixed-CSP true outside-group rerun.
...
|---|---|---|---|---|
| Mixed-CSP | Primary validated; outside-group rerun underway | `L_plus_n` log loss 0.0970 < `raw_plus_n` 0.7525; first returned outside-group rerun completed cleanly with `12000` rows, `0` checked core mismatches, and all support flags true | Drift-weighted coordinate beats raw count on mixed SAT/NAE feasibility; the frozen package is now confirmed executable outside the project environment in at least one returned run | Still within Bernoulli-CSP family; outside-group rerun set is interim (`1/3` completed, `2` pending) |
| Mixed-CSP true outside-group rerun interim | `analysis/route_a_mixed_csp/mixed_csp_true_outside_interim_report.md` | First returned outside-group rerun: `1/3` completed, `1/1` clean success, `2` pending | Records a returned WSL/Ubuntu run by `katsumasa1234` with `12000` primary rows, `0` checked core mismatches, and reproduced support flags | Interim G7 layer only; not final Mixed-CSP G7 report and not Exp43c replication |
...
| G7 independent replication | Open but now underway with one returned Mixed-CSP outside-group success. Mixed-CSP has Level 1 audit replay, Level 2 local fresh rerun, an external handoff note, a pre-published fresh-clone rehearsal, a published-remote outside-workspace rerun, a sender-side send runbook, a final true outside-group handoff checklist, and an interim outside-group report with `1/3` returned and `1/1` clean success. Exp43c now has a completed local fresh rerun, an external handoff note, a published-remote outside-workspace rerun, a final true outside-group handoff checklist, and a sender-side zip packet / return-template layer | Public replication package starts with Mixed-CSP via `analysis/g7_mixed_csp_replication_package_plan.md`; `analysis/route_a_mixed_csp/mixed_csp_true_outside_rerun_01_katsumasa1234.md` records the first returned outside-group success, and `analysis/route_a_mixed_csp/mixed_csp_true_outside_interim_report.md` records the interim state: `1/3 completed`, `1/1 clean success`, `2 pending`. Earlier layers remain recorded in `mixed_csp_audit_replay_note.md`, `mixed_csp_level2_rerun_note.md`, `mixed_csp_external_rerun_package.md`, `mixed_csp_outside_workspace_rerun_note.md`, `mixed_csp_published_remote_rerun_note.md`, `mixed_csp_true_outside_handoff_checklist.md`, and `mixed_csp_true_outside_send_runbook.md`. Exp43c now has a matching local rerun at `analysis/exp43_qcoloring/exp43c_level2_rerun_note.md`, an outside handoff boundary at `analysis/exp43_qcoloring/exp43c_external_rerun_package.md`, a published-remote outside-workspace rerun at `analysis/exp43_qcoloring/exp43c_published_remote_rerun_note.md`, a final handoff checklist at `analysis/exp43_qcoloring/exp43c_true_outside_handoff_checklist.md`, and send-ready materials at `analysis/exp43_qcoloring/exp43c_true_outside_send_runbook.md`, `analysis/exp43_qcoloring/exp43c_true_outside_send_packet_ja.md`, and `analysis/exp43_qcoloring/exp43c_g7_replication_report_template.md`. The next clean move is to send the Exp43c outside-group zip packet while absorbing the two pending Mixed-CSP returns when they arrive |
...
interim true outside-group rerun layer with one clean returned success and two
requested reruns pending. On the non-CSP side it now has one same-domain
