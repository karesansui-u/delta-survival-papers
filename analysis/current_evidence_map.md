...
| Epistemic-control protocol chain | `EpistemicControlBridge*`, `EpistemicBenchmarkProtocol.lean`, `EpistemicBenchmarkResultCertificate.lean`, `SoftwareEvidenceNetActionBridge.lean`, `analysis/epistemic_control_frozen_toy_v0/run_eval.py`, `v3/05_evidence/llm_epistemic_control_frozen_toy_v0/llm_epistemic_control_frozen_toy_v0_result_001.json`, `v3/05_evidence/llm_epistemic_control_real_eval_candidate_mapping.md` | Assumption-to-guarantee chain plus deterministic toy result artifact and real-eval planning map | If explicit finite witnesses are supplied, the protocol and result-certificate layers invoke the no-worse-net-action / coherent-mass comparison theorem; existing logs can guide future frozen protocols | LLM semantics, real model performance, benchmark validity, memory safety, workflow correctness, or retrospective support from old logs |
