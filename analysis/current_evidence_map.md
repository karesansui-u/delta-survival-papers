...
| G4 non-CSP | G4 v1/v2 minimal skeletons closed; repair-flow public dataset search paused; Backblaze loss-only branch now contains one closed no-support attempt and one same-domain support pass; C-MAPSS FD001 exact archive feasibility, a first loss-only prereg draft, a freeze-manifest draft, and a no-peek train-side smoke note are now recorded | Keep both Backblaze outcomes visible without rescue language. The main open gap remains repair-flow / cross-domain non-CSP support. The current public-work path is now split explicitly into `analysis/g4_cmapss_loss_only_feasibility_note.md` for the candidate logic, `analysis/g4_cmapss_fd001_archive_note.md` for the subset choice, `analysis/g4_cmapss_fd001_archive_feasibility_note.md` for one exact public archive acquisition, `analysis/g4_cmapss_fd001_loss_only_preregistration_draft.md` for the first freeze-eligible draft path, `analysis/g4_cmapss_fd001_loss_only/freeze_manifest_draft.md` for the freeze surface, `analysis/g4_cmapss_fd001_loss_only/train_smoke_note.md` for the no-peek parser / train-fit check, and `analysis/g4_v2_repair_flow_candidate_criteria.md` for future repair-flow gating |
...
| G7 independent replication | Open, but Mixed-CSP now has Level 1 audit replay, Level 2 local fresh rerun, an external handoff note, and a fresh-clone outside-workspace rehearsal; Exp43c now has a completed local fresh rerun and an external handoff note | Public replication package starts with Mixed-CSP via `analysis/g7_mixed_csp_replication_package_plan.md`; `analysis/route_a_mixed_csp/mixed_csp_audit_replay_note.md` records Level 1 artifact replay, `analysis/route_a_mixed_csp/mixed_csp_level2_rerun_note.md` records the separate-output fresh rerun, `analysis/route_a_mixed_csp/mixed_csp_external_rerun_package.md` records the external handoff boundary, and `analysis/route_a_mixed_csp/mixed_csp_outside_workspace_rerun_note.md` records the fresh-clone rehearsal at the same frozen commit. Exp43c now has a matching local rerun at `analysis/exp43_qcoloring/exp43c_level2_rerun_note.md` plus `analysis/exp43_qcoloring/exp43c_external_rerun_package.md` for the outside handoff boundary. The next clean move is a true outside rerun before threshold-local or observational branches |
