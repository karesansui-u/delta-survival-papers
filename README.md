...
| Component | Status |
|---|---|
| Paper 1 — Structural Collapse | **Under Review** |
...
- **Exp. 35 — Context rot is δ accumulation:**
  Under δ = 0, all 4 models (3 vendors) maintain 100% accuracy
  up to 96K tokens — zero degradation. Under δ > 0, accuracy
  collapses immediately at 500 tokens. "Context rot" is driven
  by contradiction accumulation, not context length.
...
差の74%を定量的に説明。
**実験35 — コンテキストロットはδの蓄積:**
δ = 0 下では4モデル（3ベンダー）が96Kトークンまで
正答率100%を維持 — 劣化ゼロ。δ > 0 では500トークンで
即座に崩壊。「コンテキストロット」の原因はコンテキスト長ではなく
矛盾の蓄積。
...
Exp. 35 (δ = 0 control, 4 models × 3 vendors × 500–96K tokens)
...
実験35（δ = 0対照群、4モデル×3ベンダー×500–96Kトークン）により、
...
delta-survival-paper/
  ...
  analysis/
    ...
    exp35/         δ=0 control — context rot experiments (4 models, 3 vendors)
  README.md
```
