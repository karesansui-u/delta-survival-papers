...
- 仕様固定構造層: SAT / Mixed-CSP / q-coloring では、自然測度・finite-horizon collapse chain・prospective empirical support が揃いつつあります。現時点で最も硬い経験的入口は、Mixed-CSP と Exp43c q-coloring の二つの frozen package です。どちらも外部実行者 3 名による rerun で decision-relevant output が再現され、3/3 clean outside rerun として記録されています。これは理論全体の証明ではありませんが、仕様固定構造層における law-side 座標の最初の強い足場です。
