...
本理論は、同一の構造持続核を、観測可能性の異なる三つの層で扱う。この分類は、ドメインの価値を順位づけるものではない。構造、測度、境界をどこまで事前固定できるか、あるいは代理指標と凍結検証でどこまで推定するかを分けるための運用語彙である。

| 外向け名 | 内部旧称 | 判定基準 | support の読み方 |
|---|---|---|---|
| 仕様固定構造ドメイン | Route A | 状態空間、測度、構造消耗、collapse boundary が仕様から事前固定される | theorem-side / finite-horizon / prospective empirical support を強く読む |
| 条件付き構造埋め込みドメイン | Route B | 既存理論のドリフト、差分、停止境界を \(b_t,B_n\) などへ条件付きに写せる | 限定主張として読む |
| 構造推定ドメイン | Route C | proxy、観測指標、操作的写像によって検査する | empirical anchor / operational support として読む |

構造推定ドメインは、LLM、software、組織、maintenance log のような現実ドメインで、観測指標と凍結写像により追加予測力を検査する層である。ただし構造推定ドメインの成功を、仕様固定構造ドメインの定理的成功と同じ強さで書いてはならない。
...
| 構造推定ドメインの成功が仕様固定構造ドメインの定理的成功を意味する | 証拠強度が異なる |
