...
これは universal-law closure ではない。しかし、仕様固定構造層において、\(L\) / first-moment 型の law-side 座標が raw baseline と比較され、その凍結済み package が著者環境外で同じ decision-relevant output を返したという意味で、local rerun や単なる手順公開より強い package-scoped replication support である。詳細は [`../v3/05_evidence/outside_reruns.tsv`](../v3/05_evidence/outside_reruns.tsv) と [`../analysis/g7_route_a_true_outside_replication_summary.md`](../analysis/g7_route_a_true_outside_replication_summary.md) に置く。
