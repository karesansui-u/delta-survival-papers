...
5.4 q-coloring as the next width extension

次の Route A width extension として最も自然なのは q-coloring である。理由は、SAT と見た目が大きく異なり、graph coloring、統計物理、組合せ最適化に接続するからである。
...
このため、Exp43 q-coloring cross-q feasibility では、primary endpoint を colorable / uncolorable とし、primary predictor を \(L\) と state-space size correction の組、または threshold-normalized \(L\) として設計するのが自然である。baseline は raw edge count、density、\(n\)、\(q\)、および必要に応じて encoding-size diagnostic を含める。primary split は leave-one-q-out または leave-one-threshold-band-out が望ましい。
...
5.5 Cardinality-SAT and stress extensions
...
\begin{quote}
SAT / Bernoulli-CSP / Mixed-CSP / q-coloring は、構造収支律が自然測度と bad-event exposure の上で強く閉じる Route A universality-class candidate を与える。
\end{quote}
...
第一に、Route A では、drift-weighted coordinate が raw count や encoding-size baseline を上回らない独立 family が見つかれば、Bernoulli-CSP universality-class claim は弱まる。特に q-coloring の cross-\(q\) test は、この目的に適した次の検査である。
...
1. q-coloring cross-\(q\) feasibility test。
   SAT / NAE mixed CSP の次に、graph-coloring family で drift-weighted coordinate が raw edge count / density baseline を上回るかを検査する。これは G3 Route A width と G5 prospective prediction を同時に進める。
...
4. Route A 補論群: SAT / Bernoulli-CSP / Mixed-CSP / q-coloring。
...
Route A では、SAT / Bernoulli-CSP / Mixed-CSP / q-coloring が、自然測度と bad-event exposure の上でこの構造を強く閉じる。Route C では、Paper 3 / Paper 4 の scope-as-repair、external metabolism、dependency-aware replay が、補償流の observable indicator として働く。ただし、Route C は high-probability theorem ではなく、observational prediction の層である。
