...
有限CSPにおける構造持続の予測力
— Mixed-SAT/NAE-SAT と Exp43c q-coloring による仕様固定構造ドメインの経験的検証 —

要旨

本補論は、構造持続理論の仕様固定構造ドメインに属する有限制約充足問題において、累積構造消耗 \(L\) が単なる制約数より強い予測情報を持つかを検証する。
...
これは重要である。以前の計算コスト補論では、条件つき発見確率 \(P(\mathrm{found}\mid\mathrm{SAT})\) と solver cost を扱った。しかし仕様固定構造ドメインの第一モーメント構造が直接支配するのは、まず解空間の体積と feasibility であって、特定ソルバーの探索コストではない。
...
ただし、exact-one は feasibility の飽和と encoding size 交絡が強いため、初回 primary から外した。この判断は、最初の仕様固定構造ドメインにおける empirical test を清潔に保つための保守的設計である。
...
ランダム 3-SAT は、構造持続理論にとって最も硬い仕様固定構造ドメイン anchor の一つである。状態集合は \(\{0,1\}^n\)、測度は counting measure、各制約の縮小率は仕様から直接計算できる。
...
4. Bootstrap percolation や branching process がこの意味で仕様固定構造ドメイン anchor である、とは主張しない。
...
Mixed-CSP primary は、構造持続理論の仕様固定構造ドメインにおける empirical test として、事前登録された四つの判定基準を全て通過した。
...
これは、構造持続理論における \(L\) が単なる量ではなく、制約の質を含む座標として機能することの経験的証拠である。SAT 単独の仕様固定構造ドメイン anchor は、Mixed-SAT/NAE-SAT により Bernoulli-CSP class へ一段拡張された。

さらに Exp43c q-coloring により、SAT 構文の外側に見える graph-coloring family でも、first-moment / drift coordinate が raw / density / encoding-size baseline を上回ることが示された。これは仕様固定構造ドメインの幅を一段広げる primary evidence である。

外部再実行の層では、Mixed-CSP が 3 名の外部実行者で 3/3 clean success を返し、Exp43c q-coloring も 3 名の外部実行者で 3/3 clean success を返した。したがって、仕様固定構造ドメインでは二つの frozen package について著者環境外での再実行成功が得られている。
