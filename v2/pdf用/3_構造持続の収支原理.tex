...
本稿の役割は、構造持続理論が普遍法則として確立したと宣言することではない。むしろ、構造持続の最小形式の回復量を明示しない最小核、構造持続の条件つき導出における層分離、集合値力学補論の符号付き指数核、Lean 形式化における期待値レベルの傾向、Mixed-CSP の有限時間 concentration、および Route C companion I / II の repair / external metabolism の観察を、単一の「構造持続の収支原理」として読み直すための主理論 spine を与えることである。論理依存としては、最小形式、条件つき導出、本稿の順に読むのが自然だが、読者導線としては統合版から本稿へ入り、必要に応じて最小形式や条件つき導出に降りる読み方を想定している。
\end{abstract}
...
\item
  構造持続の条件つき導出は、指数表現がどの条件で恒等式となり、どの条件で境界として安定化するかを与える。
\item
...
\tightlist
\item
  構造持続の最小形式: loss-only の最小形式。
\item
  構造持続の条件つき導出: 条件つき導出と弱依存境界。
\item
  本稿: 構造消耗量と回復量を含む構造持続の収支原理。
\item
...
したがって、本稿は普遍法則の最終宣言ではない。より正確には、構造持続理論を universal-law candidate として強化するための主理論層である。Exp43c q-coloring により、SAT 以外に見える Route A family への幅は一段広がった。また、Mixed-CSP package については、3 名の外部実行者による独立再実行が 3/3 clean success を返しており、Exp43c q-coloring package についても、1 名の外部実行者による独立再実行が clean success を返している。したがって Route A では、二つの frozen package で著者環境外でも qualitative support decision が再現されることを示している。今後の決定的な進展は、さらなる異質 family、追加の outside-group rerun、non-CSP formal embedding、および Route C の前向き検査によって与えられる。
\end{document}
