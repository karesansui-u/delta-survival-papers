...
\item
  Paper 0 統合版
\item
  Core Paper「構造持続の最小核と収支原理」 \# Paper 1 構造持続の最小形式
\end{enumerate}

\section{Paper 2 構造持続の収支原理}\label{paper-2-ux69cbux9020ux6301ux7d9aux306eux53ceux652fux539fux7406}
...
Software / DeltaLint 系は、この構造推定層の中でも、ソフトウェア崩壊そのものではなく、分散契約矛盾という早期シグナルを検査する operational benchmark として扱う。構造は、API / caller、config / runtime、documentation / implementation、lifecycle producer / consumer などにまたがる contract set である。主比較は provider 間競争ではなく、同一 model・同一 frozen context で generic review と structural-lens review を比較し、bounded validation 後の unique valid structural root causes が増えるかで判定する。2026年4月29日時点の内部 bench では、Product-arm additive ledger が5件中4件で正の増分、合計9件の Product-additive valid structural root を記録した。ただし、これは構造推定層の operational support であり、software collapse prediction や \(M\)-framework validation ではない。
