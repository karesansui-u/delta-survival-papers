...
v3 では、個別ドメインを本文へ直接増殖させない。ドメインは \texttt{03\_domains/registry.tsv} と個別 domain profile に登録し、support / no-support / outside rerun / field demonstration / bounded benchmark は \texttt{05\_evidence/} に記録する。これにより、ドメインが増えても主理論 spine と claim boundary が崩れないようにする。
...
これは universal-law closure ではない。しかし、仕様固定構造層において、\(L\) / first-moment 型の law-side 座標が raw baseline と比較され、その凍結済み package が著者環境外で同じ decision-relevant output を返したという意味で、local rerun や単なる手順公開より強い package-scoped replication support である。詳細は \href{../v3/05_evidence/outside_reruns.tsv}{\texttt{../v3/05\_evidence/outside\_reruns.tsv}} と \href{../analysis/g7_route_a_true_outside_replication_summary.md}{\texttt{../analysis/g7\_route\_a\_true\_outside\_replication\_summary.md}} に置く。
\end{abstract}
