...
本理論は、同一の構造持続核を、観測可能性と主張強度の異なる三つの層で扱う。これは対象ドメインの固定分類ではない。同じ LLM でも明示制約タスクなら仕様固定構造層に近づき、同じ物理・生物・ソフトウェア系でも proxy しか取れない場合は構造推定層に入る。これは「構造は常にある」という主張ではない。維持対象・測度・観測単位を固定できる場合に、その系を構造持続問題として扱う、という規律である。
...
条件付き構造埋め込み層 & Route B & 既存理論のドリフト、差分、停止境界を本理論の変数へ条件付きに写す & Foster-Lyapunov / queueing drift, bounded approximation candidates \\
構造推定層 & Route C & 構造そのものを直接数えるのではなく、代理指標と凍結検証によりその効果を推定する & LLM 推論劣化、継続学習、software / DeltaLint 系 \\
\end{longtable}
}
