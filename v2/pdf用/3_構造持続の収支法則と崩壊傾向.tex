...
構造を維持するとは、単に物質や資源が残っていることではない。ある機能、関係、同一性、または推論経路を保てる状態の余地が残っていることである。Paper 1 は、この余地を構造維持可能集合 \(V^{(t)}\) の測度 \(m(V^{(t)})\) として表し、制約追加による収縮モードでは累積損失 \(L_{n}\) に対して
...
本稿が主理論 spine に与える核は、損失流 \(\ell_{t}\) と補償流 \(g_{t}\) の差し引きが、構造維持可能領域の指数的変化を支配するという収支恒等式である。
...
\item
  観測された association を causal proof と呼ばない。
\item
  熱力学、情報理論、queueing theory、確率制御を置き換えるとは主張しない。
\end{enumerate}

これらは弱さの列挙ではなく、理論を空虚化しないための境界である。構造収支律は、どの層で何を仮定しているかを明示する限りで意味を持つ。

\subsection{反証可能性}\label{ux53cdux8a3cux53efux80fdux6027}
...
\subsection{次に重要な gate}\label{ux6b21ux306bux91cdux8981ux306a-gate}
...
\item
  independent replication。 Level 3 universal-law credibility には、内部再現だけでは足りない。外部研究者による再現、批判、失敗例の報告が必要である。
\end{enumerate}

\subsection{Paper 0 との関係}\label{paper-0-ux3068ux306eux95a2ux4fc2}
