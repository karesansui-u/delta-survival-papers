...
第三に、G4 v2 の empirical / operational pilot を設計する方向である。たとえば、maintenance log、incident count、repair schedule、remaining margin proxy を持つ実データで、\(d_{t}-r_{t}\) が failure / degradation を予測するかを検査できる。ただし、これは構造推定層 / operational validation であり、本補論の algebraic skeleton とは別段階である。
