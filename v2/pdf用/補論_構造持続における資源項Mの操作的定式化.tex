...
\[
S = N_{\mathrm{eff}}^{(0)} \times (\mu / \mu_{c}) \times e^{-L}
\]
...
\subsection{スカラー M の限界}\label{ux30b9ux30abux30e9ux30fc-m-ux306eux9650ux754c}
...
\subsection{本補論の問い}\label{ux672cux88dcux8ad6ux306eux554fux3044}
...
\subsection{立場と範囲}\label{ux7acbux5834ux3068ux7bc4ux56f2}
...
\item
  本補論では\textbf{介入順位予測}を、維持能力成分の分解の操作的帰結として一つの検査標的に置く。崩壊プロファイルや時間発展主張は後続の拡張とみなす。
\item
...
\subsection{Route C companion I / II との概観的接続}\label{route-c-companion-i-ii-ux3068ux306eux6982ux89b3ux7684ux63a5ux7d9a}
...
\section{最小形式}\label{ux6700ux5c0fux5f62ux5f0f}

\subsection{F / Sigma / R / M の 4 層}\label{f-sigma-r-m-ux306e-4-ux5c64}
...
\subsection{維持能力成分と供給 channel の定義}\label{ux7dadux6301ux80fdux529bux6210ux5206ux3068ux4f9bux7d66-channel-ux306eux5b9aux7fa9}
...
\[
\begin{aligned}
\widetilde M_{j} &= A_{j}(M_{j}^{\mathrm{int}}, M_{\mathrm{ext}\to j}), \\
...
\subsection{\texorpdfstring{\(M_{\mathrm{reconfiguration}}\) に関する重要な制限}{M\_\{\textbackslash mathrm\{reconfiguration\}\} に関する重要な制限}}\label{m_mathrmreconfiguration-ux306bux95a2ux3059ux308bux91cdux8981ux306aux5236ux9650}
...
\subsection{\texorpdfstring{Raw resource から維持能力成分への写像 \(\gamma_i\)}{Raw resource から維持能力成分への写像 \textbackslash gamma\_i}}\label{raw-resource-ux304bux3089ux7dadux6301ux80fdux529bux6210ux5206ux3078ux306eux5199ux50cf-gamma_i}
...
$$
\begin{aligned}
M_{j}^{\mathrm{int}} &= \gamma_{j}^{\mathrm{int}}(R, \Sigma, F), \\
\quad \\
...
\[
\begin{aligned}
R \text{ は大きいが、} \gamma_{j}^{\mathrm{int}}(R, \Sigma, F) \approx 0 \\
\quad \text{または} \quad \\
...
\subsection{\texorpdfstring{有効維持能力と集約関数 \(\Phi\)}{有効維持能力と集約関数 \textbackslash Phi}}\label{ux6709ux52b9ux7dadux6301ux80fdux529bux3068ux96c6ux7d04ux95a2ux6570-phi}
...
\begin{aligned}
\Phi_\rho(q) &= \\
\left(\sum_{i} \alpha_{i} q_{i}^\rho \right)^{1/\rho}
\end{aligned}
\]
...
\subsection{Lean で閉じている範囲}\label{lean-ux3067ux9589ux3058ux3066ux3044ux308bux7bc4ux56f2}

本補論の維持能力成分の分解は、\texttt{Survival/MaintenanceComponentDecomposition.lean} で形式化されている。Lean 側で閉じているのは、経験的な proxy の妥当性ではなく、M 側の表現文法と、M interface 上の表現定理である。
...
この主張はさらに、標準商としても定式化される。Lean 側では、raw mechanism を「同じ三成分 profile を持つなら同一視する」同値関係で割った \texttt{ObservationQuotient} を作り、任意の M-side readout がこの標準商を一意に通ることを証明している。したがって三成分 profile は単なる便利な座標ではなく、M interface 上で識別可能な差分を失わない最小完全観測商として機能する。
...
\bottomrule\noalign{}
\endlastfoot
\texttt{MaintenanceComponent.exhaustive} & 維持能力成分は buffer / recovery / reconfiguration の三つで尽きる \\
\texttt{SupplyChannel.exhaustive} & 供給 channel は internal / external の二つで尽きる \\
\texttt{componentProfile\_ext} & component profile は三成分の値で一意に決まる \\
\texttt{supplyProfile\_ext} & supply profile は internal/external × 三成分の六座標で一意に決まる \\
\texttt{fromInternalExternal\_internalProfile\_externalProfile} & 任意の supply profile は internal profile と external profile に分解できる \\
\texttt{fromInternalExternal\_eq\_iff} & その internal/external 分解は一意である \\
\texttt{effectiveProfile} & internal/external の供給を成分ごとに集約し、effective component profile を作る \\
\texttt{effectiveMaintenance\_nonneg} & 非負供給と非負性保存 aggregator のもとで、effective maintenance capacity は非負になる \\
\texttt{MaintenanceInterface.everyMechanism\_has\_threeComponentRepresentation} & M interface に入った raw mechanism は三成分 profile として表現される \\
\texttt{MaintenanceInterface.observationallyEquivalent\_iff\_three\_coordinates} & M interface 上の観測同値性は三成分の一致と同値である \\
\texttt{MaintenanceInterface.noFourthObservableCoordinate} & 三成分が一致する二つの mechanism を分ける第四 M 座標は interface 内部には存在しない \\
\texttt{MaintenanceInterface.maintenanceReadout\_eq\_of\_same\_three\_coordinates} & 任意の M-side readout は三成分が一致する mechanism を区別できない \\
\texttt{MaintenanceInterface.factors\_through\_observationQuotient\_iff\_respects\_equivalence} & 観測量が標準商を通ることと、M 観測同値性を尊重することは同値である \\
\texttt{MaintenanceInterface.maintenanceReadout\_factors\_through\_observationQuotient} & 任意の M-side readout は標準商 \texttt{ObservationQuotient} を通る \\
\texttt{MaintenanceInterface.quotientFactor\_unique} & 標準商を通る factorization は一意である \\
\texttt{MaintenanceInterface.quotientProfile\_injective} & 標準商の各クラスは三成分 profile によって忠実に表現される \\
\texttt{MaintenanceInterface.quotientProfile\_eq\_iff} & 標準商上の等号は complete profile の等号と同値である \\
\texttt{MaintenanceInterface.outsideInterface\_if\_distinguishes\_observationallyEquivalent} & 同じ三成分 profile を持つ mechanism を区別する追加量は M interface を factor しない \\
\texttt{MaintenanceInterface.outsideInterface\_if\_distinguishes\_same\_three\_coordinates} & 三成分が一致する mechanism を区別する追加量は M interface の外側に属する \\
\texttt{PartialMaintenanceInterface.representable\_or\_outside} & 候補 mechanism は三成分 profile に表現されるか、M interface の外側に置かれる \\
\end{longtable}
}
...
\subsection{補論 B との関係}\label{ux88dcux8ad6-b-ux3068ux306eux95a2ux4fc2}
...
\section{Route C companion I / II の成分対応}\label{route-c-companion-i-ii-ux306eux6210ux5206ux5bfeux5fdc}
...
\subsection{対応の原則: 成分と担い手を分ける}\label{ux5bfeux5fdcux306eux539fux5247-ux6210ux5206ux3068ux62c5ux3044ux624bux3092ux5206ux3051ux308b}
...
\subsection{Route C companion I: 推論時矛盾と外部代謝}\label{route-c-companion-i-ux63a8ux8ad6ux6642ux77dbux76feux3068ux5916ux90e8ux4ee3ux8b1d}
...
Exp.40 は、矛盾の有無ではなく、矛盾が task 外に範囲づけられているかどうかを前向きに比較した。32K 文脈に固定し、\texttt{zero\_sanity}, \texttt{scoped}, \texttt{subtle}, \texttt{structural} を各 50 試行で比較した結果は次である。
...
\texttt{scoped} が \texttt{zero\_sanity} と同水準に戻り、\texttt{subtle} と \texttt{structural} が崩れるという結果は、単に矛盾らしき文があるかどうかではなく、その衝突が task から範囲づけられているかどうかが重要であることを示した。本補論の語彙では、これは base LLM の in-context \(\gamma_{\mathrm{recovery}}\) が prompt design によって誘導された indicator と読める。

Exp.42 は、この repair をさらに分解した。\texttt{strong\_scope}, \texttt{medium\_scope}, \texttt{weak\_scope}, \texttt{subtle} の四段階で、正答率は次のようになった。
...
\bottomrule\noalign{}
\endlastfoot
strong\_scope & 50/50 = 1.00 & 0/50 \\
medium\_scope & 49/50 = 0.98 & 0/50 \\
weak\_scope & 42/50 = 0.84 & 1/50 \\
subtle & 10/50 = 0.20 & 25/50 \\
\end{longtable}
}

row-level では、exact wrong-sum adoption が \texttt{subtle} の \(25/40 mistakes = 0.625\)から、\texttt{weak\_scope} の \(1/8 mistakes = 0.125\)、\texttt{medium\_scope} / \texttt{strong\_scope} の 0 へ落ちた。これは、明示命令だけでなく、参照元 attribution という最小の source label が contradiction-taking を大きく抑えることを示す。
...
Exp.41 は、この方向が \texttt{gpt-4.1-mini} 固有でないことを検査した。\texttt{gpt-4.1-nano} では \texttt{scoped=27/30\ =\ 0.90}, \texttt{structural=1/30\ =\ 0.03}、\texttt{gemini-3.1-flash-lite-preview} では \texttt{scoped=30/30\ =\ 1.00}, \texttt{structural=14/30\ =\ 0.47} であり、二つの primary model の両方で \texttt{scoped\ \textgreater{}\ structural} が成立した。ただし \texttt{subtle} と \texttt{structural} の相対順序はモデル依存であった。したがって、本補論が受け取るべき invariant は、固定された subtle/structural ranking ではなく、scope marker が repair 的に働くという狭い方向である。

特に \texttt{gpt-4.1-nano} では \texttt{subtle=30/30\ =\ 1.00} と天井に張りついたため、secondary ordering は固定的な invariant として扱わない。
...
ON vs OFF は \texttt{p\ =\ 0.0004}, Cohen's \texttt{d\ =\ 8.80} であった。qwen3.5:27b の追試では、ON の代謝 LLM に Claude Sonnet を使用し、全体正答率は ON 64.4\%、OFF 44.4\% であった。

この効果は in-context marker とは階層が異なる。代謝 pipeline が、古い情報と新しい情報の衝突を検出し、旧値 -\textgreater{} 新値という範囲づけられた形へ変換してから base system に供給している。本補論の語彙では、これは \(M_{\mathrm{ext}\to\mathrm{recovery}}\) の indicator である。
...
\subsection{Route C companion II: LoRA 継続学習と依存再編}\label{route-c-companion-ii-lora-ux7d99ux7d9aux5b66ux7fd2ux3068ux4f9dux5b58ux518dux7de8}
...
\subsection{成分対応表}\label{ux6210ux5206ux5bfeux5fdcux8868}
...
Exp.42 attribution & in-context \(M_{\mathrm{recovery}}\) & base LLM + prompt & wrong-sum adoption: subtle 25/40 mistakes -\textgreater{} weak 1/8 -\textgreater{} medium/strong 0 & 最小 source label が repair を誘導 \\
Exp.41 width & in-context \(M_{\mathrm{recovery}}\) の幅確認 & base LLM + prompt & scoped \textgreater{} structural in 2/2 primary models & invariant は scope protection \\
...
F-v2c & \(M_{\mathrm{ext}\to\mathrm{recovery}}\) & external controller + training signal & DC 0.189 -\textgreater{} 0.333, T1 retention 0.000 & DAG 沿いの選択的 repair \\
...
\subsection{Route C companion II §7.5 の三役分離との接続}\label{route-c-companion-ii-7.5-ux306eux4e09ux5f79ux5206ux96e2ux3068ux306eux63a5ux7d9a}
...
\subsection{非主張}\label{ux975eux4e3bux5f35}
...
\tightlist
\item
  本節は \(M_{\mathrm{buffer}}^{\mathrm{int}}, M_{\mathrm{recovery}}^{\mathrm{int}}, M_{\mathrm{reconfiguration}}^{\mathrm{int}}\) や \(M_{\mathrm{ext}\to j}\) の数値を直接推定しない。
\item
  Exp.40 / 42 / 41 の scope 効果と、外部代謝 ON/OFF の効果が同一機構であるとは主張しない。共通しているのは repair-like な観測帰結であり、供給階層は異なる。
\item
  gemma3:27b の自己代謝と qwen3.5:27b + Sonnet の外部代謝を同一条件として合算しない。前者は coupled process、後者は teacher-like external process を含む。 \# F-v2c の改善を依存 DAG の寄与だけに還元しない。再提示件数、除外ポリシー、学習安定性が交絡しうる。
\end{enumerate}

\section{F-multi を実用性能として読まない。理想振り分け条件で得た上界 indicator である。}\label{f-multi-ux3092ux5b9fux7528ux6027ux80fdux3068ux3057ux3066ux8aadux307eux306aux3044ux7406ux60f3ux632fux308aux5206ux3051ux6761ux4ef6ux3067ux5f97ux305fux4e0aux754c-indicator-ux3067ux3042ux308b}

\section{\texorpdfstring{Exp.36 / Exp.39 は \(M\)-component の直接証拠ではなく、L-side quality anchor として扱う。}{Exp.36 / Exp.39 は M-component の直接証拠ではなく、L-side quality anchor として扱う。}}\label{exp.36-exp.39-ux306f-m-component-ux306eux76f4ux63a5ux8a3cux62e0ux3067ux306fux306aux304fl-side-quality-anchor-ux3068ux3057ux3066ux6271ux3046}

\section{\texorpdfstring{DeltaLint、SAT、Mixed-CSP の \(M\)-component 解釈は本節では扱わない。これらは §5-6 または別稿で扱う。}{DeltaLint、SAT、Mixed-CSP の M-component 解釈は本節では扱わない。これらは §5-6 または別稿で扱う。}}\label{deltalintsatmixed-csp-ux306e-m-component-ux89e3ux91c8ux306fux672cux7bc0ux3067ux306fux6271ux308fux306aux3044ux3053ux308cux3089ux306f-5-6-ux307eux305fux306fux5225ux7a3fux3067ux6271ux3046}
...
\textbf{Case A: high \(M_{\mathrm{buffer}}\), low \(M_{\mathrm{recovery}}\)}
...
\textbf{Case B: sufficient \(M_{\mathrm{recovery}}\), low \(M_{\mathrm{reconfiguration}}\)}
...
\textbf{Case C: high \(M_{\mathrm{ext}\to\mathrm{recovery}}\), low internal \(M_{\mathrm{recovery}}\)}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Software / SaaS が Route A ドメインであるとは主張しない。
\item
  \(\hat L\) が真の \(L\) と同一であるとは主張しない。
\item
  \(M_i\) が単一の universal metric で測れるとは主張しない。
\item
  DeltaLint の既存実績だけで本補論が実証されたとは主張しない。DeltaLint は本補論の主 validation から切り離し、Route C companion I / L-side の static-code extension として別 note で扱う。
\item
  外部供給 channel が常に望ましいとは主張しない。外部支援は短期維持を助けるが、自律的能力を代替しない場合がある。
\item
  \(M_{\mathrm{reconfiguration}}\) によって \(F\) 自体を変更してよいとは主張しない。
\item
  経験的検証プロトコル
\end{enumerate}
...
\tightlist
\item
  対象となる software / SaaS system。
\item
  時間窓ごとの \(\hat L\)、raw \(R\)、component profile の測定。
\item
  実際に行われた介入 \(I_{\mathrm{buffer}}, I_{\mathrm{recovery}}, I_{\mathrm{reconfiguration}}, I_{\mathrm{ext}\to j}\) の履歴。
\item
  介入後の outcome 変化。
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  現時点で本補論の M-framework が実証済みであるとは主張しない。
\item
  Software / SaaS が Route A ドメインであるとは主張しない。
\item
  \(\hat L_{\mathrm{pilot}}\) が真の \(L\) であるとは主張しない。
\item
  DeltaLint が本補論の validation であるとは主張しない。
\item
  単一の \(\rho_i\)、\(\Phi\)、\(A_j\) が全ドメインで正しいとは主張しない。
\item
  介入履歴なしの risk prediction だけで介入順位予測が検証されたとは主張しない。
\item
  観察データから推定した \(\mathrm{rank}_{\mathrm{obs}}\) が、直ちに因果的効果順位を表すとは主張しない。randomized intervention assignment または明示的な causal identification がない限り、本補論の intervention-ranking support は observational support である。
\item
  限界と次段階
\end{enumerate}
...
\item
  \(F / \Sigma / R / M\) の操作的分解。
\item
  software / SaaS における component-based intervention-ranking prediction。
\item
  その予測を検査するための preregistered validation protocol。
\end{enumerate}

したがって、本補論の現在地は「実証済み論文」ではなく、「強い実証可能性を持つ framework / protocol paper」である。

\subsection{Operational dataset の未取得}\label{operational-dataset-ux306eux672aux53d6ux5f97}
...
\subsection{Software は Route C であり Route A ではない}\label{software-ux306f-route-c-ux3067ux3042ux308a-route-a-ux3067ux306fux306aux3044}
...
\subsection{\texorpdfstring{\(\hat L_{\mathrm{pilot}}\) は真の \(L\) ではない}{\textbackslash hat L\_\{\textbackslash mathrm\{pilot\}\} は真の L ではない}}\label{hat-l_mathrmpilot-ux306fux771fux306e-l-ux3067ux306fux306aux3044}
...
\subsection{Component signal は直接測定ではない}\label{component-signal-ux306fux76f4ux63a5ux6e2cux5b9aux3067ux306fux306aux3044}
...
\subsection{観察的 support と因果的 support}\label{ux89b3ux5bdfux7684-support-ux3068ux56e0ux679cux7684-support}
...
\subsection{Underpowered pilot の扱い}\label{underpowered-pilot-ux306eux6271ux3044}
...
\subsection{\texorpdfstring{\(\rho_i\), \(\Phi\), \(A_j\) の一意性は主張しない}{\textbackslash rho\_i, \textbackslash Phi, A\_j の一意性は主張しない}}\label{rho_i-phi-a_j-ux306eux4e00ux610fux6027ux306fux4e3bux5f35ux3057ux306aux3044}
...
\subsection{静的形式に留まる}\label{ux9759ux7684ux5f62ux5f0fux306bux7559ux307eux308b}
...
\subsection{Domain generalization は未完}\label{domain-generalization-ux306fux672aux5b8c}
...
\subsection{DeltaLint は並行 track である}\label{deltalint-ux306fux4e26ux884c-track-ux3067ux3042ux308b}
...
\subsection{次段階の研究課題}\label{ux6b21ux6bb5ux968eux306eux7814ux7a76ux8ab2ux984c}
