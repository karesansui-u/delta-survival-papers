...
\subsection{\texorpdfstring{有効維持余力と集約関数 \(\Phi\)}{有効維持余力と集約関数 \textbackslash Phi}}\label{ux6709ux52b9ux7dadux6301ux4f59ux529bux3068ux96c6ux7d04ux95a2ux6570-phi}
...
\item
  介入履歴なしの risk prediction だけで介入効果仮説が検証されたとは主張しない。
\item
  観察データから推定した \(\mathrm{rank}_{\mathrm{obs}}\) が、直ちに因果的効果順位を表すとは主張しない。randomized intervention assignment または明示的な causal identification がない限り、下流の介入効果 support は observational support である。
\item
  限界と次段階
\end{enumerate}
...
\item
  \(F / \Sigma / R / M\) の操作的分解。
\item
  software / SaaS における component-based M-profile validation と、必要な場合の下流 intervention-effect hypothesis。
\item
...
\subsection{Software contract-coherence diagnostics は並行 track である}\label{software-contract-coherence-diagnostics-ux306fux4e26ux884c-track-ux3067ux3042ux308b}
