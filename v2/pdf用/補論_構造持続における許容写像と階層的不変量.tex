...
\item
  set-level な粗視化に対する飽和欠損恒等式、正有限質量条件、DPI 型単調性の条件切り。
\item
...
\texttt{Survival.SaturationDefect} が閉じるのは、集合論的な粗視化写像そのものではなく、readout-level の狭い spec である。すなわち、一歩ごとの粗視化 log-ratio loss が微視的 loss と欠損差 \(e_{t}-e_{t+1}\) だけ異なるなら、累積量は
