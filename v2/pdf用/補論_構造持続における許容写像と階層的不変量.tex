...
Bernoulli-CSP 側では、\texttt{Survival.BernoulliTypicalSigma} が累積 bad-event production を \(\Sigma\) として読む薄い入口を与える。これは、期待単調性、finite-path Chernoff lower-tail、good-event lower-bound certificate、collapse wrapper を同じ \(\Sigma\) 語彙で参照可能にするためのものであり、無条件の第二法則型単調性や Bernoulli-CSP class の最大閉包性までは主張しない。
