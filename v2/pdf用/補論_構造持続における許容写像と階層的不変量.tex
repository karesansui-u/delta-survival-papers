...
\item
  同型不変性と正ゲージ共変性の reader-facing wrapper。
\item
  粗視化に対する飽和欠損恒等式と、DPI 型単調性の条件切り。
\item
...
したがって、本補論は新しい Lean 定理を主張するものではない。既存の粗視化・総生成量・Bernoulli-CSP interface を、一つの数学的見取り図にまとめ、次にどの wrapper を閉じるべきかを指定する文書である。
