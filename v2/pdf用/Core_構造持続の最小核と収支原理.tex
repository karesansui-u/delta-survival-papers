...
\begin{abstract}
本稿は、分冊版である「構造持続の最小形式」と「構造持続の収支原理」を、外部読者向けに一つの導線として再構成するための骨子である。新しい理論核、新しい定理、新しい実験的 support を追加するものではない。厳密な公理、定理、証明、限界条件は、分冊版である Paper 1 / Paper 2 および対応する技術補論を基準とする。
...
本 Core Paper & Paper 1 / Paper 2 を外部読者向けに一本の導線として読む骨子 \\
...
これは「世界が経験的に指数減衰する」という主張ではない。事前固定された \(V,m\) の残存比を対数尺度で測ると、望遠鏡積から指数核が現れる、という表現定理である。
