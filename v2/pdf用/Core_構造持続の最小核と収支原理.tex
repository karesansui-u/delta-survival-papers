...
の形を取る。ここで \(L\) は累積構造消耗量、\(M\) は有効維持余力である。
...
ここに有効維持余力 \(M\) を別途導入すると、最小の構造持続量は

\[
S=Me^{-L}
\]

と書ける。\(M\) は指数核そのものから導かれる量ではなく、構造を維持するために実際に使える余力、支援能力、再構成能力を表す外側の項である。
...
本稿は、\(M\) が指数核から導かれるとは主張しない。\(M\) は有効維持余力を表す外側の項であり、その操作化は別補論の対象である。
