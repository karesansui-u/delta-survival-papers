...
\bottomrule\noalign{}
\endlastfoot
loss-only kernel & 構造維持可能集合の縮小を対数比で測ると S = M exp(-L) が現れる \\
...
\section{Route classification}\label{route-classification}

Route は、ドメインの証拠強度を表す。
...
\bottomrule\noalign{}
\endlastfoot
Route A & 状態空間、測度、構造消耗、collapse boundary が仕様から自然に固定される & theorem-side / finite-horizon / prospective empirical support を強く読む \\
Route B & 自然測度は弱いが、bounded approximation や限定的な自然 proxy がある & 限定主張として読む \\
Route C & proxy、観測指標、操作的写像によって検査する & empirical anchor / operational support として読む \\
\end{longtable}
}

Route C は、LLM、software、組織、maintenance log のような現実ドメインで、観測指標と凍結写像により追加予測力を検査する層である。ただし Route C の成功を、Route A の定理的成功と同じ強さで書いてはならない。
...
Route C の成功が Route A の定理的成功を意味する & 証拠強度が異なる \\
...
各補論は、この運用規律を全文再掲する必要はない。各補論には、その補論固有の local guardrail を短く残す。Route 分類、G6 分類、mapping status、support / no-support / silence の完全定義は、本補論と補論「構造持続写像の標準手順」を参照する。
