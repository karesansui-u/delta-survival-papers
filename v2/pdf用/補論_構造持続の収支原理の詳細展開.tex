...
\section{仕様固定構造ドメイン anchors}\label{ux4ed5ux69d8ux56faux5b9aux69cbux9020ux30c9ux30e1ux30a4ux30f3-anchors}
...
\subsection{仕様固定構造ドメイン anchor の現在地}\label{ux4ed5ux69d8ux56faux5b9aux69cbux9020ux30c9ux30e1ux30a4ux30f3-anchor-ux306eux73feux5728ux5730}
...
\subsection{仕様固定構造ドメイン anchor の限界}\label{ux4ed5ux69d8ux56faux5b9aux69cbux9020ux30c9ux30e1ux30a4ux30f3-anchor-ux306eux9650ux754c}
...
\item
  finite-horizon exposure bound は、無限時間の ergodic law や threshold theorem そのものではない。 \# 仕様固定構造ドメイン が広がっても、独立再現や異質ドメインへの写像なしに universal law は確立しない。
\end{enumerate}
...
しかし、それは構造持続の収支原理が使えないという意味ではない。むしろ、構造推定ドメインでは \(r_{t}\) の observable indicator を慎重に設計し、期待値レベルまたは prospective prediction の形で検査する必要がある。次節では、LLM companion I / II の scope-as-repair, external metabolism, dependency-aware replay を、回復量 \(r_{t}\) の構造推定ドメイン anchor として整理する。
...
\subsection{構造推定ドメインの証拠形式}\label{ux69cbux9020ux63a8ux5b9aux30c9ux30e1ux30a4ux30f3ux306eux8a3cux62e0ux5f62ux5f0f}
...
LLM companion II が構造推定ドメインに与える教訓は、単なる reconfiguration と repair を分ける必要がある、という点である。LoRA は新しい信号に反応して表現を再構成するが、依存構造を自律的に修復するとは限らない。F-v2c は依存整合性を改善するが、旧知識保持を回復しない。F-multi は保持を一部改善するが、完全な dependency repair ではない。したがって、\(r_{t}\) は単一の「資源量」ではなく、どの種類の回復がどの構造消耗に効いているかを区別して測る必要がある。
...
\subsection{構造推定ドメインのまとめ}\label{ux69cbux9020ux63a8ux5b9aux30c9ux30e1ux30a4ux30f3ux306eux307eux3068ux3081}
...
\item
  仕様固定構造ドメインの定理層とは別に、実ドメインでの recovery design を導ける。
\end{enumerate}
...
したがって Lean が閉じているのは、構造持続の収支原理の代数核である。任意ドメインで自然な \(m\) が一意に定まること、\(r_{t}\) が観測可能であること、構造推定ドメインの repair-like effects が因果機構として同定されること、あるいは構造持続の収支原理が普遍法則として確立したことは、Lean の主張範囲外である。
...
\item
  \(r_{t}\) の真値を 構造推定ドメインの観測指標から直接測定できるとは主張しない。
\item
...
第一に、仕様固定構造ドメインでは、drift-weighted coordinate が raw count や encoding-size baseline を上回らない独立 family が見つかれば、Bernoulli-CSP universality-class claim は弱まる。Exp43c q-coloring ではこの反証経路を実際に primary validation として検査し、\texttt{fm\_plus\_n} が raw / density / CNF-size baselines を上回った。したがって q-coloring は現在では positive support である。一方、Cardinality-SAT の threshold-local test はまだ draft / calibration-stage extension であり、validation evidence ではない。
...
\item
  構造推定ドメインの新規 prospective test。 scope-as-repair や dependency-aware repair が、別タスク・別モデル・別ドメインでも baseline を上回るかを事前登録で検査する。
\item
...
\item
  仕様固定構造ドメイン補論群: SAT / Bernoulli-CSP / Mixed-CSP / Exp43c q-coloring、および Cardinality-SAT などの proposed stress extensions。
\item
  G4 非CSP補論群: queueing / reliability / decay / repair-maintenance による古典例への最小埋め込み。
\item
  LLM companion I / II: 構造推定ドメイン observational anchors。
\item
