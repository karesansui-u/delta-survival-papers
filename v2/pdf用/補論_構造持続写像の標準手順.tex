...
failure mode & proxy miss / weak-axis / leakage risk / small sample / no natural (r\_t) / no fresh surface など \\
再利用可能な教訓 & 次の写像で禁止または要求すべき条件 \\
次の扱い & rescue 禁止、fresh archive へ移す、探索候補へ戻す、silence に置く等 \\
\end{longtable}
}

この台帳は、no-support を理論の外に捨てるためではなく、理論の適用条件を明確化するために使う。たとえば、凍結後に外れた写像は no-support として残し、同じ archive 上で別 proxy を探して support に昇格させない。一方、その失敗から得た構造定義や proxy は、新しい validation candidate として別データ、future surface、または未接触 split で検証してよい。
...
ただし、現時点で直接観測できない内部機構があること自体は、直ちに本手順の対象外を意味しない。重要なのは、その内部機構を仮定しなくても、維持条件、崩壊条件、または設計変更後に観測すべき readout のいずれかが操作的に記述できるかどうかである。したがって、「まだ完全には見えていないが、代理指標と設計仮説を通じて検証可能な構造」は条件付き構造埋め込み層または構造推定層の候補になりうるが、「見えない本質」そのものを対象構造に据えることは許されない。
...
代理指標実証寄りのドメイン: - 維持条件は操作的に定義可能 - 有効選択肢空間は暗黙的または高次元 - 制約削減は直接測定困難で、代理指標が必要 - → 条件付き構造埋め込み層または構造推定層に進みやすい
...
直接 \(r_{i}\) が測れない場合は、真の \(L_{k}\) そのものではなく、代理累積構造消耗 L̂\_k を置く。L̂\_k に求めるのは真の \(L_{k}\) との完全一致ではなく、少なくとも次のいずれかを再現することである。
...
次の三層のいずれかに分類する。この分類はドメインそのものの価値の分類でも、対象名に固定ラベルを貼る分類でもない。その問題設定について、どの観測形式・どの強さの主張が可能かを示す分類である。仕様固定構造層では等式または有限時間の強い境界、条件付き構造埋め込み層では既存理論からの条件付き埋め込み、構造推定層では代理指標と凍結検証による追加予測力が主張される。反証条件と基準モデル比較が機能する限り、理論の適用可能性は検証可能なまま保たれる。
...
条件: - 普遍法則を直接主張する層ではない - ただし代理指標による予測・設計仮説・再現が可能
...
構造推定層（および条件付き構造埋め込み層で代理指標を併用する場合）で必須。
...
検証対象となる予測: - 制約の質の順序予測（どの制約型がより強く削るか） - 代理指標と維持指標の単調関係 - 基準モデルに対する追加予測力
...
\item
  observability / logging: 構造消耗量と回復量を後から測れるようにし、\(L\), \(r_{t}\), margin の代理指標を残す。
\item
...
手順 13: proxy ecosystem と昇格規則

構造持続理論の実証側は、一つの代理指標を一度当てて終わるものではない。代理指標は、発見され、凍結され、検証され、失敗も記録され、より強い測定が可能になれば版を分けて置き換えられる。この仕組みを proxy ecosystem と呼ぶ。
...
→ frozen test
→ internally supported proxy
→ externally supported proxy
→ cross-domain transferable principle
\end{verbatim}

ここで support と呼べるのは frozen test 以後である。探索中に当たった proxy は、どれほど見込みがあっても candidate signal または validation candidate に留まる。逆に、探索中に外れた proxy、凍結後に外れた proxy、baseline と重複していた proxy も、失敗台帳に記録する。失敗は、同じ proxy を別名で繰り返さないための制約である。

初期の仕様固定構造層、構造推定層、非CSP proxy は、将来のより詳細な測定に置き換えられうる。たとえば粗い構造消耗 proxy が、後に direct event log、repair / maintenance log、controlled-access data、または外部監査つき評価に置き換わることは望ましい。ただし、その置き換えは旧結果の上書きではない。旧 proxy は歴史的な support / no-support / weakening outcome として残し、新 proxy は別版として凍結し、別データ、future surface、fresh archive、または outside rerun で検証する。
...
この手順の目的は、proxy の精度を無限に上げてよいようにすることではなく、精度の向上を版管理し、昇格条件を明示することである。探索は自由でよい。しかし support は、版管理された frozen validation を通ったものだけに限る。
...
良条件: - 代理累積構造消耗 L̂ が維持指標を単調に予測する - 事前固定した設計変更後 readout が維持指標と整合する - frozen mapping の SP-only model が simple baseline に対して追加の予測力を持つ
