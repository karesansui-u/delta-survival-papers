...
本手順は、対象構造、維持条件、有効選択肢空間、制約集合、累積構造消耗 L、余力 μ、初期有効選択肢多様性 N\_eff\^{}(0) を順に定義し、仕様固定構造ドメイン、条件付き構造埋め込みドメイン、構造推定ドメインのいずれかへ位置づける。さらに、反証条件の事前固定、基準モデルとの比較、予測頑健性テスト、介入設計までを一つの流れとして与える。これにより、ドメインごとに異なる語で語られてきた崩壊や劣化を、共通の骨格のもとで比較・検証するための標準手順を与える。
...
\item
  \(P_{k}\): 条件付き構造埋め込みドメインで用いる、確率的または弱依存の記述のもとでの実現残存可能性。役割としては仕様固定構造ドメインの \(R_{k}\) に対応するが、参照モデルに対する境界として表すときにこの記号を用いる。
\end{itemize}
...
ただし、現時点で直接観測できない内部機構があること自体は、直ちに本手順の対象外を意味しない。重要なのは、その内部機構を仮定しなくても、維持条件、崩壊条件、介入効果のいずれかが操作的に記述できるかどうかである。したがって、「まだ完全には見えていないが、代理指標と介入設計を通じて検証可能な構造」は条件付き構造埋め込みドメインまたは構造推定ドメインの候補になりうるが、「見えない本質」そのものを対象構造に据えることは許されない。
...
定理化寄りのドメイン: - 維持条件が外部的に明示定義できる - 有効選択肢空間が比較的明示的 - 制約による削減率が直接または近似的に計算できる - → 仕様固定構造ドメインまたは条件付き構造埋め込みドメインに進みやすい

例: SAT、明示制約型 LLM タスク、一部の形式推論環境

代理指標実証寄りのドメイン: - 維持条件は操作的に定義可能 - 有効選択肢空間は暗黙的または高次元 - 制約削減は直接測定困難で、代理指標が必要 - → 条件付き構造埋め込みドメインまたは構造推定ドメインに進みやすい
...
次の三層のいずれかに分類する。この分類はドメインそのものの価値の分類ではなく、そのドメインについて本稿でどの観測形式・どの強さの主張が可能かを示す分類である。仕様固定構造ドメインでは等式または有限時間の強い境界、条件付き構造埋め込みドメインでは既存理論からの条件付き埋め込み、構造推定ドメインでは代理指標と凍結検証による追加予測力が主張される。反証条件と基準モデル比較が機能する限り、理論の適用可能性は検証可能なまま保たれる。

仕様固定構造ドメイン（内部旧称 Route A）
...
条件付き構造埋め込みドメイン（内部旧称 Route B）

条件: - 仕様固定構造ドメインとしての等式は置けない - ただし、参照累積構造消耗量 \(L_{ref}\) からの相対誤差が有界
...
構造推定ドメイン（および条件付き構造埋め込みドメインで代理指標を併用する場合）で必須。
...
写像状態 & 構造推定ドメインの candidate / frozen experiment ごとに判定 \\
...
観測可能性の層 & 一般会話では構造推定ドメイン。明示制約タスクでは仕様固定構造ドメインまたは条件付き構造埋め込みドメイン \\
