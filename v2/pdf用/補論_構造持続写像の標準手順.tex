...
本手順は、対象構造、維持条件、有効選択肢空間、制約集合、累積構造消耗 L、余力 μ、初期有効選択肢多様性 N\_eff\^{}(0) を順に定義し、仕様固定構造層、条件付き構造埋め込み層、構造推定層のいずれかへ位置づける。さらに、反証条件の事前固定、基準モデルとの比較、予測頑健性テスト、介入設計までを一つの流れとして与える。これにより、ドメインごとに異なる語で語られてきた崩壊や劣化を、共通の骨格のもとで比較・検証するための標準手順を与える。
...
第一段階は、探索的写像フェーズである。ここでは、既存データ、専門知識、予備実験、失敗例を用いて、そのドメインで何が維持対象であり、どの測度 m、どの構造消耗指標 L または L̂、どの回復指標 \(r_{t}\) が自然かを探してよい。この段階での候補生成や再定義は、科学的発見の一部である。ただし、この段階で同じデータにうまく適合したことは、support ではない。探索中の改善は candidate signal または exploratory gain と呼び、凍結検証に進める価値があると判断したものだけを validation candidate として記録する。
...
μ と N\_eff\^{}(0) は最小形式の M（有効維持余力）を構成する二つの側面であり、μ は余力の絶対量、N\_eff\^{}(0) は初期または外生的な選択肢多様性に対応する。現在の有効選択肢の減少そのものは e\^{}\{-L\} に含まれる。両者を分離できるドメインでは分けて扱い、分離困難なドメインでは M を一体のまま扱う。
...
\item
  \(M_{k}\): 有効維持余力。余力や初期的・外生的な多様性を含む、構造持続を支える側の項である。
\item
...
探索的写像フェーズでは、観測指標や推定指標を見ながら、どの対象構造が自然かを発見してよい。ただし、その結果は candidate mapping であり、support ではない。凍結検証フェーズに入る前に、対象構造、対象スコープ、観測期間または観測単位を固定する。凍結後に、うまく当てはまる対象構造が見つかるまで定義をずらし続けることは許されない。
...
ここでの \(L_{k}\) は、真の累積構造消耗である。

推定指標形（直接 \(r_{i}\) が測れない場合）:

直接 \(r_{i}\) が測れない場合は、真の \(L_{k}\) そのものではなく、推定累積構造消耗 L̂\_k を置く。L̂\_k に求めるのは真の \(L_{k}\) との完全一致ではなく、少なくとも次のいずれかを再現することである。
...
次の三層のいずれかに分類する。この分類はドメインそのものの価値の分類でも、対象名に固定ラベルを貼る分類でもない。その問題設定について、どの観測形式・どの強さの主張が可能かを示す分類である。仕様固定構造層では等式または有限時間の強い境界、条件付き構造埋め込み層では既存理論からの条件付き埋め込み、構造推定層では観測・推定指標と凍結検証による追加予測力が主張される。反証条件と基準モデル比較が機能する限り、理論の適用可能性は検証可能なまま保たれる。

仕様固定構造層

条件: - \(A_{k}\) が定義可能 - 各段階の残存比 \(r_{i}\) が定義可能 - 累積構造消耗 \(L_{k}\) = Σ{[}-ln(\(r_{i}\)){]} が定義可能 - 共通測度のもとで残存集合の比較ができる

目標: \textbar A\_k（残存）\textbar{} = \textbar A\_k\^{}0\textbar{} e\^{}\{-L\_k\}

条件付き構造埋め込み層

条件: - 仕様固定構造層としての等式は置けない - ただし、参照累積構造消耗量 \(L_{ref}\) からの相対誤差が有界

目標: \[
e^{-L_{ref}(1+\rho)} \le P_{k} \le e^{-L_{ref}(1-\rho)}
\]

既存理論の弱依存境界を用いる。ρ は段階構造消耗間の依存が参照累積構造消耗量に及ぼす相対誤差の上界である。ここで \(P_{k}\) は、弱依存や確率モデルのもとで記述された実現残存可能性を表す。
...
\item
  observability / logging: 構造消耗量と回復量を後から測れるようにし、\(L\), \(r_{t}\), margin の観測・推定指標を残す。
\item
...
出力: - 転用元ドメイン - 転用元の loss / repair / margin / alternative-path 座標 - 転用先ドメインでの candidate mapping - candidate intervention - 凍結検証計画
...
r\_t 定義 & 回復量を扱う場合の定義。扱わない場合は loss-only と明記 \\
μ 定義 & 余力定義 \\
...
良条件: - 推定累積構造消耗 L̂ が維持指標を単調に予測する - 事前固定した設計変更後 readout が維持指標と整合する - frozen mapping の SP-only model が simple baseline に対して追加の予測力を持つ
