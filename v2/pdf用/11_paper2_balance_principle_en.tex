...
The term \(M_{n}\) is not derived from this pathwise identity. It is the resource-side scalar representing the effective maintenance surplus available to support the structure. The boundary \(M_{n}=0\) may appear as functional failure or halt even when some feasible region remains. This paper does not claim a universal predictive decomposition of \(M_{n}\).
