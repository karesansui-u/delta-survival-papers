...
Here \(S_{n}\) is the structural persistence potential for the maintenance condition \(G\). Reaching \(S_{n}=0\), or a pre-fixed threshold \(S_{n}\le S_{c}\), means reaching the structural maintenance boundary for that condition. Observationally, the boundary may appear as collapse, functional failure, halt, phase transition, or structural reorganization.

The term \(M_{n}\) is not derived from this pathwise identity. It is the resource-side scalar representing the effective maintenance surplus available for \(G\). The structural maintenance boundary can be reached through the feasible-region route represented by cumulative net consumption \(B_{n}\), the resource-side route represented by \(M_{n}=0\), or both. This paper does not claim a universal predictive decomposition of \(M_{n}\).
