...
最後に、有効維持余力 \(M\) を別途導入し、構造持続量を
\[
  S = M e^{-L}
\]
と定義する。ここで \(M\) は指数核そのものから導かれる量ではなく、構造を支える側の余力・支援能力を組み込むための拡張的定義である。
...
ここで \(M\) は、命題 1 の指数核から導かれる量ではない。支える側の余力・支援能力を組み込むために別途導入する拡張的定義である。\(m(V^{(0)})\) が構造維持可能状態の初期的広がりを表すのに対し、\(M\) はその構造を維持するために利用できる有効維持余力を表す。
