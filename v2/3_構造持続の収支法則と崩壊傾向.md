...
|---|---|---|
| \(a_t=\ell_t-g_t\) | `StructuralBalanceLaw.signedAction_eq_loss_sub_gain` | 定義として証明済み |
| \(A_n=\sum_{t<n}a_t\) | `StructuralBalanceLaw.cumulativeSignedAction_eq_sum_signedAction` | finite-prefix sum として証明済み |
...
9. 探索的に発見された \(V\), \(m\), \(\ell_t\), \(g_t\) の候補を、そのまま support と呼ばない。
