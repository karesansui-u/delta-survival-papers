...
この非CSP側の最も強い安全な言い方は、restricted repairable stochastic systems における **conditional law-side bridge** である。これは、構造収支律が一般 non-CSP universal law だと言うことではない。むしろ、queueing / Foster-Lyapunov drift のように自然な \(m\)、観測可能な \(g_t\)、条件つき collapse / hitting boundary が揃う限定クラスでは、構造収支律が既存安定性理論の内部に law-side に近い形で埋め込まれる、と言うのである。
