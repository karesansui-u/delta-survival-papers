...
本稿が主理論 spine に与える核は、Paper 1 の loss-only kernel を \(g_t=0\) の特例として回収しつつ、損失流 \(\ell_t\) と補償流 \(g_t\) の差し引きが、構造維持可能領域の指数的変化を支配するという収支恒等式である。
...
- あらゆるドメインで自然な測度 \(m\)、損失流 \(\ell_t\)、補償流 \(g_t\) が一意に定まるとは主張しない。
...
本稿の pathwise algebraic kernel は、Lean 側では `Survival/GeneralStateDynamics.lean` によってすでに machine-checked である。そこでは、収縮後集合、修復後集合、feasible mass、stage loss、stage gain、signed net action、cumulative net action が定義され、positive finite trajectory assumptions の下で
\[
  m(V^{(t+1)})=m(V^{(t)})e^{-a_t},
  \qquad
  m(V^{(n)})=m(V^{(0)})e^{-A_n}
\]
が証明されている。追加した `Survival/StructuralBalanceLaw.lean` は、この既存 theorem 群を Paper 3 の読者向け名称で束ねる薄い wrapper であり、新しい数学的仮定を追加するものではない。

Lean 対応は次の範囲に限られる。

| Paper 3 の主張 | Lean 側の対応 | 読み |
|---|---|---|
| signed action definition | `StructuralBalanceLaw.signedAction_eq_loss_sub_gain` | 定義として証明済み |
| cumulative signed action | `StructuralBalanceLaw.cumulativeSignedAction_eq_sum_signedAction` | finite-prefix sum として証明済み |
| local balance | `StructuralBalanceLaw.local_signed_exponential_balance` | positive mass assumptions の下で証明済み |
| pathwise signed kernel | `StructuralBalanceLaw.pathwise_signed_exponential_kernel` | positive finite trajectory assumptions の下で証明済み |
| loss-only 回収 | `StructuralBalanceLaw.pureContraction_recovers_loss_only_kernel` | pure contraction / zero gain の特例として証明済み |
| Lyapunov drift embedding | `StructuralBalanceLaw.lyapunov_*` wrappers | 最小代数的埋め込みとして証明済み |
| repair / maintenance balance | `StructuralBalanceLaw.repair_*` wrappers | finite-prefix damage-minus-repair skeleton として証明済み |
...
したがって Lean が閉じているのは、構造収支律の代数核である。任意ドメインで自然な \(m\) が一意に定まること、\(g_t\) が観測可能であること、Route C の repair-like effects が因果機構として同定されること、あるいは構造収支律が普遍法則として確立したことは、Lean の主張範囲外である。
...
この非CSP側の最も強い安全な言い方は、queueing / Foster-Lyapunov drift を中心とする restricted drift-based stability class における **conditional law-side bridge** である。これは、構造収支律が一般 non-CSP universal law だと言うことではない。むしろ、自然な \(m\)、観測可能な \(g_t\)、条件つき collapse / hitting boundary が揃う限定クラスでは、構造収支律が既存安定性理論の内部に law-side に近い形で埋め込まれる、と言うのである。Repair / maintenance balance は、その class を empirical \(g_t\) 側へ広げる near-bridge open-system anchor である。
