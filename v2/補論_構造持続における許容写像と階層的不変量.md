...
2. defect-controlled な粗視化に対する、飽和欠損条件と admissible-map compatibility の統合。
3. Bernoulli-CSP class をテンプレート保存 subcategory / generated class として読む外向け定理名。
4. 代理観測写像は不変量定理ではなく frozen validation に落ちる、という運用側の theorem map。
...
残る作業は、exact preservation ではなく、飽和欠損を持つ非 exact coarse-graining において、feasible trajectory、収縮作用、回復作用との compatibility と欠損項条件をどう組み合わせれば DPI 型単調性を読めるかを定式化することである。
