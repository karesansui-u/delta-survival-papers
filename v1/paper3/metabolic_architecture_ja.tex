...
さらにその研究は、非独立な制約を弱依存性サンドイッチ（$e^{-\delta(1+\rho)} \leq P \leq e^{-\delta(1-\rho)}$）で扱う枠組みに拡張しており、制約相関$\rho = 0$で$e^{-\delta}$に厳密帰着し、それ以外では保証付き保守的上界を提供する。この理論連鎖はLean~4で機械検証されており、$\texttt{sorry} = 0$かつ$\texttt{axiom} = 0$である。
