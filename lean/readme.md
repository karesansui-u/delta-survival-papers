...
| Route A non-CSP core examples | `SerialReliability`, `ConstantFractionDecay`, `BranchingProcessExtinction` | Serial reliability block diagrams, constant-fraction decay, branching-process expectation skeletons, additive log loss, and `R = exp (-L)` threshold crossing |
