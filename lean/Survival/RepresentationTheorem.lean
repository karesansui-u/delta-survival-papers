...
namespace Survival.RepresentationTheorem
...
/-- **Representation Theorem (survival potential form):**
    For any persistence functional F, the "survival potential" of a
    positive mass sequence m₀, m₁, ..., mₙ must be of the form

        m₀ · exp(-k · Σᵢ l_i)

    where l_i = -log(m_{i+1}/m_i) and k ≥ 0.

    Under the unit convention k = 1, this is S = M exp(-L). -/
theorem survival_potential_form (F : PersistenceFunctional)
...
end Survival.RepresentationTheorem
