...
epistemic-control baseline comparison、
evaluation-facing metric contract、
benchmark protocol contract、
benchmark result-certificate bridge、
...
| Premise-update output collection | `v3/05_evidence/llm_epistemic_premise_update_v0/output_collection_protocol_v0.md` / `analysis/epistemic_control_premise_update_v0/make_output_template.py` | future run の raw baseline / controlled outputs を集めるための JSONL surface を固定する。model outputs ではない |
...
| Reader-facing figure | `v3/01_theory/figures/figure5_iqc_assumption_to_guarantee_chain_en.svg` | raw 情報 → 資格化制御 → runner / evidence witness → Lean 比較定理の一枚 chain を図示する |
...
| [`EpistemicControlComparison.lean`](Survival/EpistemicControlComparison.lean) | `controlled_coherentMass_ge_baseline`: same-initial-mass かつ no-worse cumulative net action なら controlled coherent mass が baseline 以上 | bridge 上の有限 baseline comparison。実モデル性能ではなく、抽象 accounting interface の条件付き比較保証 |
| [`EpistemicControlEvaluationContract.lean`](Survival/EpistemicControlEvaluationContract.lean) | `metric_net_action_no_worse`, `metrics_controlled_coherentMass_ge_baseline`: per-step loss / repair metrics から `NetActionNoWorse` と coherent-mass 比較へ接続 | 実験 metric が theorem 仮定を witness するための有限 evaluation contract。metric 自体の妥当性や実モデル性能は証明しない |
| [`EpistemicBenchmarkProtocol.lean`](Survival/EpistemicBenchmarkProtocol.lean) | `benchmark_protocol_implies_net_action_no_worse`, `benchmark_protocol_implies_controlled_mass_ge_baseline`: valid benchmark protocol から evaluation contract と coherent-mass 比較を起動 | frozen task surface / readout / same-horizon / same-initial-mass / metric dominance / readout alignment の protocol 仮定を型として固定。実 benchmark 妥当性は証明しない |
| [`EpistemicBenchmarkResultCertificate.lean`](Survival/EpistemicBenchmarkResultCertificate.lean) | `result_certificate_implies_benchmark_valid`, `result_certificate_implies_net_action_no_worse`, `result_certificate_implies_controlled_mass_ge_baseline`: valid result certificate から benchmark protocol と comparison を起動 | external runner / result artifact が出す certificate 相当の witness を theorem 側に接続する有限 bridge。JSON parsing、実 benchmark validation、実性能は証明しない |
...
1. **ビルド整合性**: 179 Survival modules は `Survival.lean` の top-level import を通じて一貫して検証する設計である。今後は `PAPER_MAPPING.md` の freeze snapshot ごとに `lake build Survival` の通過状況を併記する。
...
4. `lake build Survival` の通過状況と、179 Survival modules の依存グラフを図示。棚卸しの完成度を検証。
