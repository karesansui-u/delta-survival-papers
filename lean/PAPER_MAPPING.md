...
- 残り 100 ファイル超のうち、相当部分は **論文本文に反映されていない数学的資産**。特に以下の 6 系統は論文主張を実質的に格上げできる候補:
  ...
  6. **Route A 非CSP core examples**（10 ファイル）— `SerialReliability` / `ConstantFractionDecay` / `BranchingProcessExtinction` / `QueueStability` / `BinarySymmetricChannel` / `FatigueDamage` / `ConsensusFaultThreshold` / `MemoryThrashing` / `BucklingThreshold` / `PercolationThreshold` により直列信頼性・一定割合減衰・分岐過程期待骨格・負荷超過崩壊・通信路信頼性・累積疲労損傷・分散故障閾値・メモリスラッシング・座屈閾値・パーコレーション閾値を形式化
...
1. **ビルド整合性**: 134 ファイルは `Survival.lean` の top-level import を通じて一貫して検証する設計である。今後は theorem map の freeze snapshot ごとに `lake build Survival` の通過状況を併記する。
