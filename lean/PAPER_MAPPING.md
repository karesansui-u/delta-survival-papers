...
現時点の Lean 側は **170 Survival modules** で閉じており、imported `Survival` target には
...
evidence-packet bridge、toy LLM control instantiation、toy software-contract instantiation、
toy software evidence-packet instantiation まで含む。
...
### G. 修復率・予算・動力学（12）— **論文 DSMF §11 の形式化**
...
1. **ビルド整合性**: 170 Survival modules は `Survival.lean` の top-level import を通じて一貫して検証する設計である。今後は `PAPER_MAPPING.md` の freeze snapshot ごとに `lake build Survival` の通過状況を併記する。
...
4. `lake build Survival` の通過状況と、170 Survival modules の依存グラフを図示。棚卸しの完成度を検証。
