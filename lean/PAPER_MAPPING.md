...
棚卸し日: 2026-04-21
対象: `lean/Survival/` 配下 113 ファイル
対応論文: `delta-survival-paper/v2/` 配下 8 本（Paper 0–4 + 補論 3 本）

## TL;DR

- 論文本文（Paper 2 §5）が形式検証対象として列挙しているのは **5 ファイル**（`AxiomsToExp` / `WeakDependence` / `RobustSurvival` / `TelescopingExp` / `LogUniqueness`）だが、`lean/Survival/` には実際には **113 ファイル**ある。
- 残り 108 ファイルのうち、相当部分は **論文本文に反映されていない数学的資産**。特に以下の 5 系統は論文主張を実質的に格上げできる候補:
...
- SAT/k-SAT の finite-horizon / iid Bernoulli bad-event chain は **SAT chain v1.0** として凍結可能な段階に達した。さらに固定割当 \(k\)-XOR-SAT exposure と固定 coloring の q-coloring edge exposure への水平展開も追加された。詳細な claim-to-theorem index は [`SAT_CHAIN_THEOREM_MAP.md`](SAT_CHAIN_THEOREM_MAP.md) に、横断的な Bernoulli-CSP template の対応表は [`BERNOULLI_CSP_UNIVERSALITY_MAP.md`](BERNOULLI_CSP_UNIVERSALITY_MAP.md) に分離した。
...
1. **ビルド整合性**: 113 ファイルは `Survival.lean` の top-level import を通じて一貫して検証する設計である。今後は theorem map の freeze snapshot ごとに `lake build Survival` の通過状況を併記する。
...
4. `lake build Survival` の通過状況と、113 ファイルの依存グラフを図示。棚卸しの完成度を検証。
