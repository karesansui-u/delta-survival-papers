...
現時点の Lean 側は **167 Survival modules** で閉じており、imported `Survival` target には
...
KL divergence / channel skeleton / BEC-style finite boundary、LLM-style epistemic control bridge と toy software-contract instantiation まで含む。
...
### G. 修復率・予算・動力学（9）— **論文 DSMF §11 の形式化**
...
| [`SoftwareContractToyRepository.lean`](Survival/SoftwareContractToyRepository.lean) | `toyRepository_composition_kernel`, `toyClaimAdmission_no_more_loss`, `toyDependencyRewrite_localizes` | software contract surface の最小 toy instantiation。実検出器の正しさではなく、repository contract state が bridge interface に乗ることを示す |
...
1. **ビルド整合性**: 167 Survival modules は `Survival.lean` の top-level import を通じて一貫して検証する設計である。今後は `PAPER_MAPPING.md` の freeze snapshot ごとに `lake build Survival` の通過状況を併記する。
...
4. `lake build Survival` の通過状況と、167 Survival modules の依存グラフを図示。棚卸しの完成度を検証。
