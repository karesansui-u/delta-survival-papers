...
| Epistemic / LLM control bridge | `EpistemicControlBridge`, `EvidencePacketBridge`, `SoftwareContractToyRepository`, `SoftwareEvidencePacketToy` | Abstract contradiction / repair control interface, coherent-mass net-action kernel wrapper, conditional memory-filter and dependency-rewrite guard lemmas, structured evidence-packet provenance / eligibility / witness / repair guardrails, a finite toy repository-contract instantiation with concrete toy mass values, and a toy software evidence-packet instantiation |
