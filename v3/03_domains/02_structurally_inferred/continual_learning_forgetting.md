...
この読みは、別紙 `llm_epistemic_control_bridge.md` の Lean bridge に対応する。
前提更新は contradiction update として coherent region を縮小しうる。
依存構造に沿った再提示や refresh は repair update として読むことができる。
ただし Lean が証明するのは、そうした抽象 operator が contraction /
repair interface と positivity 条件を満たすなら既存の net-action kernel
に乗る、という条件つき接続であり、LoRA や任意の継続学習手法の性能を
証明するものではない。
`../../../lean/Survival/LLMEpistemicControlToy.lean` では、この読みの有限
toy 版として、premise update の invalidation localization と downstream
repair coverage を evidence-packet bridge 上で確認している。
`../../../lean/Survival/DependencyClosureBudgetToy.lean` では、この局所化を
有限 cardinality budget として読み、invalidated surface 数が closure、
finite surface 全体、および repair touched set によって上界づけられることを
確認している。これは実依存グラフ発見の正しさではなく、sound closure を仮定した
場合の downstream invalidation の会計境界である。
`../../../lean/Survival/EpistemicControlComparison.lean` は、controlled 側の
cumulative net action が baseline 以下であるという追加仮定のもとで coherent
mass の baseline 以上保持を出すため、継続更新の比較主張を Lean 側では
「net action 比較仮定つき」に限定して読むことを明確にする。
`../../../lean/Survival/EpistemicControlEvaluationContract.lean` は、さらに
per-step loss / repair metric の優位性がこの net action 比較仮定を witness
することを示す。したがって実験側では、依存整合性や保持率そのものを theorem と
混同せず、どの readout が contradiction loss / repair gain として固定されるかを
別途明示する必要がある。
`../../../lean/Survival/EpistemicBenchmarkProtocol.lean` は、この明示を
benchmark protocol として固定するための層である。task surface、readout、
same horizon、same initial condition、metric dominance、readout alignment を
validity witness として要求するが、実験デザインがそれらを満たすこと自体は別途
示す必要がある。
...
outputs を result-certificate-shaped summary に変換する。model call や model
validation は行わない。
