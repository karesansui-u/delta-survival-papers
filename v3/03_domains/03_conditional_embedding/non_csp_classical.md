...
第三に、G4 v2 の empirical / operational pilot を設計する方向である。たとえば、maintenance log、incident count、repair schedule、remaining margin indicator を持つ実データで、\(d_t-r_t\) が failure / degradation を予測するかを検査できる。ただし、これは推定レイヤー / operational validation であり、本補論の algebraic skeleton とは別段階である。
