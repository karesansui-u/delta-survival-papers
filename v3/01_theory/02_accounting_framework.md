...
ここで誤読を避けるために強調しておく。事前固定が禁じているのは、写像 \((G,V,m)\) を後付けで構成することそのものではない。既存データを観察したうえで、もっともらしい \((G,V,m)\) を後から構成してよい。新しい座標や指標が、最初は既知の崩壊系列への事後的な当てはめとして得られることは、科学では珍しくない。禁じられるのは、その同一データへの当てはめを予測的支持と偽ることである。後付けで構成した写像であっても、それを凍結し、未使用データでドメイン基準モデルを超えれば、それは正当な予測支持になる。すなわち、事前固定が要求するのは「データを見る前に決めよ」ではなく、「支持を主張する前に凍結し、未使用データで検証せよ」である。当て続ける写像は、その由来が後付けであっても、この凍結検証をすでに通過している。
...
反証または非支持も事前に読める形で置く。たとえば、凍結された構造持続指標を加えても基準モデルに追加予測力を与えない場合、\(M\) 不足、\(V_G\) 縮小、\(V_{\mathrm{rec}}\) 喪失という事前分類が後続の復旧時間、外部介入の必要性、再設計の有無を区別しない場合、または別の自然な標準写像が同じ終点に対して一貫して優位になる場合、その写像と指標の組は非支持として扱う。これは会計理論全体の即時反証ではないが、当該ドメイン・当該時間地平・当該写像での支持主張を取り下げる十分な理由になる。
