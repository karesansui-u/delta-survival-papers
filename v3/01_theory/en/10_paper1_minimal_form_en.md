...
If persistence is described only by remaining resources, the question is underspecified. A system may still have resources while the set of states compatible with the target structure has already become too small.
...
Fix a system \(X\). Let \(V^{(0)}\) be the initial set of states compatible with maintaining the target structure. Under accumulating constraints, assume
\[
V^{(0)} \supseteq V^{(1)} \supseteq \cdots \supseteq V^{(n)}.
\]
...
This paper concerns pre-fixed structural maintenance problems. The target structure, initial feasible region, measure, stage sequence, and time horizon are fixed before observing the outcome.
...
6. Structural persistence quantity
...
Introduce an effective maintenance surplus \(M>0\) and define
\[
S=Me^{-L}.
\]
...
Structural failure is not only resource exhaustion. It can be shrinkage of the feasible region in which the target structure can still be maintained.
