...
For example, in finite BEC linear-code recovery, erasure-rank accounting is not a prediction feature; it is exact endpoint accounting for a fixed erasure pattern. The frozen prediction test based on low-order dependency pressure is a proxy-indicator support claim in the same specification-fixed domain. The rank correspondence to coding theory is an existing-theory connection attribute. The same domain can therefore appear in different rows depending on the claim package being discussed.
...
The bridge is extended by a finite baseline-comparison, evaluation-contract, benchmark-protocol, result-certificate, and software-evidence chain. The first named toy result artifact, `v3/05_evidence/llm_epistemic_control_frozen_toy_v0/llm_epistemic_control_frozen_toy_v0_result_001.json`, exercises that certificate loop on a deterministic toy packet. The real-eval candidate mapping, `v3/05_evidence/llm_epistemic_control_real_eval_candidate_mapping.md`, records which implementation-side logs could guide future frozen witnesses. The first frozen real-eval candidate surface is `v3/05_evidence/llm_epistemic_premise_update_v0/`, which fixes 12 premise-update / dependency-staleness tasks before outcome-bearing execution; its output-collection protocol and scorer / schema live in `v3/05_evidence/llm_epistemic_premise_update_v0/output_collection_protocol_v0.md` and `analysis/epistemic_control_premise_update_v0/`. These artifacts are protocol-shape / planning artifacts, not validation evidence for a real model or workflow.
