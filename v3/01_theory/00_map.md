...
本稿群は、単一の長大な論文ではなく、最小核、回復を含む形式への拡張、条件つき導出、写像手順、M（有効維持余力）の操作化、許容写像、仕様固定構造レイヤー、既存理論接続レイヤー、構造推定レイヤー、Lean 形式化を分けて書いている。本補論は、それらの役割、依存関係、主張強度を一つの地図として示す。
...
現時点で最も硬い外部再実行済みの経験的入口は、仕様固定構造レイヤーにある二つの凍結済み検証パッケージである。
...
| 主張強度の位置づけ | 仕様固定構造レイヤー / 既存理論接続レイヤー / 構造推定レイヤー、G6-a / G6-b / G6-c、支持状態の混同を避ける |
...
| 第5層: アンカー / 橋渡し | 仕様固定構造レイヤー、既存理論接続レイヤー、構造推定レイヤーのアンカー | CSP 補論、計算コスト補論、LLM 関連論文、Foster-Lyapunov 補論、非CSP補論 |
...
本理論は、同一の構造持続核を、固定・接続・推定の仕方が異なる三つのレイヤーで扱う。各レイヤーは異なる理論ではなく、同一の構造持続核を異なる観測・接続レベルで扱ったものである。これは対象ドメインの固定分類ではない。同じ LLM でも明示制約タスクなら仕様固定構造レイヤーに近づき、同じ物理・生物・ソフトウェア系でも観測・推定指標しか取れない場合は構造推定レイヤーに入る。これは「構造は常にある」という主張ではない。維持対象・測度・観測単位を固定できる場合に、その系を構造持続問題として扱う、という規律である。
...
| → 既存理論との接続 | 既存理論のドリフト、差分、停止境界を条件付きに写す |
| ↓ 観測可能性が低い | 構造そのものを直接数えず、観測・推定指標と凍結検証で推定する |

三つのレイヤーに共通する語彙は、維持可能領域 \(V\)、ものさし \(m\)、構造消耗量 \(d_t\)、回復量 \(r_t\)、累積消耗量 \(L\)、累積純消耗量 \(B\)、および必要に応じて外側に置かれる有効維持余力 \(M\) である。読者向けには
\[
  S=Me^{-L},
  \qquad
  S=Me^{-B},
...
\]
と書ける。ただし、仕様固定構造レイヤーで最も硬い定理側の核は、資源項 \(M\) を含む \(S\) 式全体ではなく、事前固定された \(V,m\) 上の集合値核である。違うのは式の見た目ではなく、その式に入る \(V,m,d_t,r_t\) をどの程度直接指定・観測できるか、また \(M\) をリソース側の読出しとしてどの程度明示できるかである。

| 外向け名 | 条件 | 現在の代表例 |
|---|---|---|
| 仕様固定構造レイヤー | 構造、測度、境界が仕様から事前固定される | SAT, Mixed-CSP, q-coloring, A12, A31, A06/A19, ベルヌーイ型 CSP インターフェース |
| 既存理論接続レイヤー（条件付き構造埋め込み） | 既存理論のドリフト、差分、停止境界を本理論の変数へ条件付きに写す。縦軸の中間段階ではなく、既存理論との横方向の橋渡しである | Foster-Lyapunov / 待ち行列ドリフト、有界近似候補 |
...
この配置で重要なのは、構造推定レイヤーの観測・推定指標を法則そのものと混同しないことである。推定指標が \(L\) または \(B\) の法則側座標をよく近似していることが凍結検証で確認されるほど、その指標は説明語彙から予測装置へ近づく。\(M\) はこの座標に混ぜ込まず、有効維持余力を表すリソース側のスカラーとして別に読む。ただし、その近似度は主観的に宣言されるものではなく、保留データ / 将来データ / 新規アーカイブ / 外部再実行における基準モデル + 構造持続指標の増分、または非支持 / 沈黙として判定される。
...
補論「構造持続における許容写像と階層的不変量」は、三つのレイヤーを数学的に読み直すための深部補論である。そこでは、構造維持問題を対象、その間の許容写像を射として扱い、同型では \(B_n\) が不変、正ゲージ変更では \(B_n\) が共変、粗視化では保存・単調性・誤差境界の条件が必要、観測指標による推定では凍結後の追加予測力で評価する、という階層を置く。

この補論の役割は、第二法則級の単一普遍法則を宣言することではない。むしろ、仕様固定構造レイヤー、既存理論接続レイヤー、構造推定レイヤーが、強弱のラベルではなく、どの写像で何が保たれるかの違いであることを明確にする。ベルヌーイ型 CSP インターフェースの自然性も、SAT 風の構文を後から集めたものではなく、独立同分布の悪事象曝露と対数ドリフトを保つテンプレート保存写像で閉じた限定クラスとして読む。
...
各補論には、その文脈で必要な限界と非主張を短く残す。ただし、三つのレイヤー、G6 分類、写像状態、支持判定、沈黙条件の完全な定義は、補論「構造持続理論の運用規律」と補論「構造持続写像の標準手順」に集約する。
