...
| A06/A19 消失ランク会計と行数境界 | 固定された消失数とランクのプロファイルから \(a(E)\log 2\) の厳密会計が出て、\(|E|>r\) なら一意復元不能になる。ランダム検査行列側では、失敗確率包絡と行数スラックから \(2^{-s}\) 境界が出る。消失数集中包絡が与えられた場合は、tail と階数失敗を合成できる | 低次依存圧の代理指標の経験的支持、最終ランクを使うオラクルによる予測勝利、容量達成定理、二項分布 Chernoff 境界そのもの |
