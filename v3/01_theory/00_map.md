...
| A06/A19 消失ランク会計と行数境界 | 固定された消失数とランクのプロファイルから \(a(E)\log 2\) の厳密会計が出て、\(|E|>r\) なら一意復元不能になる。ランダム検査行列側では、失敗確率包絡と行数スラックから \(2^{-s}\) 境界が出る | 低次依存圧の代理指標の経験的支持、最終ランクを使うオラクルによる予測勝利、容量達成定理、消失数集中 |
