...
| A06/A19 消失ランク会計と行数境界 | 固定された消失数とランクのプロファイルから \(a(E)\log 2\) の厳密会計が出て、\(|E|>r\) なら一意復元不能になる | 低次依存圧の代理指標の経験的支持、最終ランクを使うオラクルによる予測勝利、容量達成定理 |
