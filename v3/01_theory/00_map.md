...
現時点で最も硬い経験的入口は、仕様固定構造層にある二つの凍結済み empirical package である。

| package | outside rerun status | decision-relevant output |
|---|---|---|
...
これは universal-law closure ではない。しかし、仕様固定構造層において、\(L\) / first-moment 型の law-side 座標が raw baseline と比較され、その凍結済み package が著者環境外で同じ decision-relevant output を返したという意味で、local rerun や単なる手順公開より強い package-scoped replication support である。詳細は [`../05_evidence/outside_reruns.tsv`](../05_evidence/outside_reruns.tsv) と [`../../analysis/g7_route_a_true_outside_replication_summary.md`](../../analysis/g7_route_a_true_outside_replication_summary.md) に置く。
...
この配置で重要なのは、構造推定層の観測・推定指標を法則そのものと混同しないことである。推定指標が \(L/B/M\) の law-side 座標をよく近似していることが凍結検証で確認されるほど、その指標は説明語彙から予測装置へ近づく。ただし、その近似度は主観的に宣言されるものではなく、held-out / future / fresh archive / outside rerun における baseline + SP の増分、または no-support / silence として判定される。
