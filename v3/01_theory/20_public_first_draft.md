PublicFirst_構造持続会計理論_基礎と仕様固定アンカー
構造持続会計理論: 基礎と仕様固定アンカー
— 維持可能領域、対数比、回復込み収支、有限操作的アンカー —

要旨

構造は、資源が残っていても保てなくなりうる。本稿では、その失敗を、資源側の有効維持余力 \(M\) と、維持条件 \(G\) を担う系として持続できる領域の縮小 \(L/B\) に分けて読む枠組みを、構造持続会計（structural persistence accounting）と呼ぶ。
...
生存可能性理論（viability theory）との関係でいえば、本稿は生存可能核や到達可能集合を新しく計算する理論ではない。既存理論や対象ドメインで固定された制約集合、核、到達可能集合を受け取り、それらの縮退・回復・資源側余力を同じ会計上で読み分けるための構造持続会計である。英語では主に structural persistence accounting と呼び、生存可能性理論との接続を強調する場合には structural viability accounting とも表現できる。

ここでいう会計は、財務会計に限られない。何が残り、何が削られ、どの回復経路が残り、どの資源側余力と組み合わせて読めるかを、観測前に固定した台帳で記録するという意味での会計である。したがって本稿の主名を、以後、構造持続会計理論と呼ぶ。
