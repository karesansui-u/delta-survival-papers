...
\[
  m(V^{(n)})=m(V^{(0)})e^{-B_n}
\]
が成り立つ。資源側の有効維持余力を明示すれば、読者向けの構造持続ポテンシャルは \(S=Me^{-L}\)、または回復込みで \(S=Me^{-B}\) と書ける。
...
達成側についても、ランダム検査行列の失敗確率包絡と、消失数集中包絡が外部から与えられれば、容量境界に似た有限包絡を得られる。たとえば、行数スラック \(s\) があり、全列ランク失敗確率が \(2^{-s}\) 以下に押さえられ、消失数の尾部確率が \(\delta\) 以下なら、
\[
