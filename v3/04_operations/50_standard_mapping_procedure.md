...
本手順は、対象構造、維持条件、有効選択肢空間、制約集合、累積構造消耗 L、余力 μ、初期有効選択肢多様性 N_eff^(0) を順に定義し、仕様固定構造層、条件付き構造埋め込み層、構造推定層のいずれかへ位置づける。さらに、反証条件の事前固定、基準モデルとの比較、予測頑健性テスト、介入設計までを一つの流れとして与える。これにより、ドメインごとに異なる語で語られてきた崩壊や劣化を、共通の骨格のもとで比較・検証するための標準手順を与える。
