...
本手順は、対象となる構造条件、維持条件、有効選択肢空間、制約集合、累積構造消耗 L、余力 μ、初期有効選択肢多様性 N_eff^(0) を順に定義し、仕様固定構造層、条件付き構造埋め込み層、構造推定層のいずれかへ位置づける。さらに、反証条件の事前固定、基準モデルとの比較、予測頑健性テスト、介入設計までを一つの流れとして与える。これにより、ドメインごとに異なる語で語られてきた崩壊や劣化を、共通の骨格のもとで比較・検証するための標準手順を与える。
...
- S_k: 構造持続ポテンシャル。
    S_k = M_k R_k
  で与えられる。
- P_k: 条件付き構造埋め込み層で用いる、確率的または弱依存の記述のもとでの実現残存可能性。役割としては仕様固定構造層の R_k に対応するが、参照モデルに対する境界として表すときにこの記号を用いる。
...
ただし、現時点で直接観測できない内部機構があること自体は、直ちに本手順の対象外を意味しない。重要なのは、その内部機構を仮定しなくても、維持条件、崩壊条件、または設計変更後に観測すべき readout のいずれかが操作的に記述できるかどうかである。したがって、「まだ完全には見えていないが、観測・推定指標と設計仮説を通じて検証可能な構造」は条件付き構造埋め込み層または構造推定層の候補になりうるが、「見えない本質」そのものを対象となる構造条件に据えることは許されない。
