A12 S-t Cut-Spectrum Reliability Freeze Manifest v0c Kappa-3
============================================================
...
manifest_id: st_cut_spectrum_reliability_v0c_kappa3
...
frozen \(n=16,m=24,\kappa=3\) generation cell was infeasible under the original
random two-cluster generator and enumeration cap. The v0b successor established
support on a \(\kappa=2\)-only surface. This v0c package is a separate
\(\kappa=3\) successor surface, not a rescue or reinterpretation of either run.
...
> \(\kappa=3\) under a pre-fixed independent edge-failure law,
> \(B1+\log(1+H_{\mathrm{cut},2})\) predicts held-out \(s\)-\(t\)
> disconnection probability better than \(B1\) alone.

Claim strength if passed:

- finite synthetic A12-v0c support for scalar low-order cut-spectrum pressure;
- \(\kappa=3\)-only support in this frozen two-cluster graph family;
- independent replication of the A12 coordinate beyond the \(\kappa=2\)-only
  v0b surface;
...
```text
104c7f9694d906d135abf49540811610f8b93e8f661d668811da64d93eec4dff
```
...
The constructive cluster generator first builds clusters with edge-connectivity
at least \(\kappa\), then adds random surplus edges to reach the frozen edge
count. The two clusters are connected by exactly \(\kappa\) cross edges, and
the generator verifies the final \(s\)-\(t\) edge connectivity.
...
```text
05_evidence/a12_st_cut_spectrum_reliability/primary_v0c_kappa3
```
...
python3 -B 05_evidence/a12_st_cut_spectrum_reliability/scripts/generate_smoke.py \
  --output-dir 05_evidence/a12_st_cut_spectrum_reliability/primary_v0c_kappa3 \
  --n-values 16,20 \
  --edge-factors 2.0 \
  --kappas 3 \
  --q-values 0.20,0.30,0.40,0.50 \
...
python3 -B 05_evidence/a12_st_cut_spectrum_reliability/scripts/evaluate_smoke.py \
  --input-dir 05_evidence/a12_st_cut_spectrum_reliability/primary_v0c_kappa3 \
  --output-dir 05_evidence/a12_st_cut_spectrum_reliability/primary_v0c_kappa3 \
...
- support for the original infeasible v0 surface;
