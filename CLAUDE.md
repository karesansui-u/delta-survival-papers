...
- 3_構造持続と推論の崩壊.md — LLM推論時の適用 + 実験（30ターンON/OFF/NC, 100ターン長期）
- 4_構造持続と学習の崩壊.md — LLM学習時の適用 + LoRA実験 + 三層統合（§7.5、旧Paper 5を統合）

### 補足資料（2本）
- 参考_計算コストの構造的予測.md — SAT Route A。Lが直接計算できるドメインでの最硬の土台
- ドメイン汎用フレーム仕様書.md — DSMF。任意ドメインへの適用手順。Route A/B/C、反証条件、基準モデル比較、予測頑健性テスト（手順10.5）
