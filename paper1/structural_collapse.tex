...
\begin{enumerate}
    ...
    \item \textbf{Formal verification scope}: Lean~4 verification covers mathematical properties (algebraic consistency, theorem validity, the derivation chain from axioms A1--A3 to $e^{-\delta}$, the Paley-Zygmund second moment inequality, the pair correlation structure, and the weak dependence sandwich with robust survival bounds) across 19 modules and 286 propositions, not empirical correctness. The weak dependence extension (\S\ref{sec:weak-dependence}) reduces the gap between formal verification and empirical applicability: rather than requiring axioms to hold exactly, it provides guaranteed bounds parameterized by the measurable correlation $\rho$. However, a formally verified equation can still be empirically wrong if $\rho$ is underestimated.
