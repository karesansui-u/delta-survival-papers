...
\midrule
32K  & 10/30 & 0.67 & 0.62 & 16 \\
128K &  5/30 & 0.83 & 0.86 & 26 \\
256K &  2/30 & 0.93 & 0.93 & 18 \\
512K &  1/30 & 0.97 & 0.96 & 17 \\
\bottomrule
\end{tabular}
\end{center}

\noindent The 512K data point is a genuine out-of-sample prediction: the model $P = 1/(1 + K/L)$ with $K \approx 20$ was fixed before the experiment. The monotonic decrease in \texttt{wrong\_val} adoptions ($10 \to 5 \to 2 \to 1$) across four context lengths spanning a 16$\times$ range is consistent with context length serving as the margin variable ($\mu \propto L$) in the multi-attractor framework. The fitted $K$ at 512K ($K = 17$) remains within the range observed at shorter lengths ($K = 16$--$26$), supporting the constant-$K$ assumption.
