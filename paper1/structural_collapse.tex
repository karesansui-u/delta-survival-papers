...
\begin{abstract}
The multi-attractor extension of the survival equation $S = N_{eff} \cdot (\mu/\mu_c) \cdot e^{-\delta}$---where each attractor basin has its own survival potential and $-\ln S$ plays the role of free energy---yields a transition point formula $m^* = \ln(C_A/C_B)/(I_A - I_B)$ and predicts that Gemini's contradiction susceptibility follows $P(\text{correct}) = 1/(1 + K/L)$, where $K = m^*$ and $L$ is context length. This prediction was derived from three data points; confirmation at five context lengths (32K--512K tokens) finds $K = 19.5 \pm 3.4$ (thousands of tokens), mean absolute error 2\%, with two out-of-sample predictions within 1\%. The full chain---axioms $\to$ Lean~4 proof $\to$ quantitative prediction $\to$ experimental confirmation---is completed for this case, constituting the first quantitative test of the multi-attractor framework.

These results are grounded in a broader framework. Large language models exhibit abrupt reasoning collapse when structural contradictions accumulate in their context---a phenomenon distinct from gradual degradation under factual noise. Through controlled experiments (11 models, 5 vendors, 4B--70B+ parameters), we find: (i) multiplicative interaction between context margin ($\mu$) and structural contradiction ($\delta$)---two individually non-lethal stressors become lethal in combination, ruling out additive degradation models; (ii) a dimensional structure in $\delta$ where a single structural contradiction causes more damage than 10 factual contradictions; and (iii) a cushion mechanism where RLHF alignment absorbs contradictions until overwhelmed, producing a system-specific critical threshold $\delta_c$. Defining cumulative information loss $\delta = \sum_i I(\text{constraint}_i)$ in nats, the multiplicative survival potential captures these patterns and, in Boolean satisfiability (SAT) where $\delta$ is exactly computable, yields the parameter-free prediction $\alpha_{\text{XOR}}/\alpha_{\text{random}} = \ln 2 / |\ln(7/8)| = 5.19\times$, confirmed at $5.04 \pm 0.25$ (CV $= 5\%$). A $\delta = 0$ control experiment (6 models, 4 vendors, 500--128{,}000 tokens) confirms the complementary prediction: under zero structural contradiction, capable models maintain perfect accuracy regardless of context length, while a low-$\mu$ model (Llama~3.1:8b) saturates at 36\%, confirming the multiplicative structure. A three-model extended experiment ($n = 810$ trials, 32K--256K tokens) reveals a capability--vulnerability paradox: the weakest model is immune to subtle inter-source contradiction while the stronger model is catastrophically vulnerable ($37\% \to 7\%$ accuracy), and Gemini exhibits a third pattern of context-length dilution---the pattern the multi-attractor framework explains.

The exponential penalty $e^{-\delta}$ is a mathematical identity with the first moment of the SAT solution count, not an approximation. Extension to the second moment method (Paley--Zygmund inequality) quantitatively explains 74\% of the gap between the first-moment upper bound and the true 3-SAT threshold; the remaining 26\% is attributable to replica symmetry breaking. All results---including the second moment overlap decomposition, multi-attractor transition theorems, weak dependence bounds ($e^{-\delta(1+\rho)} \leq P \leq e^{-\delta(1-\rho)}$), and Cauchy uniqueness of the exponential form---are machine-verified in Lean~4 (20 modules, no \texttt{sorry}, no \texttt{axiom}).

\medskip
\noindent\textbf{Keywords:} LLM reasoning collapse, structural contradiction, information loss, multiplicative model, SAT phase transition, survival analysis, multi-attractor transition, formal verification
\end{abstract}
...
The survival equation's $e^{-\delta}$ is therefore \emph{identical} to the first moment of the SAT solution count, normalized by the unconstrained solution space $2^n$. This is not an analogy; it is a mathematical identity. In other domains (LLM, ecology), the same functional form is in general applied by structural analogy: $\delta$ is operationally defined via domain-specific proxies, and the exponential form is a testable working hypothesis. For the specific case of LLM tasks with explicitly named variables, however, the axioms A1--A3 admit a formal instantiation via consistent world models (\S\ref{sec:formal-instantiation}), in which $\delta$ is defined from the constraint structure---not inferred from accuracy---establishing $e^{-\delta}$ as a theorem for that task class. The first moment method \citep{alon2016} guarantees: when $\mathbb{E}[\#\text{SAT}] < 1$, the instance is almost surely unsatisfiable. Translating: when $e^{-\delta} < 2^{-n}$, the system has no viable configurations.
...
This is a restatement of Price's equation \citep{price1970} applied to $\delta$-dependent hazard; Fisher's Fundamental Theorem \citep{fisher1930} and Boltzmann's H-theorem are special cases. The survival equation's $h(\delta)$ increasing provides the precondition automatically. The theorem, together with the algebraic properties of the survival equation ($S > 0 \Leftrightarrow$ all factors positive; hazard monotonicity; error propagation), the axiom-to-exponential derivation chain (\S\ref{sec:first-moment}), and the weak dependence extension (\S\ref{sec:weak-dependence}), is formally verified in Lean~4 \citep{demoura2021} with Mathlib (20 modules, $\texttt{sorry} = 0$, $\texttt{axiom} = 0$).\footnote{In Lean~4, \texttt{sorry} is a placeholder admitting any proposition without proof; \texttt{axiom} declares an unproven assumption. $\texttt{sorry} = 0$ and $\texttt{axiom} = 0$ mean every step is machine-checked with no gaps or unverified assumptions.} The verification covers mathematical properties, not empirical validity. In addition to the single-basin properties, the formalization includes a multi-attractor extension (4 modules: \texttt{MultiAttractor}, \texttt{TransitionTheorem}, \texttt{FreeEnergy}, \texttt{ScaleInvariance}) proving that: (i) each attractor basin independently satisfies the survival equation, bridging to the single-basin case via \texttt{basinSurvival\_eq\_fullSurvival}; (ii) two basins with different per-constraint information losses $I_A \neq I_B$ have a unique transition point $m^* = \ln(C_A/C_B)/(I_A - I_B)$ where $S_A(m^*) = S_B(m^*)$; (iii) $S$-maximization is equivalent to free-energy minimization ($-\ln S = F$); and (iv) the survival equation is self-similar across hierarchical levels (scale invariance).

\subsection{Robustness Under Weak Dependence}
\label{sec:weak-dependence}

The derivation of $e^{-\delta}$ relies on Axiom~A3 (independence of constraints). In SAT, random clause generation ensures exact independence; in LLM conversations, successive contradictions may be correlated. A natural question is: how robust is the exponential form when A3 holds only approximately?

We formalize this by introducing a \emph{correlation parameter} $\rho \geq 0$ that measures the maximum pairwise dependence between constraints. For a system with joint survival probability $P$ under cumulative information loss $\delta$, we define the \emph{weak dependence sandwich}:
\begin{equation}
e^{-\delta(1+\rho)} \leq P \leq e^{-\delta(1-\rho)}
\label{eq:sandwich}
\end{equation}
where the lower bound is the \emph{conservative} estimate (positive correlation amplifies loss) and the upper bound is \emph{optimistic} (negative correlation dampens loss). Three properties are verified in Lean~4:

\begin{enumerate}
\item \textbf{Reduction to independence}: At $\rho = 0$, both bounds collapse to $e^{-\delta}$, recovering the exact result from A1--A3. The independent-case survival $\prod_i e^{-\delta_i} = e^{-\delta}$ satisfies the sandwich at $\rho = 0$.

\item \textbf{Monotonicity}: As $\rho$ increases, the conservative bound weakens (decreases) and the optimistic bound strengthens (increases), widening the interval. The independent value $e^{-\delta}$ always lies within the sandwich for $\rho \geq 0$.

\item \textbf{Bounded error}: The robust survival potential $S_{\text{robust}} = \mu \cdot e^{-\delta(1+\rho)}$ satisfies $S_{\text{robust}} \leq S_{\text{indep}} = \mu \cdot e^{-\delta}$. The multiplicative error is $e^{-\delta\rho}$, which vanishes as $\rho \to 0$.
\end{enumerate}

For the second moment method, we additionally prove that the mesh-based second moment sum admits upper and lower bounds using only the pair correlation kernel's range $[g_{\min}, g_{\max}]$, without requiring independence between clauses. Specifically, for $n$ variables and $m$ clauses with pair kernel $g: [0,1] \to [\tfrac{3}{4}, \tfrac{7}{8}]$:
\[
2^n \cdot (3/4)^m \leq \sum_{d=0}^{n} \binom{n}{d} \cdot g(d/n)^m \leq 2^n \cdot (7/8)^m
\]
This means the Paley-Zygmund lower bound on $\Pr[\#\text{SAT} > 0]$ can be applied without clause independence.

The practical consequence is that the independence assumption (A3) transforms from a binary requirement (satisfied or not) into a \emph{measurable parameter} ($\rho$): given an empirical estimate of constraint correlation, the framework provides a guaranteed conservative bound on survival. This is analogous to Shannon's channel capacity theorem: the theorem holds for any channel satisfying the model; whether a specific channel satisfies it is an empirical question, but the bound remains valid to the degree that $\rho$ is correctly estimated.

The full formalization comprises three Lean~4 modules: \texttt{WeakDependence} (sandwich definition, reduction, monotonicity), \texttt{CorrelatedSecondMoment} (mesh bounds without independence), and \texttt{RobustSurvival} (robust potential, error bounds, LLM bridge via $\delta$ interval estimation from per-constraint loss ranges).
...
\noindent The off-by-$n$ pattern ($+1, +2, +3$) occurs \emph{exclusively} in the reassignment family, consistent with Gemini treating ``$a = a + 1$'' as an imperative update. The diff distribution $\{+1\!:\!15,\; +2\!:\!10,\; +3\!:\!2,\; +4\!:\!1\}$ is consistent with cumulative reassignment: at 30\% injection density, some variables receive multiple $+1$ operations. Paradox and nullify templates produce no off-by-$n$ errors, yielding near-perfect accuracy---but this is \emph{not} because Gemini cannot parse them. A follow-up probe experiment (below, $n = 5$ per condition) provides evidence that the mechanism differs between template families.
...
\noindent The results are consistent with two independent mechanisms operating simultaneously (though the small sample size of $n = 5$ per condition warrants caution):
...
\subsubsection{Multi-Attractor Interpretation of Exp.~36}
\label{sec:exp36-ma}

The three qualitatively distinct response patterns in Exp.~36 under subtle contradiction ($\delta_5$) can be interpreted as different attractor basin structures, where basin~A is ``output the correct answer'' and basin~B is ``adopt the contradictory value.'' The multi-attractor extension (\S\ref{sec:h-theorem}) predicts $P(\text{correct}) = S_A / (S_A + S_B)$; if the context-length margin $\mu$ is proportional to $L$, this yields $P(\text{correct}) = 1/(1 + K/L)$ for a constant $K$.

\paragraph{Nano (no transition).} \texttt{wrong\_val\_adopted} $= 0/90$ across all context lengths. Interpretation: the model cannot parse $\delta_5$ contradictions, so $I_A \approx 0$ and $S_A \gg S_B$ at all $L$. The system never approaches the transition point.

\paragraph{Mini (length-independent vulnerability).} \texttt{wrong\_val\_adopted}: 33\% (32K), 37\% (128K), 33\% (256K)---no significant $L$-dependence. Interpretation: context length acts equally on both basins ($S_A$ and $S_B$ share the same $L$-dependence), so $S_B/S_A$ is a constant independent of $L$. The contradiction is neither diluted nor amplified by longer context.

\paragraph{Gemini (information dilution).} \texttt{wrong\_val\_adopted}: 33\% (32K), 17\% (128K), 7\% (256K)---monotonically decreasing. The model $P(\text{correct}) = 1 - P(\text{wrong\_val})$ at each context length is $0.67$, $0.83$, $0.93$. Fitting $P = 1/(1 + K/L)$ yields $K \approx 16$, $26$, $18$ respectively ($K \approx 20 \pm 5$, same order of magnitude given $n = 30$ per condition). This is consistent with context length serving as the margin ($\mu \propto L$) for the correct-answer basin only: longer contexts dilute the contradictory information, increasing $S_A$ relative to $S_B$.

\paragraph{Pre-registered prediction and confirmation.} The $P = 1/(1 + K/L)$ model with $K \approx 20$ (in units of thousands of tokens) was derived from Exp.~36 data at $L = 32\text{K}$, $128\text{K}$, $256\text{K}$ and used to generate a prediction \emph{before} running the 512K experiment: $P(512\text{K}) \approx 0.96$. The subsequent experiment ($n = 30$, identical protocol) yielded $P = 29/30 = 0.97$, within 1\% of the prediction (binomial 95\% CI: $[0.82, 1.00]$). The $\delta_0$ control confirmed $30/30$ correct at 512K. The full trajectory:

\begin{center}
\begin{tabular}{lcccc}
\toprule
Context & wrong\_val adopted & $P(\text{correct})$ & Predicted $P$ & $K$ (fitted) \\
\midrule
...
\noindent The 64K and 512K data points are genuine out-of-sample predictions: the model $P = 1/(1 + K/L)$ with $K \approx 20$ was derived from three points (32K, 128K, 256K) before the confirmation experiments. Both predictions matched observations within 1\%. Across five context lengths spanning a $16\times$ range, the mean absolute error is 2\%---well below the binomial noise floor ($\sigma \approx 8\%$ for $n = 30$). The monotonic decrease in \texttt{wrong\_val} adoptions ($10 \to 6 \to 5 \to 2 \to 1$) is consistent with context length serving as the margin variable ($\mu \propto L$) in the multi-attractor framework. The fitted $K$ across all five points is $19.5 \pm 3.4$, supporting the constant-$K$ assumption. The physical interpretation of $K \approx 20$ (in units of thousands of tokens) is the transition point $m^*$ from the multi-attractor theorem: the context length at which the correct-answer basin and the contradictory-value basin have equal survival potential ($S_A = S_B$).
...
An important epistemological distinction applies. In SAT, the correspondence between $e^{-\delta}$ and the first moment is a \emph{mathematical identity}---a variable substitution within the same formalism. In LLM and ecological domains, the correspondence is in general a \emph{structural analogy}: the multiplicative form is empirically tested, not derived from domain-specific first principles. For LLM tasks with explicitly named variables, however, the axioms A1--A3 admit a formal instantiation via consistent world models (\S\ref{sec:formal-instantiation}), establishing $e^{-\delta}$ as a theorem for that task class. Furthermore, ``survival'' ($S > 0$) denotes different phenomena across domains---satisfiability (existence of solutions), task accuracy (functional performance), bleaching resistance (ecological persistence)---unified not by ontological identity but by shared mathematical structure under accumulating constraints.
...
\textbf{Formal instantiation for variable-assignment tasks.}\label{sec:formal-instantiation}
The ``structural analogy'' characterization applies to LLM tasks in general. For the specific task class used in the experiments of this paper---where the context explicitly specifies a finite set of named variables and the query requires computing a function of those variables---the axioms A1--A3 admit a \emph{formal instantiation} that closes the analogy gap.

Define the \emph{effective state space} $\Omega$ as the set of \emph{consistent world models}: complete assignments of values to all named variables that are logically consistent with the context. Then:
\begin{itemize}
  \item \textbf{A1 (Finite state space)}: $|\Omega|$ is finite, bounded by the product of each variable's value domain.
  \item \textbf{A2 (Fractional elimination)}: A contradiction targeting variable $v$ eliminates all world models where $v$ takes the contradicted value---a fraction $p_v$ determined by the variable's domain, independently of the context's other content.
  \item \textbf{A3 (Independence)}: Contradictions targeting distinct variables $v \neq w$ act on disjoint coordinates of $\Omega$ and are therefore independent: $\Pr[\text{eliminated by } C_v \text{ and } C_w] = \Pr[\text{eliminated by } C_v]\cdot\Pr[\text{eliminated by } C_w]$.
\end{itemize}
Under this instantiation, \texttt{AxiomsToExp.lean} applies without modification: $e^{-\delta}$ holds as a \emph{theorem} for variable-assignment tasks, not as a structural analogy. Crucially, $\delta$ is defined from the constraint structure (the contradiction's target variable and value), not inferred from accuracy measurements---reproducing the SAT situation where $\delta$ is an input, not an output. For tasks without explicit variable separation---open-ended dialogue, multi-hop reasoning over implicit knowledge---the analogy characterization and weak dependence bounds (\S\ref{sec:weak-dependence}) remain the applicable guarantees.
...
LLM (variable tasks) & world-model instantiation & 0 & theorem (\S\ref{sec:formal-instantiation}) \\
LLM (general tasks) & operational (constraint injection) & $\geq 2$ & structural analogy \\
\bottomrule
\end{tabular}
...
The full screening hierarchy is therefore: (0)~Has optimization failed to keep pace with environmental change? (1)~Can constraints be defined independently of the state space? (2)~Do natural module boundaries exist? (3)~To what degree does A3 hold? Steps 0--1 determine whether the framework is applicable at all; steps 2--3 determine the precision of the resulting predictions. The weak dependence extension (\S\ref{sec:weak-dependence}) makes step~3 quantitative: given an empirical estimate of constraint correlation $\rho$, the conservative survival bound $\mu \cdot e^{-\delta(1+\rho)}$ holds regardless of the specific dependence structure. A3 is no longer a binary gate but a continuous parameter that modulates precision.
...
\textbf{Multi-attractor extension: transitions between states.} The single-basin survival equation describes collapse ($S \to 0$). Many real systems do not simply collapse but \emph{transition} to a qualitatively different state---laminar flow becomes turbulent, a failing organization restructures. The multi-attractor extension assigns each attractor basin $j$ its own survival potential $S_j = C_j \cdot e^{-\delta_j}$, where $C_j = N_{eff,j} \cdot (\mu_j/\mu_{c,j})$ and $\delta_j = \sum_i I_j(c_i)$ is the basin-specific cumulative information loss. The same physical constraint $c_i$ may have different information loss in different basins ($I_A(c_i) \neq I_B(c_i)$)---structurally identical to the SAT result where random 3-clauses ($I = \ln(8/7)$) and XOR pairs ($I = \ln 2$) have different per-constraint costs. Under the uniform constraint assumption ($\delta_j = m \cdot I_j$), setting $S_A = S_B$ yields a closed-form transition point:
\[
m^* = \frac{\ln(C_A / C_B)}{I_A - I_B}
\]
which is a parameter-free prediction analogous to the SAT ratio $\alpha_{\text{XOR}}/\alpha_{\text{random}} = 5.19\times$. Defining $F_j = -\ln S_j = -\ln C_j + \delta_j$ as a free-energy analogue, $S$-maximization is equivalent to $F$-minimization, and $F(\delta)$ is linear in $\delta$, implying that all transitions in this framework are first-order (discontinuous). Second-order transitions require $I_j$ to depend on constraint count---a generalization left open. The multi-attractor extension, including basin dominance before and after the transition point, scale invariance across hierarchical levels, and the $S$-max $\Leftrightarrow$ $F$-min equivalence, is formally verified in Lean~4 (4 modules, $\texttt{sorry} = 0$). Empirical validation---predicting the critical Reynolds number of channel flow ($\text{Re}_c = 5772$) from the information-loss structure---requires a domain-specific mapping $\delta(\text{Re})$ and remains open; the framework predicts \emph{that} a transition occurs and \emph{where} (in $\delta$-space), but not \emph{at what value of the domain-specific control parameter}.

The Exp.~36 data provide an initial test within an existing Level~A domain (\S\ref{sec:exp36-ma}).
...
\begin{enumerate}
    ...
    \item \textbf{Formal verification scope}: Lean~4 verification covers mathematical properties (algebraic consistency, theorem validity, the derivation chain from axioms A1--A3 to $e^{-\delta}$, the Paley-Zygmund second moment inequality, the pair correlation structure, weak dependence sandwich with robust survival bounds, and multi-attractor transition theorems including scale invariance) across 20 modules, not empirical correctness. The multi-attractor transition point $m^* = \ln(C_A/C_B)/(I_A - I_B)$ is proven unique, but whether a given physical system satisfies the uniform constraint assumption ($\delta_j = m \cdot I_j$) is an empirical question---the same epistemic structure as A1--A3. The free-energy linearity ($F(\delta) = -\ln C + \delta$) restricts the framework to first-order transitions; second-order (continuous) transitions require $I_j$ to depend on constraint count, which is left as an explicit scope boundary. However, a formally verified equation can still be empirically wrong if $\rho$ is underestimated.
...
The survival equation $S = N_{eff} \cdot (\mu/\mu_c) \cdot e^{-\delta}$ generates quantitative predictions that can be experimentally confirmed. The multi-attractor extension yields $P(\text{correct}) = 1/(1 + K/L)$ with $K = 19.5 \pm 3.4$ thousand tokens, confirmed at five context lengths (32K--512K, mean absolute error 2\%), derived from a prediction using only three data points. The full chain---axioms $\to$ Lean~4 proof $\to$ quantitative prediction $\to$ experimental confirmation---is complete for this case. The framework also explains LLM reasoning collapse: multiplicative $\mu \times \delta$ interaction, dimensional structure in $\delta$, and a cushion mechanism with cross-model convergence, explained by the first moment method from combinatorics. The parameter-free SAT prediction $\alpha_{\text{XOR}}/\alpha_{\text{random}} = 5.19\times$ is confirmed at $5.04 \pm 0.25$ (CV $= 5\%$).
...
The framework introduces no novel mathematical results; its contribution is an operational methodology: measuring structural conflict as information loss in nats, grounded in the first moment correspondence. The weak dependence extension addresses the principal theoretical concern---that real-world constraints are not independent---by providing guaranteed survival bounds parameterized by the measurable correlation $\rho$, with exact reduction to $e^{-\delta}$ at $\rho = 0$. The multi-attractor extension provides a formally verified transition theory: defining $F = -\ln S$ as a free-energy analogue, transitions between attractor basins occur at $m^* = \ln(C_A/C_B)/(I_A - I_B)$, and $P = 1/(1 + K/L)$ with $K = m^*$ matches Gemini's context-length dilution across five data points. The full chain---axioms $\to$ Lean~4 proof $\to$ quantitative prediction $\to$ experimental confirmation---is completed for this case. The two domains are complementary---SAT validates the mathematical identity, LLM validates both the multiplicative interaction and the multi-attractor transition structure, revealing that constraint quality, not quantity, governs collapse. The challenge ahead is extending this methodology to Level~B domains where $\delta$ requires proxy measurement.
