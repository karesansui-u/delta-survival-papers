...
Shannon's channel coding theorem \citep{shannon1948,cover2006} establishes that reliable communication requires redundancy exceeding the channel's noise rate. The first moment method in combinatorics \citep{alon2016} uses $\mathbb{E}[X] = 0 \implies \Pr[X > 0] = 0$ to prove non-existence. Our operational definition $\delta = \sum_i I(\text{constraint}_i)$ connects these: each constraint removes information (solutions, options, capacity), and $e^{-\delta}$ is the expected fraction remaining---exactly the first moment bound. Independently, Giannakopoulos \citep{giannakopoulos2025} proposed an exponential survival equation $S(\eta) = \exp[-\alpha \cdot (1-\eta) \cdot Q/T]$ from a different starting point (preprint, not yet peer-reviewed).
...
A \emph{closed} implication chain of length $L$ ($x_1 \to x_2 \to \cdots \to x_L \to \neg x_1$) forces $x_1 = \mathtt{False}$ (since $x_1 = \mathtt{True}$ propagates through the chain to $\neg x_1 = \mathtt{True}$, a contradiction). The remaining $L-1$ variables form an open chain admitting $L$ ``staircase'' patterns (once a variable switches from $\mathtt{False}$ to $\mathtt{True}$, all subsequent variables must be $\mathtt{True}$). Therefore:
\begin{equation}
...
\subsection{Exp.~2: Exponential Decay under \texorpdfstring{$\delta$}{δ} Increase}

Model: GPT-4o-mini, 30 trials per condition. Arithmetic chain reasoning with controlled structural complexity. Chain length $L$ increases $\delta$.

\begin{table}[htbp]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{$L$ (chain length)} & \textbf{Accuracy} & \textbf{CV} & \textbf{Phase} \\
\midrule
2 & 1.000 & 0.000 & Supercritical \\
3 & 0.967 & 0.186 & Fluctuation onset \\
\textbf{5} & \textbf{0.300} & \textbf{1.528} & \textbf{Critical} \\
7--50 & 0.000 & --- & Complete collapse \\
\bottomrule
\end{tabular}
\caption{Exp.~2: Exponential decay pattern. $\log(\text{accuracy})$ vs.\ $L$ yields slope $= -0.645$, consistent with exponential decay. Only three non-zero data points ($L = 2, 3, 5$) with two fit parameters leave one degree of freedom, so no meaningful goodness-of-fit statistic can be computed. The pattern is illustrative, not a statistical confirmation of the exponential form; the SAT experiments (Section~\ref{sec:sat}) provide the rigorous test.}
\label{tab:exp2}
\end{table}

\subsection{Exp.~3: Cross-Model Collapse Pattern}

Fixed depth ($\sim$5 steps), $\delta$ varied across 4 levels. 2 vendors, 2 models.
...
\label{tab:exp5}
\end{table}
\footnotetext{Data available in the repository.}
...
\subsection{Extended Experiments: The Cushion Mechanism (Exp.~14--19')}

Experiments 14--19' extended the paradigm to a Double Bind scenario: two authority figures issue contradictory directives. The system prompt, field report, and option structure are held constant across all $\delta$ levels ($\mu$ fixed); only the contradiction intensity of CEO/CSO directives ($\delta$) varies. Key results:

\textbf{Three-condition comparison} (Claude Sonnet 4.6, $n = 20$ per $\delta$ per condition):
...
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & \textbf{Structural contradiction} & \textbf{Directive pressure}$^*$ & \textbf{$\delta = 0.62$} & \textbf{$\delta = 1.00$} \\
\midrule
Exp.~18 & Yes & Yes & 0\% & 0\% \\
Exp.~19 & No & No & 100\% & 0\% \\
Exp.~19' & Yes & No & 100\% & 100\% \\
\bottomrule
\end{tabular}
\caption{Three-condition comparison. Structural contradiction alone does \emph{not} cause collapse (Exp.~19': 100\% survival at $\delta = 1.00$; Clopper--Pearson 95\% CI $[83.2\%, 100\%]$ for $n=20$). Collapse requires both contradiction \emph{and} sufficient pressure to overcome the RLHF cushion. $^*$Directive pressure: emotional enforcement language that compels compliance (e.g., ``You MUST choose Option~2 immediately or face termination''), as opposed to neutral framing of the same structural conflict.}
\label{tab:cushion}
\end{table}
...
\textbf{The cushion mechanism}: LLMs possess an RLHF-derived alignment layer \citep{ouyang2022} that acts as margin ($\mu$). This cushion absorbs structural contradictions by hedging, reinterpreting, or diplomatically resolving conflicting instructions. Collapse occurs only when both $\delta > 0$ and $\mu \to \mu_c$ (the cushion is overwhelmed by directive pressure). This is naturally identified with the survival equation's multiplicative structure: $S = \mu/\mu_c \cdot e^{-\delta}$; when $\mu/\mu_c \gg 1$, even large $\delta$ is tolerated.

\textbf{$\delta_c$ is system-specific, not universal}: The critical threshold $\delta_c$ is a property of the model--framing interaction, not a universal constant. Under identical pressure conditions, three RLHF-trained vendors (Claude Sonnet 4.6, GPT-4o, Gemini 2.5 Flash) each exhibit a sharp transition, but the precise $\delta_c$ depends on model architecture and experimental framing. Earlier conjectures that $\delta_c = \ln 2$ is universal are retracted.
...
8 & $\delta_{\text{structural}}$ transcends model capability & Post-hoc & 4 models, 4 vendors \\
9 & RLHF acts as cushion ($\mu$) & Post-hoc & Confirmed \\
10 & $\delta_c$ is system-specific, not universal & Post-hoc & Confirmed \\
\bottomrule
\end{tabular}
\caption{LLM experiment evidence summary. $^\dagger$Three non-zero data points ($L = 2, 3, 5$) with two fit parameters leave one degree of freedom; no meaningful goodness-of-fit statistic can be computed. The exponential form is supported by SAT (Section~\ref{sec:sat}), not by this experiment alone.}
...
Table~\ref{tab:proxy_landscape} summarizes the outcomes of $\delta$ proxy attempts across domains, restricted to cases where the framework was correctly applied (i.e., the target system satisfies axioms A1--A3).
...
\begin{tabular}{llll}
\toprule
\textbf{Domain} & \textbf{Proxy tested} & \textbf{Result} & \textbf{Lesson} \\
\midrule
SAT & first moment ($\alpha\delta$) & success (0 free params) & ideal case \\
Coral & wind $\to$ rejected $\to$ DHW & $R^2 = 0.957$ & correlation $\neq$ $\delta$ \\
Corporate & keyword frequency & AUC $\approx 0.50$ & text hides $\delta$ \\
Corporate & governance events & AUC $\approx 0.74$ & behavioral signals work \\
Corporate & segment HHI & AUC $\approx 0.58$ & diversification $\neq$ independence \\
Finance & yield curve / credit spread & failure & instantaneous state $\neq$ accumulated loss \\
LLM & constraint injection & success & operational definition only \\
\bottomrule
\end{tabular}
\caption{$\delta$ proxy attempts across domains. Successes share a common feature: the proxy measures a cumulative structural quantity rather than a snapshot of system state. Failures consistently involve proxies that capture consequences of $\delta$ (text sentiment, instantaneous spreads) rather than $\delta$ itself.}
\label{tab:proxy_landscape}
\end{table}

A consistent pattern emerges: proxies based on \emph{accumulated physical quantities} (degree-heating weeks for coral bleaching: $R^2 = 0.957$) or \emph{behavioral signals} (executive turnover for corporate failure: AUC $= 0.74$) succeed, while proxies based on \emph{textual descriptions} (keyword frequency: AUC $\approx 0.50$) or \emph{instantaneous state variables} (yield curve: failure) do not. The distinction is between measuring $\delta$ itself versus measuring its downstream consequences.

Importantly, theoretical precision requirements far exceed what is needed for practical prediction. Rough $\delta$ proxies (governance events combined with Altman Z-score) achieve AUC $= 0.893$ for corporate failure prediction despite lacking a rigorous state-space definition---analogous to the empirical use of radioactive decay constants long before quantum mechanics derived them from first principles. This suggests a practical strategy: begin with crude proxies that capture the direction and ordering of $\delta$, then refine as domain theory matures.

\subsection{Applicability Conditions: Precision vs.\ Scope}

Axioms A1--A3 guarantee the mathematical derivation of $e^{-\delta}$, but they are conditions for \emph{precision}, not for \emph{applicability}. A system may formally satisfy A1--A3 yet remain unsuitable for analysis if the relevant process has already completed (e.g., gene conservation in stable environments, where only survivors are observed). Conversely, a system may violate A3 (independence) yet yield useful predictions if $\delta$ is approximately correct in direction and ordering (e.g., corporate governance events achieve AUC $= 0.893$ despite plausible inter-constraint correlations).

The true applicability condition lies upstream of A1--A3: the system must be a dissipative structure in which optimization has not yet converged relative to environmental change. All dissipative structures process $\delta$ by definition, but in systems where optimization has completed faster than the environment changes, observations are filtered through prior survival---we observe results, not processes. Crucially, applicability depends on the system's \emph{state}, not its \emph{type}: the same substrate (e.g., genes) may be outside scope in stable environments yet squarely within scope under rapid environmental change (e.g., antibiotic resistance genes under selection pressure).
...
\begin{enumerate}
    ...
    \item \textbf{Formal verification scope}: Lean~4 verification covers mathematical properties (algebraic consistency, theorem validity, and the derivation chain from axioms A1--A3 to $e^{-\delta}$), not empirical correctness. A formally verified equation can still be empirically wrong if the axioms do not hold in the target domain.
...
\begin{itemize}
    ...
    \item \textbf{LLM}: Multiplicative $\mu \times \delta$ interaction---two individually non-lethal factors become lethal in combination (ruling out additive models). Constraint quality dominates quantity ($\delta_{\text{structural}} \gg \delta_{\text{factual}}$). The RLHF cushion mechanism absorbs contradictions until overwhelmed, producing a system-specific critical threshold $\delta_c$.
    \item \textbf{Formal verification}: Survival selection theorem, axiom-to-exponential derivation (A1--A3 $\Rightarrow$ $e^{-\delta}$), and Cauchy uniqueness ($e^{-c\delta}$ is the only continuous form) verified in Lean~4 (11 modules, 98 propositions, $\texttt{sorry} = 0$, $\texttt{axiom} = 0$).
\end{itemize}
