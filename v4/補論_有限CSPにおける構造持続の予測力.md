...
本補論は、構造持続理論の仕様固定構造層に属する有限制約充足問題において、累積段階損失 \(L\) が単なる制約数より強い予測情報を持つかを検証する。
...
構造持続理論の最小形式では、状態集合の縮小は対数段階損失として蓄積される。制約充足問題では、状態集合は候補割当ての集合であり、各制約はその集合を一定割合だけ削る。
...
これは重要である。以前の計算コスト補論では、条件つき発見確率 \(P(\mathrm{found}\mid\mathrm{SAT})\) と solver cost を扱った。しかし仕様固定構造層の第一モーメント構造が直接支配するのは、まず解空間の体積と feasibility であって、特定ソルバーの探索コストではない。
...
この G7 layer は、各 package の frozen 手順が著者環境外でも実行可能であり、qualitative support decision が修復されることを示す。Mixed-CSP と Exp43c q-coloring は、ともに現時点で 3 件の returned clean success を持つ。
