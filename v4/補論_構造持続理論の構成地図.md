...
これは普遍法則の閉包ではない。しかし、仕様固定構造層において、\(L\) / first-moment 型の法則側座標が raw baseline と比較され、その凍結済み検証パッケージが著者環境外で同じ判定関係出力を返したという意味で、local rerun や単なる手順公開より強い package-scoped replication support である。詳細は [`../v3/05_evidence/outside_reruns.tsv`](../v3/05_evidence/outside_reruns.tsv) と [`../analysis/g7_route_a_true_outside_replication_summary.md`](../analysis/g7_route_a_true_outside_replication_summary.md) に置く。
...
この層分けで重要なのは、文書の重要度を順位づけることではない。重要なのは、役割を混ぜないことである。たとえば LLM companion は主理論核の証明ではなく、主理論核を LLM 推論や継続学習へ写した構造推定層の観測的アンカーである。Foster-Lyapunov 補論は、新しい安定性定理の証明ではなく、既存の drift calculus が構造持続の純損失量 \(b_t\) へ条件付きに埋め込めることを示す bridge である。
...
4. `01_theory/10_log_ratio_accounting.md`
5. `01_theory/11_balance_accounting.md`
6. `CLAIMS.md`
...
| internal definition | B_n = sum_{t<n}(d_t-r_t) | 損失量 d_t から修復量 r_t を差し引いた純損失量の累積 |
