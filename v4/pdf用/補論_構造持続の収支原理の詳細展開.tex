...
本稿は、この修復を含む構造系を扱うために、構造持続の最小核を「段階損失量」から「段階損失量と修復量の収支」へ移す。各時刻の段階損失量を \(d_{t}\)、修復量を \(r_{t}\)、純損失量を
...
と書く。ここで \(B_{n}\) は時刻 \(n\) までの累積純損失量、\(M_{n}\) はその時点で明示的に追跡する維持資源、\(S_{n}\) は時刻 \(n\) における構造持続量である。必要なら \(R_{n}:=e^{-B_{n}}\) を生存可能性の持続率（残存比）と呼ぶ。集合値核だけを見る場合には、この式は

\[
m(V_{n}) = m(V_0) e^{-B_{n}}
\]

という最小収支恒等式になる。ここで \(b_{t}>0\) は段階損失優位、\(b_{t}=0\) は維持、\(b_{t}<0\) は修復優位を表す。したがって、本稿でいう「収支」は equilibrium を意味しない。崩壊・維持・修復の三つの傾向を、同じ差し引き量の符号として扱うための収支 \((accounting)\) の原理である。この三局面は均衡判定ではなく、後述するように、事前固定された観測単位の上での局所的な会計分類である。
...
構造を維持するとは、単に物質や資源が残っていることではない。ある機能、関係、同一性、または推論経路を保てる状態の余地が残っていることである。構造持続の最小形式では、この余地を生存可能集合 \(V_{t}\) の測度 \(m(V_{t})\) として表し、その縮小を対数比で測る損失側の最小核を孤立させた。構造持続の条件つき導出では、その核から指数表現をどこまで恒等式として読めるか、どこからが独立性や弱依存といった追加条件に依存するかを切り分けた。
...
本稿では、この差し引きを \textbf{構造持続の収支原理} と呼ぶ。英語では \textbf{Structural Persistence Balance} と呼ぶ。ただし、ここでの balance は equilibrium ではなく、収支、すなわち段階損失と修復の accounting を意味する。したがって「均衡法則」とは訳さない。収支が正であれば段階損失が優位であり、収支が負であれば修復が優位である。維持はその中間の一つの regime にすぎない。
...
この問いは、構造持続の最小核を否定するものではない。むしろ、その最小核を \(r_{t}=0\) の特例として回収する。そのうえで、repair, learning, redundancy, external support, rollback のような修復作用を、同じ対数尺度上の修復量 \(r_{t}\) として導入する。
...
ここで \(d_{t}\) は段階損失量、\(r_{t}\) は修復量、\(b_{t}\) は純損失量、\(B_{n}\) は累積純損失量である。この表記にすると、構造持続の最小形式の \(S=Me^{-L}\) は、修復量を明示しない \(r_{t}=0\) の場合、したがって \(B_{n}=L_{n}\) となる特例として回収される。言い換えると、修復量を入れない場合、累積純損失量は最小形式の累積情報損失、または累積段階損失量に一致する。修復量を含む場合には \(r_{t}\) が正でありうるため、\(B_{n}\) は増えるとは限らない。

\subsection{本稿の位置づけ}\label{ux672cux7a3fux306eux4f4dux7f6eux3065ux3051}

本稿は、M 分解を普遍理論の中核に据えるものではない。\(M\)（有効資源）の維持能力成分の分解は、補論「構造持続における M（有効資源）の操作的定式化」で扱う operational mapping layer である。すなわち、現実ドメインで \(r_{t}\) や修復能力をどのように測るかを与える層であって、本稿の主理論核そのものではない。

本稿が主理論 spine に与える核は、構造持続の最小形式の損失側の最小核を \(r_{t}=0\) の特例として回収しつつ、段階損失量 \(d_{t}\) と修復量 \(r_{t}\) の差し引きが、生存可能領域の指数的変化を支配するという収支恒等式である。
...
\item
  集合値力学補論は、収縮作用 \(K_{t}\) と修復作用 \(R_{t}\) の合成から、符号付き純損失量 \(B_{n}\) に対する指数核を与える。
\item
...
\item
  あらゆるドメインで自然な測度 \(m\)、段階損失量 \(d_{t}\)、修復量 \(r_{t}\) が一意に定まるとは主張しない。
\item
  \(r_{t}\) が無料で得られるとは主張しない。修復には資源制約、外部供給、時間遅れ、劣化、交絡がありうる。
\item
...
本稿が与えるのは、より限定された主張である。すなわち、事前固定された構造維持問題において、段階損失量と修復量が同じ対数尺度で定義できるなら、その差し引き \(b_{t}\) の累積純損失量 \(B_{n}\) に対して指数的な残存則が成り立つ。そして確率過程として扱う場合には、\(\mathbb E[b_{t}]\) の符号が傾向を与え、bounded increments や MGF などの追加条件があれば高確率境界へ進める、ということである。
...
\subsection{生存可能集合と二段階更新}\label{ux751fux5b58ux53efux80fdux96c6ux5408ux3068ux4e8cux6bb5ux968eux66f4ux65b0}

系 \(X\) と、各時刻 \(t\) における生存可能集合 \(V_{t}\subseteq X\) を考える。\(m\) は、各 \(V_{t}\) の比較に用いる有限測度である。本稿でも構造持続の最小形式と同様に、対象構造、測度、時間地平、観測単位は事前に固定されているものとする。
...
で定める。\(V_{t}^-\) は収縮直後の集合、\(V_{t+1}\) は修復後の集合である。

現実の系では、収縮と修復が同時または相互作用的に起きる場合がある。本稿ではそれを、観測単位ごとの構造粒度を落とした discrete step の内部で \(K_{t}\) と \(R_{t}\) の合成として畳み込む。

以後、考える有限時間地平の範囲で、\(m(V_{t})\) および \(m(V_{t}^-)\) は有限かつ正であると仮定する。この仮定は、以下の対数比が well-defined であるために必要である。

\subsection{段階損失量・修復量・純損失量}\label{ux6bb5ux968eux640dux5931ux91cfux4feeux5fa9ux91cfux7d14ux640dux5931ux91cf}
...
と定める。これは収縮作用 \(K_{t}\) が、その段階で生存可能領域をどれだけ削ったかを測る量である。\(K_{t}(V_{t})\subseteq V_{t}\) であるから、\(d_{t}\ge 0\) である。
...
と定める。これは修復作用 \(R_{t}\) が、収縮後の領域をどれだけ押し広げたかを測る量である。\(V_{t}^- \subseteq R_{t}(V_{t}^-)=V_{t+1}\) であるから、\(r_{t}\ge 0\) である。

ここで重要なのは、\(d_{t}\) と \(r_{t}\) がどちらも質量比の対数で測られていることである。これにより、段階損失と修復を同じ尺度で差し引ける。
...
と定める。\(L_{n}\) は loss-only での累積情報損失、または本稿の語彙での累積段階損失量である。\(R_{n}^{\mathrm{rec}}\) は累積修復量、\(B_{n}\) は累積純損失量である。

\subsection{純損失量の対数比表現}\label{ux7d14ux640dux5931ux91cfux306eux5bfeux6570ux6bd4ux8868ux73fe}
...
\tightlist
\item
  \(b_{t}>0\): 段階損失量が修復量を上回り、生存可能領域は縮小する。
\item
  \(b_{t}=0\): 段階損失量と修復量が釣り合い、測度上の維持が起きる。
\item
  \(b_{t}<0\): 修復量が段階損失量を上回り、生存可能領域は拡大する。
\end{itemize}
...
b_{t:t+2} \\
&= \\
-\log\frac{m(V_{t+2})}{m(V_{t})} \\
&= \\
b_{t}+b_{t+1}
\end{aligned}
\]

となるため、累積純損失量 \(B_{n}\) は時間併合に対して加法的に整合する。一方で、細かい粒度では「損失の後に修復」と読める経路が、粗い粒度では \(b_{t:t+2}=0\) の維持局面として読まれることがある。したがって、局面分類、collapse boundary、hitting-time bound を経験的または確率的に主張する場合には、対象構造だけでなく、観測単位、段階列、時間地平も P2 の一部として事前固定する必要がある。
...
\[
\frac{m(V_{n})}{m(V_0)} = \prod_{t=0}^{n-1} e^{-b_{t}} = e^{-\sum_{t=0}^{n-1} b_{t}} = e^{-B_{n}}.
\]
...
この定理は、指数型が loss-only 収縮モードに限られないことを示す。段階損失量と修復量を同じ対数尺度で測れる限り、残存量は累積純損失量 \(B_{n}\) に対して指数型で表される。
...
命題 3（loss-only 収縮モード）。 すべての時刻で修復量が存在しない、すなわち \(r_{t}=0\) であるなら、
...
確率空間 \((\Omega,\mathcal F,\mathbb P)\) 上で、各時刻の段階損失量 \(d_{t}(\omega)\)、修復量 \(r_{t}(\omega)\)、純損失量
...
\[
m(V_{n}(\omega)) = m(V_0(\omega)) e^{-B_{n}(\omega)}
\]
...
各 \(b_{t}\) が可積分であるとする。このとき累積純損失量の期待中心を
...
ここで「tendency」と呼ぶのは、期待中心 \(\bar B_{n}\) の向きを述べているからである。これは、個々の経路の単調性ではない。また、\(m(V_{n})\) の期待値がただちに \(m(V_0)e^{-\bar B_{n}}\) に等しいという主張でもない。一般に
...
である。したがって、本節の主張は、まず累積純損失量 \(B_{n}\) の期待中心に関するものである。
...
第一に、構造持続の収支原理の純損失量 \(b_{t}=d_{t}-r_{t}\) を確率変数として扱うと、その期待値の符号は累積純損失量 \(B_{n}\) の期待中心の向きを決める。

第二に、段階損失優位、維持、修復優位という三 regime は、同じ純損失量 \(b_{t}\) の符号として定義できる。
...
と同値である。したがって、有限時間 collapse event は、累積純損失量 \(B_{n}\) が閾値 \(B_\theta\) を越える事象として表せる。
...
\tightlist
\item
  期待値レベルの傾向: \(\mathbb E[b_{t}]\) または one-step production 条件から、累積純損失量の期待中心の向きを得る。
\item
...
次に必要なのは、この schema がどの concrete domain で自然に閉じるかである。仕様固定構造層では、SAT / Mixed-CSP / Bernoulli-CSP が最も硬い anchor であり、Exp43c q-coloring により SAT 構文の外側へ empirical support が一段広がった。構造推定層では、LLM companion I / II の scope-as-repair, external metabolism, dependency-aware replay が、修復量 \(r_{t}\) の observational indicator として扱われる。
...
が得られる。修復量を明示的に入れない loss-only finite CSP では \(r_{i}=0\) であり、\(B=L\) である。したがって
...
しかし、それは構造持続の収支原理が使えないという意味ではない。むしろ、構造推定層では \(r_{t}\) の observable indicator を慎重に設計し、期待値レベルまたは prospective prediction の形で検査する必要がある。次節では、LLM companion I / II の scope-as-repair, external metabolism, dependency-aware replay を、修復量 \(r_{t}\) の構造推定層 anchor として整理する。
...
\tightlist
\item
  未整理矛盾、裸の競合値、前提更新、依存不整合などを損失側指標として置く。
\item
  scope marker、source attribution、外部代謝、依存 DAG controller、空間分離などを修復側指標として置く。
\item
...
ここで \(D_{\mathrm{obs}}\) は観測された段階損失指標、\(R_{\mathrm{obs}}^{\mathrm{rec}}\) は観測された修復指標である。\(B_{\mathrm{eff}}\) は、§2 の \(B_{n}\) と同じ強度の pathwise quantity ではない。あくまで、観測可能な推定量である。
...
構造持続の収支原理の語彙では、\texttt{subtle} や \texttt{structural} は損失側指標である。これに対して \texttt{scoped} は、競合値を task 外へ範囲づける文脈内の修復側指標として読める。つまり、同じ「矛盾らしき文」が存在しても、その衝突がどの範囲に属するかを整理することで、有効な \(R_{\mathrm{obs}}^{\mathrm{rec}}\) が増え、観測収支 \(B_{\mathrm{eff}}\) が下がる、という読みである。
...
LLM companion II が構造推定層に与える教訓は、単なる reconfiguration と repair を分ける必要がある、という点である。LoRA は新しい信号に反応して表現を再構成するが、依存構造を自律的に修復するとは限らない。F-v2c は依存整合性を改善するが、旧知識保持を修復しない。F-multi は保持を一部改善するが、完全な dependency repair ではない。したがって、\(r_{t}\) は単一の「資源量」ではなく、どの種類の修復がどの段階損失に効いているかを区別して測る必要がある。
...
\item
  修復指標を追加した baseline + SP model が、domain baseline に対する out-of-sample 予測力の増分を持つかを検査できる。
\item
...
この対応は、G6-a の analogy としては明確である。閉じた loss-only 系では、一方向の累積量が増え、生存可能性が下がる。開いた系では、修復量 \(r_{t}\) が入り、\(b_{t}=d_{t}-r_{t}\) の符号によって崩壊、維持、修復の三 regime が分かれる。この点は、開放系が外部から自由エネルギーや資源を取り入れて秩序構造を維持するという直感と対応する。
...
したがって、熱力学との接続は現時点では G6-a から G6-b の範囲である。具体的な物理系を取り、\(V_{t}\) を「対象構造を保つ微視状態集合」、\(m\) を熱力学的 multiplicity または path measure、\(r_{t}\) を外部駆動や自由エネルギー供給に対応させれば、より強い correspondence が作れる可能性はある。しかし、本稿はそれを一般に証明しない。
...
\bottomrule\noalign{}
\endlastfoot
dissipative production / entropy production & 段階損失量 (d\_t) \\
external driving / resource throughput & 修復量 (r\_t) \\
...
この correspondence は有用である。しかし、非平衡熱力学は物理的保存則、局所詳細釣り合い、熱浴、化学ポテンシャルなどの具体的構造を持つ。本稿の \(r_{t}\) は、それらをすべて抽象化した修復座標であり、それ自体が物理的流量であるとは限らない。
...
§2.2 の \(d_{t},r_{t}\ge 0\) という二段階 sign convention に合わせて読むなら、\(Z_{t}\) の増加分を段階損失量 \(d_{t}\)、減少分を修復量 \(r_{t}\) に分ければよい。その差し引きが、ここで直接定義した \(b_{t}=Z_{t+1}-Z_{t}\) に一致する。
...
確率制御では、制御入力 \(u_{t}\) によって状態遷移やコストを変え、ある Lyapunov 関数や value function の drift を望ましい向きに保つ。構造持続の収支原理で言えば、制御入力は修復量 \(r_{t}\) を変える作用として読める。
...
\item
  対象構造を事前に固定する。
\item
  生存可能集合 \(V\) または観測量 \(Y\) を事前に固定する。
\item
  測度 \(m\)、時間地平、更新単位を事前に固定する。
\item
  損失側と修復側を同じ対数尺度または対応する観測指標で分ける。
\item
...
になる。生存可能集合 \(V_{t}\)、測度 \(m\)、収縮作用 \(K_{t}\)、修復作用 \(R_{t}\) が事前に固定され、対数比が well-defined なら、厳密な finite-prefix 表記として
...
第二に、期待値レベルの傾向である。\(b_{t}\) を確率変数として扱えるなら、\(\mathbb E[b_{t}]\) の符号は、累積純損失量の期待中心がどちらへ動くかを与える。これは high-probability collapse ではなく、中心の方向に関する主張である。
...
ここでいう positive finite trajectory assumptions は、各段階の \(m(V_{t})\) と中間質量が正であり、対数比が well-defined であるという仮定である。この仮定は測度 \(m\) の自然性を証明するものではない。対象構造 \(V\)、測度 \(m\)、段階損失量 \(d_{t}\)、修復量 \(r_{t}\) を各ドメインで事前固定できるかは、Lean ではなく運用上の gate である。
...
\item
  観測された association を causal proof と呼ばない。
\item
  熱力学、情報理論、queueing theory、確率制御を置き換えるとは主張しない。 \# 探索的に発見された対象構造、測度、段階損失量、修復量の候補を、そのまま support と呼ばない。
\end{enumerate}
...
\tightlist
\item
  構造持続の最小形式: 修復を明示しない最小核。
\item
  構造持続の収支原理: 段階損失量と修復量を含む主原理。
\item
...
\item
  M 補論: \(r_{t}\) や修復能力を実ドメインで測る operational mapping。
\end{enumerate}
...
この式は、構造持続の最小核を \(r_{t}=0\) の特例として回収し、修復を含む場合には修復量 \(r_{t}\) によって崩壊、維持、修復の三 regime を同じ座標上で扱う。期待値レベルでは \(\mathbb E[b_{t}]\) の符号が傾向を与え、追加の concentration 条件があれば finite-horizon collapse / hitting-time bound へ進める。
