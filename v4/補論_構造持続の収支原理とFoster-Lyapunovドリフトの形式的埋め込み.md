...
本補論の主張は限定的である。Foster-Lyapunov theorem の正再帰性定理や幾何的エルゴード性を再証明するものではない。Markov 性、irreducibility、小集合条件、moment 条件など、元理論の仮定はそのまま保持される。本補論が示すのは、Foster-Lyapunov drift calculus の負荷差分が、構造持続の収支原理の純損失量 \(b_t\) として読める、という最小の G6-c formal embedding である。

この最小埋め込みは Lean 側でも `Survival/LyapunovBalanceEmbedding.lean` として形式化されている。Lean が証明しているのは、望遠鏡和、指数的維持量の局所恒等式、正負部分による \(b_t=d_t-r_t\) 分解、および `QueueStability.lean` の excess demand を純損失量として読む wrapper である。positive recurrence や \(R_n \le R_0 e^{-cn}\) 型の指数減衰境界は、次の iteration の対象であり、本補論の範囲外である。


1. 目的と位置づけ

構造持続の収支原理は、段階損失量 \(d_t\) と修復量 \(r_t\) の差し引きを
\[
  b_t = d_t - r_t
\]
と置き、その累積純損失量
...
本稿群では、既存理論との接続を G6-a / G6-b / G6-c の三段階に分ける。この分類は主論文本文の主線ではなく、運用規律および詳細展開補論に置かれた接続強度の整理である。熱力学や情報理論との対応は、多くの場合 G6-a または G6-b に留まる。一方、queueing theory や Markov chain stability に現れる Foster-Lyapunov drift 条件は、構造持続の収支原理の純損失量 \(b_t\) へ直接埋め込める。
...
\begin{quote}
Foster-Lyapunov drift calculus の負荷差分は、構造持続の収支原理の純損失量 \(b_t\) として読める。したがって、その expectation-level drift 条件は、構造持続の収支原理の recovery / collapse tendency の特例として埋め込める。
\end{quote}
...
これは構造持続の収支原理の累積純損失量と同じ形式である。\(Z_t\) が増えると、負荷が増える。\(Z_t\) が減ると、修復が勝っている。
...
構造持続の収支原理の標準形では、純損失量を
\[
  b_t=d_t-r_t
\]
と書く。ここで \(d_t\ge 0\) は段階損失量、\(r_t\ge 0\) は修復量である。
...
したがって、負荷が増えるステップは段階損失量として、負荷が減るステップは修復量として読める。

この読み替えは、物理的資源入力を同定したという意味ではない。あくまで、Lyapunov 負荷の増減を構造持続の収支原理の符号つき純損失量へ写す最小的な分解である。
...
queue の累積純損失量と累積 overload loss の対応は `queue_cumulativeAction_eq_cumulativeOverloadLoss`、
\(\mu\ge\lambda\) 側の純損失量は `queue_increment_nonpos_of_stable`、
\(\lambda<\mu\) でない側の純損失量は `queue_increment_pos_of_overloaded` に対応する。
...
第二に、仕様固定構造層 CSP calibration とは別の方向で、構造持続の収支原理の generality を強化する。random CSP の threshold-local grid に依存せず、負荷、修復入力、過負荷という語彙を既存の安定性理論へ接続できる。
...
本補論は、構造持続の収支原理の G6-c iteration 1 を reader-facing な形で閉じる。閉じたのは、Foster-Lyapunov drift calculus の最小代数的埋め込みである。すなわち、Lyapunov 負荷 \(Z_t\)、純損失量 \(b_t\)、累積純損失量 \(B_n\)、相対維持量 \(R_t\)、段階損失量 \(d_t\)、修復量 \(r_t\) の対応である。
